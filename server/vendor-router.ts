...
const vendorTechStackComponentSchema = z.object({
  ...
  componentType: z
    .string()
    .trim()
    .min(2, "Component type must be at least 2 characters")
    .max(120),
...
const vendorInputSchema = z
  .object({
    ...
    cloudProviders: z
      .array(z.enum(vendorCloudProviderValues))
      .max(6)
      .default([]),
    ...
    regulatoryJurisdictions: z
      .array(z.enum(vendorJurisdictionValues))
      .min(1)
      .max(6),
    certifications: z
      .array(z.enum(vendorComplianceStandardValues))
      .max(10)
      .default([]),
    dataProcessingActivities: z
      .array(z.enum(vendorDataProcessingActivityValues))
      .min(1)
      .max(10),
    ...
    techStackComponents: z
      .array(vendorTechStackComponentSchema)
      .max(20)
      .default([]),
  })
  .superRefine((input, ctx) => {
    if (
      ...
      ctx.addIssue({
        ...
        message:
          "At least one cloud provider is required for public-cloud or hybrid hosting.",
        path: ["cloudProviders"],
      });
    }
  });
...
export const vendorRouter = router({
  ...
  create: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      void recordActivity({
        ...
        console.warn(
          "[Activity] Failed to record vendor_profile_created",
          error
        );
      });
  ...
  patch: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existing = await getVendorProfileById(
        vendorId,
        ctx.user.id,
        ctx.organizationId
      );
      ...
      const vendor = await patchVendorBasicFields(
        vendorId,
        ctx.user.id,
        patch,
        ctx.organizationId
  ...
  assess: activeOrgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await requireModulePermission(ctx, "vendor_assessment", "canCreate");
      const startedAt = Date.now();
      const vendor = await getVendorProfileById(
        input.vendorId,
        ctx.user.id,
        ctx.organizationId
      );
      ...
      void createAdminNotification({
        ...
        console.warn(
          "[Notification] Failed to create assessment notification",
          error
        );
      });
  ...
  assessDraft: activeOrgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const draftVendor: Vendor = {
        ...
        regulatoryJurisdictions: serializeVendorMultiValue(
          input.regulatoryJurisdictions
        ),
        certifications: serializeVendorMultiValue(input.certifications),
        dataProcessingActivities: serializeVendorMultiValue(
          input.dataProcessingActivities
        ),
  ...
  report: activeOrgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      void recordActivity({
        ...
        console.warn(
          "[Activity] Failed to record assessment_report_exported",
          error
        );
      });
  ...
  gapSummary: orgProcedure.query(async ({ ctx }) => {
    await requireModulePermission(ctx, "vendor_assessment", "canView");
    const { runDualJurisdictionAssessment } = await import(
      "./supplier-assessment"
    );
