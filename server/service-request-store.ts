...
export async function listRequests(
  ...
  if (!db || orgId < 0) {
    return [...MEM_REQUESTS.filter(r => r.organizationId === orgId)].sort(
  ...
  return db
    ...
    .orderBy(desc(serviceRequests.createdAt)) as unknown as Promise<
    ServiceRequestRow[]
  >;
}
...
export async function getRequest(
  ...
  if (!db || orgId < 0) {
    return (
      MEM_REQUESTS.find(r => r.id === id && r.organizationId === orgId) ?? null
    );
  }
  const [row] = await db
    .select()
    .from(serviceRequests)
    .where(
      and(eq(serviceRequests.id, id), eq(serviceRequests.organizationId, orgId))
    )
    .limit(1);
...
export async function createRequest(
  ...
  const db = await getDb();
  const now = new Date();
  const preferredStartDate = input.preferredStartDate
