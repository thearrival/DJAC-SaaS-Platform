...
export function computeExploitabilityScore(
  ...
  const avgSim =
    simulations.length > 0
      ? simulations.reduce((s, r) => s + r.successProbability, 0) /
        simulations.length
      : 0;
...
export function computeBusinessImpactScore(
  asset: Pick<
    CtemAsset,
    "handlesPersonalData" | "handlesCriticalData" | "criticalityScore"
  >
): number {
...
export function computeRiskScore(
  ...
  vulns: Pick<
    CtemVulnerability,
    "severity" | "isPatched" | "exploitAvailable"
  >[],
  simulations: Pick<CtemAttackSimulation, "successProbability">[]
...
export function deriveExposureMappings(
  ...
  if (
    input.assetHandlesPersonalData &&
    /data|leak|exfil|exposure|sensitive|personal|PII|privacy/i.test(titleLower)
  ) {
    if (isSaudiRegion) {
      results.push({
        vulnerabilityId: input.vulnerabilityId,
...
export async function executeContinuousComplianceRun(params: {
  ...
  if (!runId) {
    const [insertedRun] = await db
      .insert(continuousComplianceRuns)
      .values({
        organizationId,
  ...
  try {
    ...
    const prevScores = await db
      .select({
        assetId: ctemRiskScores.assetId,
        finalPriorityScore: ctemRiskScores.finalPriorityScore,
      })
      .from(ctemRiskScores)
      .where(inArray(ctemRiskScores.assetId, assetIds));
    const prevScoreMap = new Map(
      prevScores.map(s => [s.assetId, s.finalPriorityScore])
    );
    ...
    for (const asset of assets) {
      ...
      exploitable += vulns.filter(
        v => v.exploitAvailable && !v.isPatched
      ).length;

      const scored = computeRiskScore(asset, vulns, sims);
      const prev = prevScoreMap.get(asset.id) ?? null;
      const delta =
        prev !== null ? (scored.finalPriorityScore ?? 0) - prev : null;

      // Drift alert: >20 point increase OR new critical exploitable vuln
      if (
        (delta !== null && delta > 20) ||
        vulns.some(
          v => v.severity === "critical" && v.exploitAvailable && !v.isPatched
        )
      ) {
        alertRaised = true;
      }
...
function simulateInMemoryRun(params: {
  ...
  const assetResults: RunResult["assetResults"] = Array.from(
    { length: numAssets },
    (_, i) => {
  ...
  const avgScore = Math.round(
    assetResults.reduce((s, r) => s + r.finalScore, 0) / numAssets
  );
