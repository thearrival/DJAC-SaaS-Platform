...
// ---------------------------------------------------------------------------
const memShares = new Map<string, { share: ReportShare; expiresAt: Date }>();
...
export async function getReportShareByToken(
  ...
  const [row] = await db
    .select()
    .from(reportShares)
    .where(and(eq(reportShares.token, token), gt(reportShares.expiresAt, now)))
    .limit(1);
