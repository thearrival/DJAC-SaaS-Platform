...
export const orgMembersRouter = router({
  ...
  invite: orgAdminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const isDuplicate = await checkDuplicateInvite(
        ctx.organizationId,
        inviteEmail
      );
      if (isDuplicate) {
        throw new TRPCError({
          code: "CONFLICT",
          message:
            "This email already has a membership or pending invite in your organization.",
        });
      }
...
<div style="font-family:sans-serif;max-width:560px;margin:auto">
  ...
  lookupInvite: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const expired =
        Date.now() - new Date(row.createdAt).getTime() > SEVEN_DAYS;
