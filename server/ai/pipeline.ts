...
const DEFAULT_KNOWN_FRAMEWORKS = [
  "PIPL",
  "CSL",
  "DSL",
  "MLPS 2.0",
...
async function buildRagContext(facts: ExtractedFact[]): Promise<RagControl[]> {
  ...
  const tokenSet = new Set(
    facts.flatMap(fact =>
      toTokens(`${fact.key} ${fact.value} ${fact.evidence}`)
    )
  );

  const scored = controls
    .map(control => {
      const haystack = `${control.controlCode} ${control.controlName || ""} ${
        control.category || ""
...
function _mergeRecommendations(
  existing: string[],
  additional: string[]
): string[] {
...
export async function executeAssessmentPipeline(
  ...
  while (!validatorPassed && attempts < maxAttempts) {
    ...
    const validation =
      externalValidation ||
      runValidator(assessment, remediationPlan, ragControls);
