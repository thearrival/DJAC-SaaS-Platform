  ...
  setWorker(worker: JobWorker): void {
    ...
    this.worker = new Worker(
      ...
      },
      {
        connection: {
          url:
            this.queue.opts.connection &&
            typeof this.queue.opts.connection === "object" &&
            "url" in this.queue.opts.connection
    ...
    this.worker.on("failed", (job, error) => {
      ...
      record.error =
        error?.message || job.failedReason || "Unknown queue failure.";
  ...
  async getHistoryDiagnostics(
    ...
    return {
      ...
      ...(snapshots[0]?.updatedAt
        ? { lastUpdatedAt: snapshots[0].updatedAt }
        : {}),
      details:
