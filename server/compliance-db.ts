...
async function loadFallbackData(): Promise<ComplianceFallbackData> {
  ...
  const fallbackFrameworks: FrameworkRow[] =
    referenceModule.complianceFrameworks.map((framework, index) => ({
      id: index + 1,
      code: framework.code,
      name: framework.name,
  ...
  referenceModule.complianceRelationships.forEach((relationship, index) => {
    ...
    const targetFrameworkId = frameworkIdByCode.get(
      relationship.targetFrameworkCode
    );
    ...
    fallbackRelationships.push({
      ...
      targetFrameworkId,
      relationshipType:
        relationship.relationshipType as RelationshipRow["relationshipType"],
...
export async function getFrameworkByCode(code: string) {
  ...
  if (!db) {
    ...
    return (
      fallback.frameworks.find(framework => framework.code === code) ?? null
    );
  }
...
export async function getFrameworksByCountry(country: string) {
  return withReferenceCache(
    `frameworks:country:${country.toLowerCase()}`,
    async () => {
      const db = await getDb();
      if (!db) {
        const fallback = await loadFallbackData();
        return fallback.frameworks.filter(
          framework => framework.country === country
        );
      }
      ...
      const rows = await db
        .select()
        .from(frameworks)
        .where(eq(frameworks.country, country));
      if (rows.length > 0) {
        return rows;
      }
...
export async function getControlsByFramework(frameworkId: number) {
  return withReferenceCache(`controls:framework:${frameworkId}`, async () => {
    ...
    if (!db) {
      const fallback = await loadFallbackData();
      return fallback.controls.filter(
...
export async function getFrameworkRelationships(sourceFrameworkId: number) {
  return withReferenceCache(
    `relationships:framework:${sourceFrameworkId}`,
    async () => {
      const db = await getDb();
      if (!db) {
        const fallback = await loadFallbackData();
    ...
    relationshipType === "coordination" || relationshipType === "harmonization"
      ? await db
          ...
          .where(
            or(
              eq(
                frameworkRelationships.relationshipType,
      ...
      : await db
          ...
          .from(frameworkRelationships)
          .where(
            eq(frameworkRelationships.relationshipType, relationshipType as any)
          );
...
export async function getComplianceComparison(
  ...
  return withReferenceCache(cacheKey, async () => {
    ...
    if (
      !db ||
      allFrameworkRows.length === 0 ||
      allRelationshipRows.length === 0
    ) {
      const fallback = await loadFallbackData();
    ...
    const framework1 = allFrameworkRows.find(
      framework => framework.id === framework1Id
    );
    const framework2 = allFrameworkRows.find(
      framework => framework.id === framework2Id
    );

    if (!framework1 || !framework2) return null;

    const controls1 = allControlRows.filter(
      control => control.frameworkId === framework1Id
    );
    const controls2 = allControlRows.filter(
      control => control.frameworkId === framework2Id
    );

    const rawRelationships = allRelationshipRows.filter(
      row =>
        (row.sourceFrameworkId === framework1Id &&
          row.targetFrameworkId === framework2Id) ||
        (row.sourceFrameworkId === framework2Id &&
          row.targetFrameworkId === framework1Id)
    );
    ...
    const relationships = rawRelationships
      .map(row => {
        ...
        return {
          ...enriched,
          sourceDirection: isReverseDirection
            ? framework2.code
            : framework1.code,
          targetDirection: isReverseDirection
            ? framework1.code
            : framework2.code,
        };
      })
...
export async function getComplianceMatrix() {
  return withReferenceCache("matrix:all", async () => {
    ...
    for (const row of allRelationships) {
      ...
      if (
        SEVERITY_RANK[normalized.severity] > SEVERITY_RANK[entry.maxSeverity]
      ) {
