...
export async function listAssessments(orgId: number): Promise<AssessmentRow[]> {
  ...
  if (!db || orgId < 0) {
    return [...MEM_ASSESSMENTS.filter(a => a.organizationId === orgId)].sort(
  ...
  return db
    ...
    .orderBy(desc(securityMaturityAssessments.createdAt)) as unknown as Promise<
    AssessmentRow[]
  >;
}
...
export async function getLatestAssessment(
  ...
  if (!db || orgId < 0) {
    const all = [
      ...MEM_ASSESSMENTS.filter(a => a.organizationId === orgId),
...
export async function getAssessment(
  ...
  if (!db || orgId < 0) {
    return (
      MEM_ASSESSMENTS.find(a => a.id === id && a.organizationId === orgId) ??
      null
    );
  }
  const [row] = await db
...
export async function createAssessmentRow(
  ...
  const [inserted] = await db
    .insert(securityMaturityAssessments)
    .values({
      organizationId: orgId,
      title: input.title,
      frameworkRef: input.frameworkRef ?? null,
...
export async function deleteAssessmentRow(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const idx = MEM_ASSESSMENTS.findIndex(
