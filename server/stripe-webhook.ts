...
function resolveTier(
  ...
  return (
    PRICE_CATALOG.find(
      tier => tier.plan === plan && tier.interval === interval
    ) ?? null
  );
}
  ...
  switch (event.type) {
    ...
    case "invoice.payment_succeeded": {
      ...
      const orgResult = await db
        ...
        .from(organizations)
        .where(
          eq(organizations.stripeCustomerId, String(invoice.customer ?? ""))
        )
        .limit(1);
      ...
      if (!planInfo && priceId) {
        console.error(
          "[Stripe Webhook] Unknown price ID — plan not resolved:",
          priceId
        );
      }
      if (subId && planInfo) {
    ...
    case "customer.subscription.updated": {
      ...
      if (!planInfo && priceId) {
        console.error(
          "[Stripe Webhook] Unknown price ID on subscription update — plan not resolved:",
          priceId
        );
      }
      if (planInfo) {
