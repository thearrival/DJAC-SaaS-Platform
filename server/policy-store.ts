...
export async function listPolicies(orgId: number): Promise<PolicyRow[]> {
  ...
  if (!db || orgId < 0) {
    return [...MEM_POLICIES.filter(p => p.organizationId === orgId)].sort(
  ...
  return db
    ...
    .orderBy(desc(compliancePolicies.createdAt)) as unknown as Promise<
    PolicyRow[]
  >;
}
...
export async function patchPolicy(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const policy = MEM_POLICIES.find(
  ...
  const existing = await db
    ...
    .where(
      and(
        ...
        eq(compliancePolicies.organizationId, orgId)
      )
    );
...
export async function getPolicyStatus(
  ...
  if (!db || orgId < 0) {
    return (
      MEM_POLICIES.find(p => p.id === id && p.organizationId === orgId)
        ?.status ?? null
    );
  }
  const rows = await db
...
export async function deletePolicy(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const idx = MEM_POLICIES.findIndex(
      p => p.id === id && p.organizationId === orgId
    );
