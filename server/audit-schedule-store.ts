...
export async function listAudits(orgId: number): Promise<AuditRow[]> {
  ...
  if (!db || orgId < 0) {
    return MEM_AUDITS.filter(a => a.organizationId === orgId).sort(
  ...
  return db
    ...
    .orderBy(asc(auditSchedules.scheduledDate)) as unknown as Promise<
    AuditRow[]
  >;
}
...
export async function patchAudit(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const idx = MEM_AUDITS.findIndex(
      a => a.id === id && a.organizationId === orgId
    );
    if (idx >= 0)
      Object.assign(MEM_AUDITS[idx], updates, { updatedAt: new Date() });
    return;
  }
  await db
    ...
    .where(
      and(eq(auditSchedules.id, id), eq(auditSchedules.organizationId, orgId))
    );
}
