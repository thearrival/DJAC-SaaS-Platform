...
export async function getOrgScorecard(orgId: number, userId: number) {
  ...
  if (!db) {
    const inMemVendors = await listVendorProfiles(
      userId,
  ...
  const fwMap = new Map<
    string,
    { code: string; name: string; scores: number[]; count: number }
  >();
  for (const r of latest) {
    if (!fwMap.has(r.frameworkCode)) {
      fwMap.set(r.frameworkCode, {
        code: r.frameworkCode,
  ...
  const frameworkList = Array.from(fwMap.values()).map(fw => ({
    ...
    avgScore:
      fw.scores.length > 0
        ? Math.round(fw.scores.reduce((a, b) => a + b, 0) / fw.scores.length)
        : 0,
    count: fw.count,
  }));
  ...
  const gapRows = await db
    ...
    .innerJoin(
      vendorAssessments,
      eq(assessmentGaps.assessmentId, vendorAssessments.id)
    )
    .innerJoin(vendors, eq(vendorAssessments.vendorId, vendors.id))
    .where(eq(vendors.organizationId, orgId));
  ...
  const reportRows = await db
    ...
    .where(
      and(
        eq(complianceReports.organizationId, orgId),
        eq(complianceReports.status, "ready")
      )
    )
