...
export async function listAssets(orgId: number): Promise<AssetRow[]> {
  ...
  if (!db || orgId < 0) {
    return [...MEM_ASSETS.filter(a => a.organizationId === orgId)].sort(
...
export async function getAsset(
  ...
  if (!db || orgId < 0) {
    return (
      MEM_ASSETS.find(a => a.id === id && a.organizationId === orgId) ?? null
    );
  }
  const [row] = await db
    .select()
    .from(assetInventory)
    .where(
      and(eq(assetInventory.id, id), eq(assetInventory.organizationId, orgId))
    )
    .limit(1);
...
export async function getAssetForPatch(
  orgId: number,
  id: number
): Promise<AssetRow | null> {
  const db = await getDb();
...
export async function deleteAsset(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const idx = MEM_ASSETS.findIndex(
      a => a.id === id && a.organizationId === orgId
    );
...
export async function getAllOrgAssets(orgId: number): Promise<AssetRow[]> {
  ...
  return db
    .select()
    .from(assetInventory)
    .where(eq(assetInventory.organizationId, orgId)) as unknown as Promise<
    AssetRow[]
  >;
}
