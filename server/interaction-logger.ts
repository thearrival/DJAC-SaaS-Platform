...
const SENSITIVE_KEY_REGEX =
  /(password|secret|token|authorization|cookie|api[_-]?key|access[_-]?key|private[_-]?key)/i;
...
export async function recordUserInteraction(
  ...
  const localUserId =
    (ctx.user as { localUserId?: number } | null)?.localUserId ?? null;
  const userId = ctx.user && ctx.user.id > 0 ? ctx.user.id : null;
  const organizationId =
    ctx.organizationId && (ctx.organizationId as number) > 0
      ? ctx.organizationId
      : null;
