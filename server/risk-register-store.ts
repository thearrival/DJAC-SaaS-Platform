...
export type PatchRiskFields = Partial<
  Omit<CreateRiskInput, "reviewDate" | "vendorId" | "ownerId">
> & { vendorId?: number | null; ownerId?: number | null };
...
export async function listRisks(orgId: number): Promise<RiskRow[]> {
  ...
  if (!db || orgId < 0) {
    return MEM_RISKS.filter(r => r.organizationId === orgId).sort(
...
export async function patchRisk(
  ...
  await db
    ...
    .set(updates)
    .where(
      and(eq(riskRegister.id, id), eq(riskRegister.organizationId, orgId))
    );
  return { id };
}
