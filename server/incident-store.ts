...
export async function listIncidents(orgId: number): Promise<IncidentRow[]> {
  ...
  if (!db || orgId < 0) {
    return [...MEM_INCIDENTS.filter(i => i.organizationId === orgId)].sort(
  ...
  return db
    ...
    .orderBy(desc(complianceIncidents.createdAt)) as unknown as Promise<
    IncidentRow[]
  >;
}
...
export async function patchIncident(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const incident = MEM_INCIDENTS.find(
...
export async function getIncidentStatus(
  ...
  if (!db || orgId < 0) {
    return (
      MEM_INCIDENTS.find(i => i.id === id && i.organizationId === orgId)
        ?.status ?? null
    );
  }
  const rows = await db
...
export async function deleteIncident(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const idx = MEM_INCIDENTS.findIndex(
      i => i.id === id && i.organizationId === orgId
    );
