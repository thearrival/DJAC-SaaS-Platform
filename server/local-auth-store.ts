...
export async function findLocalUserByPhone(
  phone: string
): Promise<LocalUser | null> {
  const db = await getDb();
  if (!db) {
  ...
  const [row] = await db
    .select()
    .from(localUsers)
    .where(eq(localUsers.phoneNumber, phone))
    .limit(1);
  return row ?? null;
}
...
export async function checkEmailExists(email: string): Promise<boolean> {
  ...
  if (!db)
    return (
      isLocalMemoryFallbackEnabled() &&
      localMemoryUsers.some(u => u.email === email)
    );
  const rows = await db
...
export async function checkPhoneExists(phone: string): Promise<boolean> {
  ...
  if (!db)
    return (
      isLocalMemoryFallbackEnabled() &&
      localMemoryUsers.some(u => u.phoneNumber === phone)
    );
  const rows = await db
    .select({ id: localUsers.id })
    .from(localUsers)
...
export async function listLocalUsersForAdmin(): Promise<
  Pick<
    LocalUser,
    | "id"
    | "name"
...
> {
  const db = await getDb();
  if (!db)
    return isLocalMemoryFallbackEnabled()
      ? localMemoryUsers.map(u => ({
          id: u.id,
          name: u.name,
          email: u.email,
...
export async function updateLocalUserTotpSecret(
  id: number,
  secret: string
): Promise<void> {
  const db = await getDb();
...
export async function enableLocalUserMfa(
  id: number,
...
export async function consumeLocalUserBackupCode(
  id: number,
