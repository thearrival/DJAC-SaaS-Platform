...
export function calcCompositeScore(params: {
  ...
  const base =
    params.avgAssessmentScore !== null
      ? 40 * (params.avgAssessmentScore / 100)
      : 60;
  const deductions =
    params.criticalGaps * 8 +
    params.openGaps * 2 +
    params.criticalRisks * 6 +
    params.openRisks * 1.5 +
    params.criticalIncidents * 5 +
    params.openIncidents * 1 +
...
export async function getVendorComplianceList(orgId: number) {
  ...
  const [riskRows, remRows, incRows] = await Promise.all([
    db
      .select({
        vendorId: riskRegister.vendorId,
  ...
  return vendorRows
    .map(v => {
      ...
      const vGaps = gapRows.filter(
        g => g.assessmentId != null && vGapIds.has(g.assessmentId!)
      );
      const openGaps = vGaps.length;
      const criticalGaps = vGaps.filter(g => g.severity === "critical").length;

      const vRisks = riskRows.filter(
        r =>
          r.vendorId === v.id &&
          r.status !== "closed" &&
          r.status !== "accepted"
      );
      const criticalRisks = vRisks.filter(
        r => (r.likelihood ?? 1) * (r.impact ?? 1) >= 15
      ).length;

      const vRem = remRows.filter(
        r =>
          r.vendorId === v.id &&
          r.status !== "resolved" &&
      ...
      const vInc = incRows.filter(
        i =>
          i.affectedVendorId === v.id &&
          i.status !== "closed" &&
          i.status !== "resolved"
      );
      const criticalIncidents = vInc.filter(
        i => i.severity === "critical"
      ).length;
      const openIncidents = vInc.length;

      const scores = vAssess
      ...
      return {
        ...
        riskTier: v.riskTier ?? null,
        assessmentCount: vAssess.length,
        avgAssessmentScore,
        openGaps,
        criticalGaps,
        openRisks: vRisks.length,
        ...
        criticalIncidents,
        compositeScore,
        riskLevel: calcRiskLevel(compositeScore),
      };
    })
    .sort((a, b) => a.compositeScore - b.compositeScore);
}

export async function getVendorComplianceProfile(
  orgId: number,
  vendorId: number
) {
  const db = await getDb();
  ...
  const [assessRows, riskRows, remRows, incRows] = await Promise.all([
    ...
    db
      ...
      .where(
        and(
          eq(riskRegister.organizationId, orgId),
          eq(riskRegister.vendorId, vendorId)
        )
      ),
    db
      .select()
      ...
      .where(
        and(
          eq(remediationTasks.organizationId, orgId),
          eq(remediationTasks.vendorId, vendorId)
        )
      ),
    db
      .select()
      ...
      .where(
        and(
          eq(complianceIncidents.organizationId, orgId),
          eq(complianceIncidents.affectedVendorId, vendorId)
        )
      ),
  ]);
  ...
  return {
    vendor: {
      ...
      headquartersLocation: vendor.headquartersLocation ?? null,
      riskTier: vendor.riskTier ?? null,
    },
