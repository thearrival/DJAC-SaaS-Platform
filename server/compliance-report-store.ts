...
export async function getComplianceSummary(orgId: number) {
  ...
  return {
    ...
    risks: {
      ...
      critical: rRows.filter(r => (r.likelihood ?? 1) * (r.impact ?? 1) >= 15)
        .length,
    },
    remediation: {
...
export async function getComplianceModuleData(
  orgId: number,
