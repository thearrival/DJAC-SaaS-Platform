...
export const serviceRequestRouter = router({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (ENV.smtpHost) {
        ...
        sendEmail({
          ...
          logEvent(
            "system",
            "service-request-create-notify-fail",
            { err },
            "warn"
          )
        );
      ...
      broadcastSSE("service_request_created", {
        id: insertId,
        orgId,
      ...
      void recordUserInteraction(ctx, {
        ...
        entityId: insertId,
        inputSnapshot: { serviceType: input.serviceType },
      });
  ...
  adminUpdate: protectedProcedure
    .input(
      z.object({ id: z.number().int().positive() }).merge(adminUpdateSchema)
    )
    .mutation(async ({ ctx, input }) => {
      if (!hasMinRole(ctx.user?.role, "admin"))
        throw new TRPCError({ code: "FORBIDDEN" });
      ...
      if (data.clientResponse !== undefined)
        updateValues.respondedAt = new Date();
