...
function sanitizePayload(value: unknown): string | null {
  ...
  try {
    return JSON.stringify(value, (key, val) => {
      if (/(password|secret|token|auth|cookie|api[_-]?key)/i.test(key))
        return "[redacted]";
...
function getHashedIpFromHeaders(
...
export async function recordAuditEvent(
  ...
  try {
    ...
    const BROADCAST_CATEGORIES: AuditCategory[] = [
      "auth",
      "billing",
      "system",
      "data_write",
...
export async function recordTrpcFailureEvent(input: {
  ...
  try {
    const localUserId =
      (input.ctx?.user as { localUserId?: number } | null | undefined)
        ?.localUserId ?? null;
    const userId =
      input.ctx?.user && input.ctx.user.id > 0 ? input.ctx.user.id : null;
    const outcome = [
      "BAD_REQUEST",
      "PARSE_ERROR",
      "UNAUTHORIZED",
    ...
    await db.insert(auditLogs).values({
      ...
      ipHash: input.ctx
        ? hashIp(getClientIp(input.ctx))
        : getHashedIpFromHeaders(input.headers ?? {}),
      userAgent:
