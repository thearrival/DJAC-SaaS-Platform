...
const inMemoryTechStackByVendor = new Map<
  number,
...
function toInsertValues(
  ...
  return {
    ...
    operatingCountries: normalizeText(
      serializeVendorMultiValue(input.operatingCountries)
    ),
    cloudProvider: normalizeText(
      serializeVendorMultiValue(input.cloudProviders)
    ),
    dataLocations: normalizeText(
      serializeVendorMultiValue(input.dataLocations)
    ),
    regulatoryJurisdictions: normalizeText(
      serializeVendorMultiValue(input.regulatoryJurisdictions)
    ),
    certifications: normalizeText(
      serializeVendorMultiValue(input.certifications)
    ),
...
export async function listTechStackComponentsByVendorIds(
  ...
  const db = await getDb();
  if (!db) {
    return vendorIds.flatMap(
      vendorId => inMemoryTechStackByVendor.get(vendorId) ?? []
    );
  }
  ...
  return db
    .select()
    .from(techStackComponents)
    .where(inArray(techStackComponents.vendorId, vendorIds));
}
