...
const regionSchema = z.enum(
  GLOBAL_REGIONS as [GlobalRegion, ...GlobalRegion[]]
);
...
export const complianceFrameworkRouter = router({
  ...
  globalRegistrySummary: publicProcedure.query(() => {
    return getGlobalRegistrySummary();
  }),

  globalFrameworks: publicProcedure.query(() => {
    return listGlobalFrameworkPacks();
  }),

  globalFrameworksByRegion: publicProcedure
    .input(regionSchema)
    .query(({ input }) => {
  ...
  globalFrameworkByCode: publicProcedure
    .input(z.string().trim().min(1).max(120))
    .query(({ input }) => {
      return getGlobalFrameworkPackByCode(input);
    }),
  ...
  globalAiAgents: publicProcedure.query(() => {
    return listGlobalAIAgents();
  }),

  globalKnowledgeGraph: publicProcedure.query(() => {
    return buildGlobalGraphSeed();
  }),

  globalRegistrySearch: publicProcedure
    .input(
      z.object({
    ...
    .query(({ input }) => {
      return searchGlobalRegistry(input.query, input.limit);
    }),
  ...
  comparison: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const data = await getComplianceComparison(
        input.framework1Id,
        input.framework2Id
      );
      void recordUserInteraction(ctx, {
