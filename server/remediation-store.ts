...
export type PatchTaskFields = Partial<
  Omit<CreateTaskInput, "dueDate" | "assignedToUserId">
> & { assignedToUserId?: number | null };
...
export async function listTasks(orgId: number): Promise<TaskRow[]> {
  ...
  if (!db || orgId < 0) {
    return MEM_TASKS.filter(t => t.organizationId === orgId).sort(
...
export async function createTask(
  ...
  const [insertedRemediation] = await db
...
export async function removeTask(orgId: number, id: number): Promise<boolean> {
  const db = await getDb();
  if (!db || orgId < 0) {
    const idx = MEM_TASKS.findIndex(
      t => t.id === id && t.organizationId === orgId
    );
