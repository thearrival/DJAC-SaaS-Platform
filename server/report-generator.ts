...
const LABELS: Record<ReportLocale, Record<string, string>> = {
  en: {
    ...
    engineValue:
      "DJAC v1.0 — Multi-Jurisdictional Compliance Automation Engine",
    critical: "CRITICAL",
    high: "HIGH",
    medium: "MEDIUM",
    low: "LOW",
    ...
    disclaimer:
      "This report was generated by the DJAC automated compliance engine from curated " +
      "reference content. It is provided for general information and internal compliance " +
      "planning only and does not constitute legal advice. Regulatory requirements change " +
      "frequently and may differ based on your specific circumstances, industry, and data " +
      "flows. Always verify every requirement, deadline, and obligation against current " +
      "official sources published by the relevant regulator (SDAIA, NCA, CAC, MIIT, EU, and " +
      "other authorities as applicable) and consult a qualified legal or compliance " +
      "professional before relying on this content for regulatory submissions.",
  },
  ar: {
    title: "تقرير تقييم الامتثال — DJAC",
    subtitle:
      "الامتثال التنظيمي متعدد الولايات القضائية | جاهز للجهات الحكومية",
    ...
    disclaimerTitle: "إشعار مهم وإخلاء مسؤولية",
    disclaimer:
      "تم إنشاء هذا التقرير بواسطة محرك الامتثال الآلي DJAC استناداً إلى محتوى مرجعي منسّق. " +
      "يُقدَّم لأغراض المعلومات العامة والتخطيط الداخلي للامتثال فقط ولا يُعدّ استشارة قانونية. " +
      "تتغير المتطلبات التنظيمية باستمرار وقد تختلف بحسب ظروفك وقطاعك وتدفقات بياناتك. " +
      "تحقّق دائماً من كل متطلب وموعد نهائي والتزام مقابل المصادر الرسمية الحالية الصادرة عن " +
      "الجهات التنظيمية المختصة (SDAIA وNCA وCAC وغيرها) واستشر مختصاً قانونياً أو امتثالي مؤهلاً " +
      "قبل الاعتماد على هذا المحتوى في التقديمات التنظيمية.",
  },
  zh: {
    title: "DJAC 合规评估报告",
    ...
    disclaimerTitle: "重要通知与免责声明",
    disclaimer:
      "本报告由 DJAC 自动化合规引擎基于整理后的参考内容生成，仅供一般性参考和内部合规规划使用，不构成法律意见。" +
      "监管要求变化频繁，并可能因您的具体情况、行业和数据流而有所不同。在用于监管申报之前，请务必对照" +
      "相关监管机构（如 SDAIA、NCA、CAC 等）发布的最新官方来源核实每一项要求、期限和义务，并咨询合格的法律或合规专业人士。",
  },
};
...
export function generateComplianceReport(opts: ReportOptions): {
  ...
  const REPORT_TYPE_TEMPLATES: Record<
    ReportType,
    { titleSuffix: Record<ReportLocale, string>; templateName: string }
  > = {
    full_compliance: {
      titleSuffix: {
        en: "Full Compliance Assessment",
        ar: "تقييم الامتثال الكامل",
        zh: "全面合规评估",
      },
      templateName: "Full Compliance Template",
    },
    gap_analysis: {
      titleSuffix: {
        en: "Gap Analysis Report",
        ar: "تقرير تحليل الفجوات",
        zh: "差距分析报告",
      },
      templateName: "Gap Analysis Template",
    },
    vendor_assessment: {
      titleSuffix: {
        en: "Vendor Assessment Report",
        ar: "تقرير تقييم الموردين",
        zh: "供应商评估报告",
      },
      templateName: "Vendor Assessment Template",
    },
    risk_assessment: {
      titleSuffix: {
      ...
      templateName: "Risk Assessment Template",
    },
    executive_summary: {
      titleSuffix: {
      ...
      templateName: "Executive Summary Template",
    },
    regulatory_deadline: {
      titleSuffix: {
        en: "Regulatory Deadline Tracker",
        ar: "متتبع المواعيد التنظيمية",
        zh: "监管截止日期追踪",
      },
      templateName: "Deadline Tracker Template",
    },
  };
  ...
  const includeSection = (section: string) => {
    ...
    if (reportType === "executive_summary")
      return ["header", "exec", "recommendations", "appendix"].includes(
        section
      );
    if (reportType === "regulatory_deadline")
      return ["header", "exec", "obligations", "appendix"].includes(section);
    if (reportType === "risk_assessment")
      return [
        "header",
        "exec",
        "security",
        "obligations_critical",
        "recommendations",
        "appendix",
      ].includes(section);
    if (reportType === "gap_analysis")
      return ["header", "exec", "gaps", "recommendations", "appendix"].includes(
        section
      );
    if (reportType === "vendor_assessment")
      return [
        "header",
        "exec",
        "frameworks",
  ...
  parts.push(`\n**Key Insights:**\n`);
  parts.push(
    `- Critical obligations are concentrated in cross-border data transfer and incident response.\n`
  );
  parts.push(
  ...
  if (includeSection("security")) {
    ...
    parts.push(`\n**Key Risk Insights:**\n`);
    parts.push(
      `- Penalty exposure is highest for unreported breaches and cross-border data transfer violations.\n`
    );
    parts.push(
      `- Saudi PDPL and China PIPL both impose multi-million penalties and criminal liability for severe violations.\n`
    );
    parts.push(
      `- NCA-ECC and MLPS 2.0 require ongoing technical and organizational controls to avoid service suspension.\n`
    );
    if (includeSaudi) {
      parts.push(
        `\n**${L("saudi")} — PDPL / NCA:**\n\n` +
  ...
  if (includeSection("obligations") || includeSection("obligations_critical")) {
    ...
    parts.push(`\n**Actionable Insights:**\n`);
    parts.push(
      `- Immediate focus: appoint DPO and complete cross-border data transfer assessments.\n`
    );
    parts.push(
      `- Ongoing: maintain evidence packs and incident response readiness.\n`
    );
    parts.push(
      `- Annual: schedule internal cybersecurity audits and regulatory self-assessments.\n`
    );
      ...
      parts.push(
        `### ${L("ongoingHeader")}\n\n` +
          ...
          ongoing
            .map(
              o =>
                `| ${o.requirement} | ${o.framework} | ${o.country} | ${o.authority} |`
            )
            .join("\n") +
          "\n\n"
      );
  ...
  parts.push(
    `- Maintain incident response and evidence packs for all frameworks.\n`
  );
  parts.push(
    `- Schedule annual internal audits and regulatory self-assessments.\n`
  );
    ...
    parts.push(
      `\n### ${L("appendixReferences")}\n\n` +
        relevantLaws
          .map(
            l =>
              `- **${l.title}** *(${l.jurisdiction})* — ${l.summary.slice(0, 140)}...`
          )
          .join("\n") +
        "\n"
    );
  ...
  // ── Disclaimer (regulatory content is curated, not legal advice) ──────────
  parts.push(hr());
  parts.push(`## ${L("disclaimerTitle")}\n\n${L("disclaimer")}\n`);
