...
const SIDEBAR_PLACEHOLDER =
  "“Got something very important to point out to your readers? Use a pull quote to make it stand out.”";
const execFileAsync = promisify(execFile);
const ENABLE_NATIVE_PDF_CONVERSION =
  parsedEnv.REPORT_NATIVE_PDF_CONVERSION && parsedEnv.NODE_ENV !== "test";
...
function paragraphXml(
  ...
  if (
    ...
    paragraphProps.push(
      `<w:spacing${typeof options.before === "number" ? ` w:before="${options.before}"` : ""}${typeof options.after === "number" ? ` w:after="${options.after}"` : ""}/>`
    );
  }
...
function tableXml(rows: string[][]) {
  ...
  const bodyRows = rows
    .map(
      (row, rowIndex) =>
        `<w:tr>${Array.from({ length: columnCount }, (_, colIndex) => tableCellXml(row[colIndex] ?? "", rowIndex === 0)).join("")}</w:tr>`
    )
    .join("");
...
function coverMetadataXml(
  report: GeneratedReportModel,
  options: ReportOptions
) {
  const generatedValue = new Date(report.generatedAt).toLocaleString("en-US");
  return [
    paragraphXml(`Generated: ${generatedValue}`, {
      color: "7A7A7A",
      size: 20,
      after: 40,
    }),
    paragraphXml(`Report ID: ${report.reportId}`, {
      color: "7A7A7A",
      size: 20,
      after: 40,
    }),
    paragraphXml(`Jurisdiction: ${options.jurisdiction}`, {
      color: "7A7A7A",
      size: 20,
      after: 0,
    }),
...
function buildDocxBodyXml(
  ...
  const sectPrMatch = originalDocumentXml.match(
    /<w:sectPr[\s\S]*?<\/w:sectPr>/
  );
  const sidebarParagraphMatch = originalDocumentXml.match(
    /<w:p>[\s\S]*?(?:descr="Sidebar"|Text Box 5)[\s\S]*?<\/w:p>/
  );
  const sidebarParagraphXml = sidebarParagraphMatch
    ? sidebarParagraphMatch[0].replace(
        SIDEBAR_PLACEHOLDER,
        escapeXml(`“${extractSidebarQuote(report.markdown)}”`)
      )
    : "";

  const titleLines = wrapCoverTitle(
    options.locale === "en" ? report.title.toUpperCase() : report.title,
    options.locale
    ...
    .map(block => {
      if (block.kind === "heading") {
        return paragraphXml(block.text, {
          styleId: block.level === 2 ? "Heading2" : "Heading3",
          after: 80,
        });
      }
      if (block.kind === "table") {
  ...
  const coverXml = [
    ...
    subtitle
      ? paragraphXml(
          options.locale === "en" ? subtitle.toUpperCase() : subtitle,
          { styleId: "Subtitle", after: 240 }
        )
      : "",
...
async function buildDocxBuffer(
  ...
  try {
    ...
    console.error(
      "[DJAC DOCX] Template content injection failed, generating minimal DOCX:",
      err
    );
    return buildMinimalDocxBuffer(report);
  }
...
async function buildPdfBuffer(
  ...
  try {
    ...
    if (watermarkPdfDoc.getPageCount() === 0)
  ...
  const _normalizeText = (value: string) =>
    fonts.supportsUnicode ? value : sanitizeFallbackText(value);
  ...
  function renderHeaderFooter(page: PDFPage): void {
    ...
    if (logoImage) {
      try {
        ...
        let scale = 1;
        if (dims.width > maxLogoW)
          scale = Math.min(scale, maxLogoW / dims.width);
        if (dims.height > maxLogoH)
          scale = Math.min(scale, maxLogoH / dims.height);
  ...
  function draw_section_paragraph(
    page: PDFPage,
    y: number,
    text: string,
    bold = false
  ...
  function draw_section_table(
    ...
    function drawHeaderRow(pg: PDFPage, startY: number): number {
      ...
      for (let c = 0; c < colCount; c++) {
        ...
        pg.drawLine({
          start: { x: xOff, y: startY - headerHeight + 2 },
          end: { x: xOff, y: startY + 2 },
          thickness: 0.7,
      ...
      pg.drawLine({
        start: { x: marginX, y: startY - headerHeight + 2 },
        end: { x: marginX + tableWidth, y: startY - headerHeight + 2 },
        thickness: 0.7,
        color: rgb(0.62, 0.68, 0.76),
      });
      pg.drawLine({
        start: { x: marginX + tableWidth, y: startY - headerHeight + 2 },
        end: { x: marginX + tableWidth, y: startY + 2 },
        thickness: 0.7,
        color: rgb(0.62, 0.68, 0.76),
      });
    ...
    for (let r = 0; r < tableRows.length; r++) {
      ...
      for (let c = 0; c < colCount; c++) {
        ...
        page.drawLine({
          start: { x: xOffset, y: rowY - rowHeight + 2 },
          end: { x: xOffset, y: rowY + 2 },
          thickness: 0.5,
      ...
      page.drawLine({
        start: { x: marginX, y: rowY - rowHeight + 2 },
        end: { x: marginX + tableWidth, y: rowY - rowHeight + 2 },
        thickness: 0.5,
        color: rgb(0.8, 0.8, 0.82),
      });
      page.drawLine({
        start: { x: marginX + tableWidth, y: rowY - rowHeight + 2 },
        end: { x: marginX + tableWidth, y: rowY + 2 },
        thickness: 0.5,
        color: rgb(0.8, 0.8, 0.82),
      });
      y = rowY - rowHeight - 4;
  ...
  pages.forEach((entry, index) => {
    ...
    entry.page.drawText(pageNumberText, {
      x:
        pageWidth -
        marginX -
        fontRegular.widthOfTextAtSize(pageNumberText, 6.5),
      y: FOOTER_H - 9,
      size: 6.5,
      font: fontRegular,
...
export async function generateComplianceReportDocx(
  options: ReportOptions
): Promise<GeneratedReportDocx> {
  const report = generateComplianceReport(options);
