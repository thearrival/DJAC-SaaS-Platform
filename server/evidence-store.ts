...
const MEM_EVIDENCE: EvidenceRow[] = [
  ...
  },
  {
    ...
    updatedAt: new Date("2025-02-20T14:00:00Z"),
  },
  {
    id: 3,
    organizationId: 1,
  ...
  },
  {
    ...
    description:
      "Reference link to the privacy information management certification standard.",
    addedByUserId: null,
...
export async function listEvidence(
  ...
  if (sourceType)
    conditions.push(eq(complianceEvidence.sourceType, sourceType));
  if (sourceId) conditions.push(eq(complianceEvidence.sourceId, sourceId));
  return db
    .select()
    .from(complianceEvidence)
    .where(and(...conditions))
    .orderBy(desc(complianceEvidence.createdAt)) as unknown as Promise<
    EvidenceRow[]
  >;
}
...
export async function getEvidenceForRemoval(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const item = MEM_EVIDENCE.find(
      e => e.id === evidenceId && e.organizationId === orgId
    );
    return item ? { id: item.id, organizationId: item.organizationId } : null;
  }
  const [row] = await db
    .select({
      id: complianceEvidence.id,
...
export async function removeEvidence(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const idx = MEM_EVIDENCE.findIndex(
      e => e.id === evidenceId && e.organizationId === orgId
    );
