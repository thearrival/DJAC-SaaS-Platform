...
const MEM_ITEMS: ThreatIntelRow[] = [
  {
    ...
    summary:
      "State-sponsored threat actors are actively exploiting a stack-based buffer overflow in Ivanti Connect Secure VPN appliances. The vulnerability allows unauthenticated remote code execution and has been used to deploy custom malware implants. Immediate patching and factory reset of affected appliances is recommended.",
    ...
    indicators:
      "23.95.82.4,185.220.101.47,hxxps://update-ivanti[.]com,/api/v1/license/key-status/..",
  ...
  },
  {
    ...
    organizationId: null,
    title:
      "Black Basta Ransomware Targeting Gulf Region Financial Institutions",
    summary:
      "Black Basta affiliates have been observed conducting targeted ransomware campaigns against banking and financial services organizations in the Gulf Cooperation Council.",
  ...
  },
  {
    ...
    summary:
      "A command injection vulnerability in the GlobalProtect feature of Palo Alto Networks PAN-OS software allows an unauthenticated attacker to execute arbitrary code on the firewall with root privileges.",
    threatActor: "UTA0218",
    ...
    affectedSectors: "all",
    indicators:
      "PAN-OS < 10.2.9-h1,/var/log/pan/sslvpn_ngx_error.log anomalies",
    ...
    items = (await db
      ...
      .where(
        and(
          ...
          or(
            isNull(threatIntelItems.organizationId),
            eq(threatIntelItems.organizationId, orgId)
          )
        )
      )
...
export async function getThreatItem(
  orgId: number,
  id: number
