...
async function loadConsultationRequestById(id: number) {
  ...
  if (!db) {
    return (
      memoryConsultationRequests.find(request => request.id === id) ?? null
    );
  }
...
export async function updateAccessRequestStatus(
  input: AccessRequestStatusUpdateInput
) {
  const db = await getDb();
  const now = new Date();

  if (!db) {
    const target = memoryAccessRequests.find(
      item => item.id === input.accessRequestId
    );
    if (!target) {
      return null;
    }
...
export async function markAdminNotificationRead(notificationIdValue: number) {
  ...
  const now = new Date();
  if (!db) {
    const target = memoryAdminNotifications.find(
      item => item.id === notificationIdValue
    );
    if (!target) {
      return null;
    }
...
export async function respondToConsultationRequest(
  input: ConsultationResponseInput
) {
  const db = await getDb();
  const now = new Date();
  if (!db) {
    const target = memoryConsultationRequests.find(
      item => item.id === input.consultationId
    );
    if (!target) {
      return null;
    }
...
export async function listAssessmentSummaries(limit = 50) {
  ...
  const gapRows =
    assessmentIds.length === 0
      ...
      : await db
          .select()
          .from(assessmentGaps)
          .where(inArray(assessmentGaps.assessmentId, assessmentIds));
  ...
  return rows.map(row => {
    ...
    const criticalGapCount = relatedGaps.filter(
...
export async function listAdminVendorSummaries(limit = 100) {
  ...
  const techRows = await listTechStackComponentsByVendorIds(
  ...
  return vendors.map(vendor => {
    ...
    const components = techRows.filter(
      component => component.vendorId === vendor.id
    );
...
export async function getInteractionPrivacyStats(
  retentionDays = 90
): Promise<InteractionPrivacyStats> {
  const db = await getDb();
...
export async function enforceInteractionRetention(
  retentionDays = 90,
  dryRun = true,
  actorUserId?: number | null
): Promise<InteractionRetentionRunResult> {
...
export async function getAdminInteractionHeatmap(
  windowDays = 14,
  limit = 2000
): Promise<AdminInteractionHeatmap> {
  ...
  return rows.map(row => ({
    ...
    assignedAdminName:
      row.assignedAdminUserId != null
        ? (userMap.get(row.assignedAdminUserId)?.name ?? null)
        : null,
...
function buildActivitySeries(
  activityRows: Awaited<ReturnType<typeof listActivityFeed>>
) {
...
function buildRegionCoverage(
  ...
  for (const vendor of vendorRows) {
    for (const region of trackedRegions) {
      const touchesRegion =
        vendor.operatingCountries.includes(region) ||
        vendor.dataLocations.includes(region);
      if (!touchesRegion) {
        continue;
      }
...
export async function getAdminOverview(): Promise<AdminOverview> {
  ...
  return {
    totals: {
      ...
      adminUsers: userRows.filter(user => hasMinRole(user.role, "admin"))
        .length,
      openAccessRequests: accessRows.filter(
        row => row.status !== "approved" && row.status !== "archived"
      ).length,
      openConsultations: consultationRows.filter(row => row.status !== "closed")
        .length,
      vendors: vendorRows.length,
      assessments: assessmentRows.length,
      criticalGaps: assessmentRows.reduce(
        (total, row) => total + row.criticalGapCount,
        0
      ),
      unreadNotifications: notificationRows.filter(
        row => !row.isRead && !row.readAt
      ).length,
    },
