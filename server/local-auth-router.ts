...
export const localAuthRouter = router({
  ...
  register: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
          ...
          ...(input.userType === "professional"
            ? {
                ...
                complianceResponsibility:
                  input.complianceResponsibility ?? null,
              }
            : {}),
        });
      } else {
      ...
      void recordAuditEvent(ctx, {
        ...
        payload: { userType: input.userType, email: normalizedEmail },
      });
  ...
  login: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const dummyHash =
        "$2a$12$notarealhashjustpadding000000000000000000000000000000000";
      ...
      void recordAuditEvent(ctx, {
        ...
        entityId: user.id,
        localUserId: user.id,
        payload: { method: "local", userType: user.userType },
      });
  ...
  updateProfile: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      broadcastSSE("user_profile_updated", {
        userId,
  ...
  changePassword: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      broadcastSSE("user_password_changed", {
        userId,
        ts: new Date().toISOString(),
      });
      return { success: true as const };
  ...
  sendOtp: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      void recordAuditEvent(ctx, {
        ...
        entityType: "otpCodes",
        payload: {
          identifier: input.identifier.replace(/(.{3}).*(@.*)/, "$1***$2"),
          purpose: input.purpose,
        },
      });
      return {
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.purpose === "login") {
        ...
        void recordAuditEvent(ctx, {
          ...
          payload: { method: "otp", userType: existing.userType },
        });
        broadcastSSE("user_login", {
          userId: existing.id,
      ...
      if (existing) {
        ...
        void recordAuditEvent(ctx, {
          ...
          entityId: existing.id,
          localUserId: existing.id,
          payload: { method: "otp_register", userType: existing.userType },
        });
        broadcastSSE("user_login", {
          userId: existing.id,
          email: existing.email,
      ...
      void recordAuditEvent(ctx, {
        ...
        payload: { method: "otp", userType: newUser.userType },
      });
      broadcastSSE("user_registered", {
        userId: newUser.id,
        email: newUser.email,
