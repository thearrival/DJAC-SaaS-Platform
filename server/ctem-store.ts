...
export function inMemoryDemoData(orgId: number) {
  ...
  const demoVulns = [
    {
      ...
      exploitAvailable: 1,
      isPatched: 0,
      discoveredAt: new Date("2025-11-01"),
    },
    {
      id: 2,
      assetId: 1,
    ...
    },
    {
      ...
      assetId: 3,
      cveId: "CVE-2025-0282",
      title: "TLS 1.0/1.1 Cipher Suite Weakness",
      severity: "medium",
      cvssScore: 55,
      exploitAvailable: 0,
      isPatched: 1,
    ...
    },
    {
      ...
      title: "Privilege Escalation in Storage SDK",
      severity: "critical",
      cvssScore: 95,
      exploitAvailable: 1,
      isPatched: 0,
    ...
    },
    {
      ...
      assetId: 5,
      cveId: "CVE-2025-1234",
      title: "Credential Exposure via Debug Endpoint",
      severity: "high",
      cvssScore: 80,
      exploitAvailable: 1,
      isPatched: 0,
  ...
  const demoScores = demoAssets.map(a => {
    ...
    const expScore = Math.min(
      100,
      (a.isInternetFacing ? 40 : 10) +
        Math.round((a.criticalityScore / 10) * 20) +
        Math.round(highestCvss * 0.4)
    );
    const explScore = Math.min(
      100,
      Math.round((exploitable.length / Math.max(vulns.length, 1)) * 50) +
        (exploitable.length > 0 ? 30 : 0)
    );
    const bizScore = Math.min(
      100,
      (a.handlesPersonalData ? 30 : 0) +
        (a.handlesCriticalData ? 30 : 0) +
        Math.round((a.criticalityScore / 10) * 40)
    );
    const final = Math.round(
      expScore * 0.35 + explScore * 0.4 + bizScore * 0.25
    );
    const tier =
      final >= 80
        ? "critical"
    ...
    return {
      ...
      businessImpactScore: bizScore,
      finalPriorityScore: final,
      priorityTier: tier,
      previousFinalScore: final - Math.ceil(Math.random() * 8),
    };
  });
...
export async function listCtemAssets(
  ...
  return db
    ...
    .from(ctemAssets)
    .where(and(...where))
    .orderBy(desc(ctemAssets.createdAt));
}
...
export async function getCtemAssetOwner(
  assetId: number
  ...
  const [row] = await db
    ...
    .from(ctemAssets)
    .where(eq(ctemAssets.id, assetId));
  return row ?? null;
}
...
export async function updateCtemAsset(
  ...
  await db
    ...
    .set({
      ...
      ...(patch.handlesPersonalData !== undefined && {
        handlesPersonalData: patch.handlesPersonalData ? 1 : 0,
      }),
      ...(patch.handlesCriticalData !== undefined && {
        handlesCriticalData: patch.handlesCriticalData ? 1 : 0,
      }),
...
export async function listCtemVulnerabilities(
  ...
  const orgAssets = await db
    ...
    .from(ctemAssets)
    .where(eq(ctemAssets.organizationId, orgId));
  ...
  const mappings =
    vulnIds.length > 0
      ? await db
          ...
          .where(inArray(complianceExposureMappings.vulnerabilityId, vulnIds))
      : [];
  return rows.map(row => ({
    ...row,
...
export async function getCtemVulnAssetOwner(
  vulnId: number
  ...
  const [row] = await db
    ...
    .from(ctemVulnerabilities)
    .where(eq(ctemVulnerabilities.id, vulnId));
  return row ?? null;
}

export async function createCtemVulnerability(input: {
  assetId: number;
  cveId?: string | null;
  title: string;
  ...
  const [row] = await db
    .select()
    .from(ctemVulnerabilities)
    .where(eq(ctemVulnerabilities.id, id));
  return row;
}
...
export async function patchCtemVulnerability(
  ...
  if (patch.isPatched !== undefined) {
    update.isPatched = patch.isPatched ? 1 : 0;
    if (patch.isPatched) update.patchedAt = new Date();
  }
  if (patch.isConfirmed !== undefined)
    update.isConfirmed = patch.isConfirmed ? 1 : 0;
  if (patch.exploitAvailable !== undefined)
    update.exploitAvailable = patch.exploitAvailable ? 1 : 0;
...
export async function listCtemRiskScores(
  ...
  const scoreFilters: ReturnType<typeof eq>[] = [
...
export async function getCtemRiskSummary(orgId: number) {
  ...
  if (!db) {
    ...
    const byTier = (t: string) =>
      scores.filter((s: any) => s.priorityTier === t).length;
    const lastRun = {
      runStatus: "completed",
      startedAt: new Date(Date.now() - 3600000),
      avgPriorityScore:
        scores.reduce((s: number, r: any) => s + r.finalPriorityScore, 0) /
    ...
    return {
      ...
      exploitableVulns: vulns.filter(
        (v: any) => v.exploitAvailable && !v.isPatched
      ).length,
  ...
  for (const m of mappings) {
    ...
    const newWorst =
      sevOrder.indexOf(m.severityImpact as (typeof sevOrder)[number]) <
      sevOrder.indexOf(cur.maxSev as (typeof sevOrder)[number])
        ? m.severityImpact
        : cur.maxSev;
  ...
  const avgScore =
    scores.length > 0
      ? Math.round(
          scores.reduce((s, r) => s + r.finalPriorityScore, 0) / scores.length
        )
      : 0;
  ...
  return {
    ...
    exploitableVulns: vulnCounts.filter(v => v.exploitAvailable && !v.isPatched)
      .length,
    ...
    frameworkExposure: Array.from(fwMap.entries()).map(
      ([frameworkCode, v]) => ({
        frameworkCode,
...
export async function listCtemRuns(
  orgId: number,
  limit = 20
): Promise<ContinuousComplianceRun[]> {
  const db = await getDb();
  if (!db) {
    return [
      {
...
export async function getCtemFrameworkExposure(orgId: number) {
  ...
  const vulnIds =
    assetIds.length > 0
      ? (
          await db
            .select({ id: ctemVulnerabilities.id })
            .from(ctemVulnerabilities)
            .where(inArray(ctemVulnerabilities.assetId, assetIds))
        ).map(v => v.id)
      : [];
  ...
  for (const m of mappings) {
    if (!m.frameworkCode) continue;
    if (!fwMap.has(m.frameworkCode))
      fwMap.set(m.frameworkCode, {
        total: 0,
        critical: 0,
        high: 0,
        medium: 0,
  ...
  return Array.from(fwMap.entries()).map(([frameworkCode, counts]) => ({
    frameworkCode,
    ...counts,
  }));
}
...
export async function getCtemAssetOrgId(
  assetId: number
): Promise<number | null> {
  const db = await getDb();
  if (!db) return null;
