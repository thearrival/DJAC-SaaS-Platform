...
export const roleRouter = router({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        ...
        (input.newRole === "super_admin" ||
          input.newRole === "yalla_hack_employee")
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
      ...
      if (
        ...
        !hasMinRole(input.newRole, actorRole)
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
      ...
      const target = await assignUserRole(
        input.targetUserId,
        input.newRole as PlatformRole
      );
      if (!target)
        throw new TRPCError({
  ...
  assignLocalUserRole: platformAdminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        !hasMinRole(actorRole, "super_admin") &&
        (input.newUserType === "super_admin" ||
          input.newUserType === "yalla_hack_employee")
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: NOT_PLATFORM_ADMIN_ERR_MSG,
      ...
      const target = await assignLocalUserRole(
        input.targetLocalUserId,
        input.newUserType
      );
      if (!target)
        throw new TRPCError({
      ...
      await recordAuditEvent(ctx, {
        ...
        outcome: "success",
        payload: {
          previousUserType: target.userType,
          newUserType: input.newUserType,
        },
      });
