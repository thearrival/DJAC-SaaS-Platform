...
const MEM_DSRS: DsrRow[] = [
  {
    ...
    requesterEmail: "li.wei@example.cn",
    description:
      "Requesting a copy of all personal data held about me under PIPL Article 45.",
    status: "in_review",
    priority: "normal",
    dueDate: computeDueDate("China", new Date(_now.getTime() - 5 * 86400000)),
    completedAt: null,
    assignedToUserId: null,
    notes: "Verification of identity completed.",
    createdAt: new Date(_now.getTime() - 5 * 86400000),
    updatedAt: new Date(_now.getTime() - 3 * 86400000),
  },
  {
    id: 2,
    organizationId: 1,
    ...
    requesterName: "Khalid Al-Mansour",
    requesterEmail: "k.almansour@example.sa",
    description: "Requesting deletion of all personal data per PDPL Article 6.",
    status: "received",
    priority: "high",
    ...
    assignedToUserId: null,
    notes: null,
    createdAt: new Date(_now.getTime() - 2 * 86400000),
  ...
  },
  {
    ...
    jurisdiction: "China",
    requesterName: "Zhang Min",
    requesterEmail: "zhang.min@example.cn",
    description:
      "Requesting data portability to transfer records to another provider.",
    status: "completed",
    priority: "normal",
    dueDate: computeDueDate("China", new Date(_now.getTime() - 25 * 86400000)),
    completedAt: new Date(_now.getTime() - 3 * 86400000),
    assignedToUserId: null,
    notes: "Data exported and sent via secure channel.",
    createdAt: new Date(_now.getTime() - 25 * 86400000),
    updatedAt: new Date(_now.getTime() - 3 * 86400000),
  },
  {
    id: 4,
    ...
    jurisdiction: "Saudi Arabia",
    requesterName: "Fatimah Al-Hassan",
    requesterEmail: "f.alhassan@example.sa",
    description:
      "Incorrect date of birth in our records — requesting correction.",
    status: "pending_info",
    priority: "urgent",
    dueDate: new Date(_now.getTime() + 3 * 86400000),
    completedAt: null,
    assignedToUserId: null,
...
export function buildDsrSummary(
  rows: { status: string; dueDate: Date | string }[]
) {
  ...
  for (const row of rows) {
    ...
    const due =
      typeof row.dueDate === "string" ? new Date(row.dueDate) : row.dueDate;
    if (
      due < now &&
...
export async function listDsrs(
  ...
  if (!db || orgId < 0) {
    ...
    if (filters?.requestType)
      items = items.filter(d => d.requestType === filters.requestType);
    if (filters?.jurisdiction)
      items = items.filter(d => d.jurisdiction === filters.jurisdiction);
  ...
  if (filters?.requestType)
    conditions.push(eq(dsrRequests.requestType, filters.requestType));
  if (filters?.jurisdiction)
    conditions.push(eq(dsrRequests.jurisdiction, filters.jurisdiction));
  return db
    .select()
    .from(dsrRequests)
...
export async function patchDsr(
  ...
  const db = await getDb();
  if (!db || orgId < 0) {
    const idx = MEM_DSRS.findIndex(
      d => d.id === id && d.organizationId === orgId
    );
...
export async function getDsrSummary(orgId: number) {
  ...
  if (!db || orgId < 0) {
    return buildDsrSummary(
      MEM_DSRS.filter(d => d.organizationId === orgId).map(d => ({
        status: d.status,
        dueDate: d.dueDate,
      }))
    );
