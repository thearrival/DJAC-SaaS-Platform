...
function metricLabel(
  ...
  if (metric === "assessments")
    return t("region.metricAssessments", "Assessments");
...
export function ComplianceRegionMap({
  ...
  const focused =
    normalizedCoverage.find(item => item.region === focusedRegion) ?? null;
    ...
    <Card className="border-border/70 bg-gradient-to-br from-slate-50 to-white dark:from-slate-900 dark:to-slate-950">
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap gap-2">
          {(["vendors", "assessments", "criticalGaps"] as MetricKey[]).map(
            key => (
              <Button
                key={key}
                size="sm"
        ...
        <div className="rounded-xl border border-border bg-white/90 p-3 shadow-inner dark:bg-slate-950/60">
          <svg viewBox="0 0 900 420" className="h-[340px] w-full">
            ...
            {corridorFlows.map((flow, index) => {
              ...
              const midpointY =
                Math.min(sourcePosition.y, targetPosition.y) - 36;
        ...
        {focused ? (
          <div className="rounded-lg border border-border bg-muted/50 p-3">
            ...
            <div className="mt-2 grid grid-cols-1 gap-2 text-xs text-muted-foreground sm:grid-cols-3">
              <p>
                {t("region.labelVendors", "Vendors:")}{" "}
                <span className="font-medium text-foreground">
                  {focused.vendors}
                </span>
              </p>
              <p>
                {t("region.labelAssessments", "Assessments:")}{" "}
                <span className="font-medium text-foreground">
                  {focused.assessments}
                </span>
              </p>
              <p>
                {t("region.labelCriticalGaps", "Critical Gaps:")}{" "}
                <span className="font-medium text-foreground">
                  {focused.criticalGaps}
                </span>
              </p>
            </div>
          </div>
          ...
          <p className="text-xs text-muted-foreground">
            {t(
              "region.hoverHint",
              "Hover a region node to inspect compliance load."
            )}
          </p>
        )}
      </CardContent>
