...
export function FrameworkRelationshipOrb({
  ...
  const copy = {
    en: {
      title: "Framework",
      accent: "Nexus",
      subtitle: "Cross-border compliance relationships",
    },
    ar: {
      title: "إطار",
      accent: "الترابط",
      subtitle: "علاقات الامتثال العابر للحدود",
    },
      ...
      {!compact && (
        <>
          <h3 className="font-display font-semibold text-foreground text-sm mb-1">
            {copy.title}{" "}
            <span className="text-gradient-cyan">{copy.accent}</span>
          </h3>
        ...
        >
          <defs>
            ...
            <radialGradient id="centerGrad" cx="50%" cy="50%" r="50%">
              <stop
                offset="0%"
                stopColor="hsl(185,100%,49%)"
                stopOpacity="0.25"
              />
              <stop
                offset="50%"
                stopColor="hsl(260,74%,56%)"
                stopOpacity="0.1"
              />
              <stop
                offset="100%"
          ...
          <motion.circle
            ...
            animate={
              reduceMotion
                ? undefined
                : { r: [50, 58, 50], opacity: [0.5, 0.8, 0.5] }
            }
            transition={
              reduceMotion
                ? undefined
                  ...
                  : {
                      cx: fwData.map(
                        fw =>
                          CX +
                          RADIUS *
                            Math.cos(((fw.angle + p * 90) * Math.PI) / 180)
                      ),
                      cy: fwData.map(
                        fw =>
                          CY +
                          RADIUS *
                            Math.sin(((fw.angle + p * 90) * Math.PI) / 180)
                      ),
                    }
              }
              transition={
            ...
            return (
              ...
              >
                <motion.circle
                  ...
                  animate={
                    !reduceMotion && isHovered ? { r: [24, 28, 24] } : {}
                  }
                  transition={
          ...
          >
            <motion.circle
              ...
              animate={
                reduceMotion
                  ? undefined
                  : { r: [30, 36, 30], opacity: [0.2, 0.4, 0.2] }
              }
              transition={
                reduceMotion ? undefined : { duration: 3, repeat: Infinity }
              }
            />
            <circle
              cx={CX}
          ...
          {[0, 1, 2].map(wave => (
            <motion.circle
              ...
              transition={
                reduceMotion
                  ? undefined
                  : { duration: 4, repeat: Infinity, delay: wave * 1.3 }
              }
              style={{ transformOrigin: `${CX}px ${CY}px` }}
            />
          ))}
