...
function statusLabel(status: PipeStatus): string {
  switch (status) {
    case "compliant":
      return "Compliant";
    case "approval_required":
      return "CAC Approval Required";
    case "blocked":
      return "Data Transfer Blocked";
    case "simulated_violation":
      return "Violation Simulated";
  }
}
...
function calcScore(key: NodeKey, pipes: ActivePipe[]): number {
  ...
  const scoreMap: Record<PipeStatus, number> = {
    compliant: 100,
    approval_required: 62,
    blocked: 18,
    simulated_violation: 14,
  };
  return Math.round(
    connected.reduce((s, p) => s + scoreMap[p.activeStatus], 0) /
      connected.length
  );
}
...
function CompassRose({
  ...
  return (
    <g opacity={0.45} aria-label="Compass rose">
      <polygon
        points={`${cx},${cy - s} ${cx - s * 0.26},${cy - s * 0.26} ${cx + s * 0.26},${cy - s * 0.26}`}
        fill={color}
      />
      <polygon
        points={`${cx},${cy + s} ${cx - s * 0.26},${cy + s * 0.26} ${cx + s * 0.26},${cy + s * 0.26}`}
      ...
      />
      <polygon
        points={`${cx + s},${cy} ${cx + s * 0.26},${cy - s * 0.26} ${cx + s * 0.26},${cy + s * 0.26}`}
        fill={color}
        opacity={0.35}
      />
      <polygon
...
function RiskKPIBar({
  ...
  const riskWeights: Record<PipeStatus, number> = {
    ...
    blocked: 85,
    simulated_violation: 100,
  };
  const totalRisk = activePipes.reduce(
    (acc, p) => acc + riskWeights[p.activeStatus],
    0
  );
  const riskScore = Math.max(
    0,
    Math.round(100 - totalRisk / Math.max(1, activePipes.length))
  );
  const riskColor =
    riskScore >= 80
      ? isDark
...
export const SinoGulfHeatmap = memo(function SinoGulfHeatmap() {
  ...
  const filteredPipes = useMemo(
    () =>
      filterFw === "all"
        ? activePipes
        : activePipes.filter(p => p.frameworks.includes(filterFw)),
  ...
  const counts = useMemo(() => {
    const c = {
      ...
      approval_required: 0,
      blocked: 0,
      simulated_violation: 0,
    };
    for (const p of activePipes) c[p.activeStatus]++;
    return c;
  }, [activePipes]);

  const handlePipeMouse = useCallback(
    (e: React.MouseEvent, pipe: ActivePipe | null) => {
      if (pipe) {
        setHoveredPipe(pipe.id);
        setTooltip({ clientX: e.clientX, clientY: e.clientY, pipe });
      } else {
        setHoveredPipe(null);
        setTooltip(null);
      }
  ...
  const handlePipeClick = useCallback(
    (e: React.MouseEvent, pipe: ActivePipe) => {
      e.stopPropagation();
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            >
              {t(
                "heatmap.sinoGulf.title",
                "Sino-Gulf Data Residency Corridors"
              )}
            </h2>
          </div>
        </div>
        ...
        >
          ...
          {(
            [
              ...
              ["blocked", "Blocked"],
              ...(whatIfMode
                ? [["simulated_violation" as PipeStatus, "Simulated"]]
                : []),
            ] as [PipeStatus, string][]
          ).map(([status, lbl]) => (
            <span
              key={status}
              style={{
            ...
            >
              <span
                style={{
                  ...
                  height: 5,
                  borderRadius: "50%",
                  background: pipeColor(status, isDark),
      ...
      {whatIfMode && (
        ...
        >
          ...
          >
            <strong>Simulation active</strong> — 1M+ user-record transfer:
            Riyadh to Beijing. Impacted regulations:{" "}
            <strong>PIPL Art. 38-40</strong>, <strong>PDPL Art. 29</strong>,{" "}
            <strong>DSL Art. 31</strong>. Orange and red corridors require
      ...
      <div ref={containerRef} style={{ position: "relative", flex: 1 }}>
        ...
        >
          ...
          {(() => {
            const p1 = project(48, 27);
            const p2 = project(57, 24);
          ...
          {(() => {
            const pa = project(32, 26);
            const pb = project(44, 12);
            return (
              <ellipse
                cx={(pa.x + pb.x) / 2}
                cy={(pa.y + pb.y) / 2}
                rx={Math.abs(pb.x - pa.x) / 2}
                ry={Math.abs(pb.y - pa.y) / 2}
                fill={C.ocean}
                opacity={0.82}
              />
            );
          ...
          {(() => {
            const pa = project(55, 24);
            const pb = project(74, 5);
            return (
              <ellipse
                cx={(pa.x + pb.x) / 2}
                cy={(pa.y + pb.y) / 2 + 10}
          ...
          {(() => {
            const pa = project(80, 22);
            const pb = project(100, 5);
            return (
              <ellipse
                cx={(pa.x + pb.x) / 2}
                cy={(pa.y + pb.y) / 2}
          ...
          {(() => {
            const pa = project(108, 22);
            const pb = project(121, 0);
            return (
              <ellipse
                cx={(pa.x + pb.x) / 2}
                cy={(pa.y + pb.y) / 2}
          ...
          {(() => {
            const pa = project(97, 15);
            const pb = project(106, 3);
            return (
              <ellipse
                cx={(pa.x + pb.x) / 2}
                cy={(pa.y + pb.y) / 2}
          ...
          {filteredPipes.map(pipe => {
            ...
            return (
              <g key={pipe.id}>
                ...
                {([0, 0.35, 0.7] as const).map((offset, i) => (
                  <circle
                    key={i}
                    r={pipe.volume === "high" ? 2.8 : 2.2}
                    fill={color}
                    opacity={0.9}
            ...
            ([key, node]) => {
              ...
              const hasAlert =
                ...
                activePipes.some(
                  p =>
                    (p.from === key || p.to === key) &&
                    (p.activeStatus === "simulated_violation" ||
                      p.activeStatus === "blocked")
                );
              const accent = hasAlert
                ? pipeColor("simulated_violation", isDark)
                : C.cyan;
              ...
              return (
                <g key={key}>
                  ...
                  <circle
                    ...
                    r={15}
                    fill="none"
                    stroke={isDark ? `${sColor}22` : `${sColor}1a`}
                    strokeWidth={4.5}
                  />
                  {/* Score arc */}
                  <path
                  ...
                  >
                    {node.label}
                    <tspan fill={accent} fontSize={7} fontWeight={800} dx={3}>
                      {node.code}
                    </tspan>
                  </text>
                  {/* Jurisdiction */}
        ...
        {tooltip &&
          ...
          (() => {
            ...
            if (left + TW > rect.width - 8)
              left = tooltip.clientX - rect.left - TW - 8;
            if (top + TH > rect.height - 8)
              top = tooltip.clientY - rect.top - TH - 8;
            return (
              <div
                style={{
                  position: "absolute",
              ...
              >
                ...
                >
                  <div style={{ color, flexShrink: 0 }}>
                    {statusIcon(tooltip.pipe.activeStatus)}
                  </div>
                  <span
                ...
                >
                  ...
                  <span style={{ color, fontSize: 10, fontWeight: 700 }}>
                    {statusLabel(tooltip.pipe.activeStatus)}
                  </span>
                </div>
                <div
                  style={{
                ...
                {/* Data categories */}
                {tooltip.pipe.dataCategories &&
                  tooltip.pipe.dataCategories.length > 0 && (
                    <div
                      style={{
                        display: "flex",
                        gap: 3,
                ...
                {tooltip.pipe.articles && tooltip.pipe.articles.length > 0 && (
                  <div
                    style={{ borderTop: `1px solid ${color}20`, paddingTop: 6 }}
                  >
                    <span
                      style={{
                        fontSize: 7.5,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  ...
                  <span style={{ color, fontSize: 11, fontWeight: 600 }}>
                    — {statusLabel(pipe.activeStatus)}
                  </span>
                  <span style={{ color: C.muted, fontSize: 10 }}>
                    {fromNode.code} {fromNode.label} &rarr; {toNode.code}{" "}
                    {toNode.label}
                  </span>
                </div>
                <div
                  style={{
                    ...
                    gap: 5,
                    flexWrap: "wrap",
                    marginBottom: pipe.articles ? 7 : 0,
                  }}
                >
      ...
      >
        <Info
          style={{ color: C.muted, width: 11, height: 11, flexShrink: 0 }}
        />
        <p style={{ color: C.muted, fontSize: 9, margin: 0 }}>
          {t(
            "heatmap.sinoGulf.hint",
            "Hover a corridor for details · Click to pin · Filter by regulatory framework · What-If simulates a 1M+ record mass-transfer scenario"
          )}
        </p>
      </div>
    </div>
