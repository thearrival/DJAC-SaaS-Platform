...
function buildRiskTrend(hasPersonalData: boolean, revenue: number) {
  ...
  return MONTHS.map((month, i) => {
    ...
    const seasonalFactor = [
      1, 1.05, 1.1, 1.3, 1.25, 1.2, 1.15, 1.1, 1.4, 1.35, 1.3, 1.5,
    ][i];
...
export function RegulatoryPulseMatrix() {
  ...
  const gapCount = useMemo(() => {
    ...
    return (m as Array<{ maxSeverity?: string }>).filter(
      row => row.maxSeverity === "critical" || row.maxSeverity === "high"
    ).length;
  }, [matrixQuery.data]);
      ...
      >
              ...
              {(["all", "cn", "sa"] as const).map(j => (
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    background:
                      filterJurisdiction === j ? C.cyan : "transparent",
                    color:
          ...
          >
            {filteredFeed.map(action => {
              ...
              return (
                ...
                >
                  <div
                    style={{ color: typeColor, marginTop: 1, flexShrink: 0 }}
                  >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {action.article && (
                        <span
                          style={{ color: "var(--djac-muted)", fontSize: 9.5 }}
                        >
                          {action.article}
                        </span>
                      )}
        ...
        >
          ...
          >
            ...
            >
              <TrendingUp
                className="h-3 w-3 inline mr-1.5"
                style={{ verticalAlign: "middle", color: C.orange }}
              />
          ...
          <ResponsiveContainer width="100%" height={160}>
            ...
            >
              ...
              />
              <ReferenceLine
                y={piplMaxPenalty + pdplMaxPenalty}
                stroke={C.red}
                strokeDasharray="4 4"
                label={{ value: "Max", fill: C.red, fontSize: 9 }}
              />
              <Line
                type="monotone"
                dataKey="pdpl"
          ...
          >
            {[
              ["pdpl", C.yellow, "PDPL Exposure"],
              ["pipl", C.orange, "PIPL Exposure"],
              ["combined", C.red, "Combined"],
            ].map(([k, c, l]) => (
              <div
                key={k as string}
              ...
              >
                ...
                />
                <span style={{ color: "var(--djac-muted)", fontSize: 9.5 }}>
                  {l as string}
                </span>
              </div>
            ))}
        ...
        <div style={{ padding: "12px 18px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div>
              ...
              >
                {t(
                  "regPulse.annualRevenue",
                  "Estimated Annual Revenue (SAR / CNY)"
                )}
              </Label>
              <Input
                type="text"
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <Label
                ...
                style={{
                  fontSize: 11,
                  color: hasPersonalData ? C.orange : "var(--djac-muted)",
                  cursor: "pointer",
                  fontWeight: hasPersonalData ? 700 : 400,
                }}
              >
                {hasPersonalData
                  ? t(
                  ...
                  : t(
                      "regPulse.personalDataOff",
                      "Personal Data Component: Off (PIPL 3% cap)"
                    )}
              </Label>
            </div>
            ...
            >
              {[
                {
                  label: "PIPL Max Penalty",
                  value: formatCurrency(piplMaxPenalty, "cny"),
                  color: C.orange,
                  article: hasPersonalData ? "Art. 66 (5%)" : "Art. 66 (3%)",
                  flag: "🇨🇳",
                },
                {
                  label: "PDPL Max Penalty",
                  value: formatCurrency(pdplMaxPenalty, "sar"),
                  color: C.yellow,
                  article: "Art. 23–24 (3%)",
                  flag: "🇸🇦",
                },
                ...
                >
                  ...
                  >
                    ...
                    />
                    <span style={{ color, fontSize: 9.5, fontWeight: 700 }}>
                      {flag} {label}
                    </span>
                  </div>
                  <p
              ...
              <p style={{ margin: 0, fontSize: 11, color: "var(--djac-text)" }}>
                ...
                {hasPersonalData && (
                  <span style={{ color: C.orange, fontWeight: 700 }}>
                    {" "}
                    (+{formatCurrency(Math.round(revenue * 0.02), "sar")} PIPL
                    personal-data increment)
                  </span>
                )}
              </p>
            </div>
