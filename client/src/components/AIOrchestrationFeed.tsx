...
const STAGE_META: Record<StageKey, StageMeta> = {
  queued: {
    label: "Queue",
    icon: Clock,
    description: "Waiting for worker slot",
  },
  gatekeeper: {
    label: "Gatekeeper",
    icon: Shield,
    description: "Security & PII pre-check",
  },
  intake: {
    label: "Intake Clerk",
    icon: FileSearch2,
    description: "Document parsing & normalization",
  },
  extractor: {
    label: "Extractor",
    icon: Cpu,
    description: "Entity & clause extraction",
  },
  rag_context: {
    label: "RAG Context",
    icon: Brain,
    description: "Retrieve regulatory embeddings",
  },
  judge: {
    label: "Judge",
    icon: Scale,
  ...
  validator: {
    label: "Validator",
    icon: ClipboardCheck,
    description: "Schema & scoring validation",
  },
  reporter: {
    label: "Reporter",
    icon: FileSearch2,
    description: "PDF / JSON report emission",
  },
  persistence: {
    label: "Persist",
    icon: Archive,
    description: "Audit ledger write",
  },
  completed: {
    label: "Complete",
    icon: CheckCircle2,
    description: "Assessment finalised",
  },
  failed: {
    label: "Failed",
    icon: AlertTriangle,
...
export function AIOrchestrationFeed() {
  ...
  function stageNodeColor(idx: number): string {
    ...
    if (latestJob.status === "failed" && idx === currentStageIndex)
      return C.red;
  ...
  function stageNodeStatus(
    ...
    if (idx === currentStageIndex)
      return latestJob.status === "completed" ? "done" : "active";
    return "pending";
  }
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {jobsQuery.isError && (
            <WifiOff className="h-3.5 w-3.5" style={{ color: C.red }} />
          )}
          <Badge
            variant="outline"
            style={{
                      ...
                      {nodeStatus === "active" && (
                        <div
                          style={{
                            ...
                            opacity: 0,
                            animation:
                              "djac-stage-ping 1.5s ease-in-out infinite",
                          }}
                        />
                      )}
                      {nodeStatus === "active" &&
                  ...
                  {idx < STAGE_ORDER.length - 1 && (
                    <div
                      style={{
                        ...
                        height: 2,
                        background:
                          stageNodeColor(idx) === "var(--djac-border)"
                            ? "var(--djac-border)"
                            : `${stageNodeColor(idx)}50`,
                        flexShrink: 0,
                        marginBottom: 20,
                      }}
                    />
      ...
      {assessment && (
        ...
        >
          <MiniGauge
            ...
            color={C.cyan}
            label={t("assessmentHistory.scoreOverall", "Overall")}
          />
          <MiniGauge
          ...
          <div style={{ flex: 1 }}>
            ...
            {assessment.gaps && assessment.gaps.length > 0 && (
              <p
                style={{ color: "var(--djac-muted)", fontSize: 10, margin: 0 }}
              >
                {assessment.gaps.length} compliance gap
                {assessment.gaps.length !== 1 ? "s" : ""} detected
              </p>
            )}
          </div>
        </div>
      ...
      <div style={{ padding: "10px 18px 0" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {jobs.length === 0 ? (
            ...
            >
              {t("orchFeed.noJobs", "No assessment jobs yet.")}
            </p>
          ) : (
            jobs.slice(0, 4).map(job => {
              const stIdx = stageIndex(job.stage);
              const progress = Math.round(
                (stIdx / (STAGE_ORDER.length - 1)) * 100
              );
              const statusColor =
      ...
      >
        ...
        >
          ...
          >
            <Zap
              className="h-2.5 w-2.5 inline mr-1"
              style={{ color: C.cyan, verticalAlign: "middle" }}
            />
            {t("orchFeed.schemaLog", "Schema Enforcement Log")}
          </p>
          <span style={{ color: "var(--djac-muted)", fontSize: 9 }}>
        ...
        >
          {events.length === 0 ? (
            ...
            >
              {latestJob
                ? t("orchFeed.waitingEvents", "Waiting for pipeline events…")
                : t("orchFeed.noEvents", "No events recorded.")}
            </p>
          ) : (
            events.map((ev, i) => {
