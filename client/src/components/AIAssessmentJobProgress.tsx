  ...
  if (snapshot.status === "running") {
    return (
      <Badge className="bg-blue-100 text-blue-800">
        {t("common.status.running", "Running")}
      </Badge>
    );
  }
...
export function AIAssessmentJobProgress({
  snapshot,
}: {
  snapshot: AiAssessmentJobSnapshot;
}) {
    ...
    <div className="rounded-lg border border-blue-200 bg-blue-50/70 p-4 dark:bg-blue-950/20">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
        <div className="flex items-center gap-2 text-foreground">
          ...
          <span className="font-medium">
            {t("supplier.progressTitle", "AI Orchestrator Progress")}
          </span>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-3 text-sm text-muted-foreground">
          <span>
            {t("supplier.progressStage", "Stage")}:{" "}
            {formatStage(snapshot.stage, t)}
          </span>
          <span>{progress}%</span>
        </div>
      ...
      {snapshot.status === "completed" && snapshot.persistence ? (
        <p className="mt-3 text-sm text-muted-foreground">
          {t("supplier.progressPersistence", "Persistence")}:{" "}
          {snapshot.persistence.skipped
