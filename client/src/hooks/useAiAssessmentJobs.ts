...
export function useAiAssessmentJobs() {
  ...
  const [snapshots, setSnapshots] = useState<
    Record<string, AiAssessmentJobSnapshot>
  >({});
  ...
  const subscribeToJob = usePersistFn(
    (jobId: string, listener?: JobSnapshotListener) => {
      ...
      if (listener) {
        const existing =
          callbacksRef.current.get(normalizedJobId) ||
          new Set<JobSnapshotListener>();
