...
const RISK_COLORS: Record<
  RiskLevel,
  { bg: string; text: string; border: string; gauge: string; arc: string }
> = {
  critical: {
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <span
          className={`absolute bottom-0 left-1/2 -translate-x-1/2 text-base font-bold leading-none ${c.text}`}
        >
          {score}
        </span>
      </div>
...
function ModulePill({
  ...
  return (
    <span
      className={`text-xs px-2 py-0.5 rounded flex items-center gap-1 ${warn && value > 0 ? "bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-300" : "bg-muted text-muted-foreground"}`}
    >
      {label}: <b>{value}</b>
    </span>
  );
...
export default function VendorComplianceProfiles() {
  ...
  const [sortBy, setSortBy] = useState<"score-desc" | "score-asc" | "name">(
    "score-desc"
  );
  ...
  } = trpc.vendorCompliance.list.useQuery();
  const {
    data: profile,
    isLoading: profileLoading,
    isError: profileError,
    refetch: refetchProfile,
  } = trpc.vendorCompliance.profile.useQuery(
  ...
  const filtered = useMemo(() => {
    ...
    const list = vendors.filter(v => {
      ...
      if (
        ...
        !(v.headquartersLocation ?? "").toLowerCase().includes(q)
      )
        return false;
      return true;
    });
    ...
    <div className="djac-page">
      ...
      <div>
        <h1 className="text-2xl font-bold">
          {t("vcp.title", "Vendor Compliance Profiles")}
        </h1>
        <p className="text-sm text-muted-foreground mt-0.5">
          {t(
            "vcp.subtitle",
            "360° compliance health for each vendor across all modules"
          )}
        </p>
      </div>
        ...
        <Select value={filterLevel} onValueChange={setFilterLevel}>
          ...
          <SelectContent>
            <SelectItem value="all">
              {t("vcp.filterAllLevels", "All risk levels")}
            </SelectItem>
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="score-desc">
              {t("vcp.sortScoreDesc", "Score: high → low")}
            </SelectItem>
            <SelectItem value="score-asc">
              {t("vcp.sortScoreAsc", "Score: low → high")}
            </SelectItem>
            <SelectItem value="name">
              {t("vcp.sortName", "Name A → Z")}
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      {vendorsError ? (
        <Card>
          <CardContent className="py-8 text-center">
            <p className="text-sm text-muted-foreground">
              {t("vendorRisk.error", "Failed to load vendors.")}
            </p>
            <Button
              variant="outline"
              size="sm"
        ...
        <p className="text-sm text-muted-foreground py-8 text-center">
          {t("vcp.loading", "Loading vendor data…")}
        </p>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 space-y-2">
          <Users className="w-10 h-10 mx-auto text-muted-foreground opacity-40" />
          <p className="text-sm text-muted-foreground">
            {t("vcp.emptyTitle", "No vendors found")}
          </p>
          <p className="text-xs text-muted-foreground">
            {t("vcp.emptyDesc", "Add vendors via the Vendor Assessment page.")}
          </p>
        </div>
      ) : (
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <div className="flex-1 min-w-0 space-y-1.5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <ModulePill
                          label={t("vcp.assess", "Assessments")}
                          value={v.assessmentCount}
                        />
                        <ModulePill
                          label={t("vcp.gaps", "Open Gaps")}
                          value={v.openGaps}
                          warn
                        />
                        <ModulePill
                          label={t("vcp.risks", "Open Risks")}
                          value={v.openRisks}
                          warn
                        />
                        <ModulePill
                          label={t("vcp.remed", "Remediations")}
                          value={v.openRemediations}
                          warn
                        />
                        <ModulePill
                          label={t("vcp.incidents", "Incidents")}
                          value={v.openIncidents}
                          warn
                        />
                      </div>
                    </div>
      ...
      {/* Profile Dialog */}
      <Dialog
        open={profileVendorId !== null}
        onOpenChange={v => {
          if (!v) setProfileVendorId(null);
        }}
      >
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {profileData?.vendor?.vendorName ??
                t("vcp.profile", "Vendor Profile")}
            </DialogTitle>
            <DialogDescription>
              {t(
                "vcp.profileDesc",
                "Full compliance breakdown across all modules"
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          {profileError ? (
            <div className="py-6 text-center">
              <p className="text-sm text-muted-foreground">
                {t("vendorDetail.loadError", "Failed to load vendor details")}
              </p>
              <Button
                variant="outline"
                size="sm"
            ...
            <p className="text-sm text-muted-foreground py-6 text-center">
              {t("vcp.loading", "Loading…")}
            </p>
          ) : !profileData ? (
            <p className="text-sm text-muted-foreground py-6 text-center">
              {t("vcp.noProfile", "No data available")}
            </p>
          ) : (
            <div className="space-y-4 py-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  {
                    label: t("vcp.assess", "Assessments"),
                    value: profileData.assessments.length,
                    icon: BarChart2,
                    href: `/vendor-assessment?vendorId=${profileVendorId}`,
                  },
                  {
                    label: t("vcp.gaps", "Gap Findings"),
                    value: profileData.gaps.length,
                    icon: ShieldOff,
                    href: `/gap-tracker?vendorId=${profileVendorId}`,
                  },
                  {
                    label: t("vcp.risks", "Risk Entries"),
                    value: profileData.risks.length,
                    icon: AlertTriangle,
                    href: `/risk-register?vendorId=${profileVendorId}`,
                  },
                  {
                    label: t("vcp.remed", "Remediations"),
                    value: profileData.remediationTasks.length,
                    icon: Wrench,
                    href: `/remediation-planner?vendorId=${profileVendorId}`,
                  },
                  {
                    label: t("vcp.incidents", "Incidents"),
                    value: profileData.incidents.length,
                    icon: ShieldAlert,
                    href: `/incident-register?vendorId=${profileVendorId}`,
                  },
                ].map(({ label, value, icon: Icon, href }) => (
                  <Link
                    key={label}
                    href={href}
                    className="flex items-center gap-2 rounded-lg border p-3 hover:bg-muted/50 transition-colors group"
                  >
              ...
              {profileData.gaps.length > 0 && (
                <section>
                  <h3 className="text-sm font-semibold mb-2 flex items-center gap-1">
                    ...
                    <Link
                      href={`/gap-tracker?vendorId=${profileVendorId}`}
                      className="ml-auto text-xs text-primary flex items-center gap-0.5 hover:opacity-75 transition-opacity"
                    >
                      {t("vcp.viewAll", "View all")}{" "}
                  ...
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {profileData.gaps.slice(0, 20).map((g: any) => (
                      ...
                      >
                        <Badge
                          className={`text-xs px-1.5 ${g.severity === "critical" ? "bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300" : g.severity === "high" ? "bg-orange-100 text-orange-700 dark:bg-orange-950 dark:text-orange-300" : "bg-muted text-muted-foreground"}`}
                        >
                          {g.severity}
                        </Badge>
                ...
                <section>
                  ...
                  <div className="space-y-1 max-h-32 overflow-y-auto">
                    {profileData.incidents.slice(0, 10).map((i: any) => (
                      ...
                      >
                        <Badge
                          className={`text-xs px-1.5 ${i.severity === "critical" ? "bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300" : "bg-muted text-muted-foreground"}`}
                        >
                          {i.severity}
                        </Badge>
