...
function useBookmarks() {
  const [bookmarks, setBookmarks] = useState<Set<string>>(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem("ll-bookmarks") ?? "[]"));
    } catch {
      return new Set();
    }
  });
...
function ArticleDetailPanel({
  ...
  const q = trpc.compliance.lawBySlug.useQuery(
    { slug },
    { refetchOnWindowFocus: false }
  );
  const allQ = trpc.compliance.laws.useQuery(undefined, {
    staleTime: 60_000,
    refetchOnWindowFocus: false,
  });
  ...
  if (!law)
    return (
      <p className="py-3 text-sm text-muted-foreground">No record found.</p>
    );
...
export default function LawLibrary() {
  ...
  // Error toasts
  useEffect(() => {
    if (allLawsQuery.error)
      toast.error(t("laws.loadError", "Failed to load law library."));
  }, [allLawsQuery.error]);
  useEffect(() => {
    if (searchQuery.error)
      toast.error(t("laws.searchError", "Search failed. Please try again."));
  }, [searchQuery.error]);
  ...
  const displayedItems = useMemo(() => {
    ...
    let out =
      fwFilter === "All"
        ? base
        : base.filter(it => it.frameworkCodes.includes(fwFilter));
    ...
    <div className="djac-page space-y-6">
      ...
      <div className="flex flex-col gap-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder={t(
              ...
              "Search laws, sections, obligations, penalties…"
            )}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            size="sm"
            variant={fwFilter === "All" ? "default" : "outline"}
          ...
          >
            ...
            <Badge
              variant="secondary"
              className="ml-0.5 h-4 text-[10px] px-1.5"
            >
        ...
        <div className="space-y-3">
          {isSearching && (
            <p className="text-xs text-muted-foreground">
              ...
              {displayedItems.length !== 1 ? "s" : ""} for &ldquo;{query}&rdquo;
            </p>
          )}
          ...
          {displayedItems.map(item => {
            ...
            const highlights = (
              item as { highlights?: { title: string; excerpt: string }[] }
            ).highlights;
            const matchedTopics = (item as { matchedTopics?: string[] })
              .matchedTopics;
