...
function validateVendorForm(form: VendorFormState) {
  ...
  if (
    ["single-public-cloud", "multi-cloud", "hybrid"].includes(
      form.hostingEnvironment
    ) &&
    ...
    return {
      ...
      fallback:
        "Choose at least one cloud provider for your hosting environment.",
    } satisfies ValidationMessage;
  }
...
function ThemedValueSelect<T extends string>({
  ...
  const normalizedValue =
    value && value.length > 0 ? value : allowEmpty ? "__empty__" : undefined;

  return (
    <Select
      value={normalizedValue}
      onValueChange={next =>
        onChange((next === "__empty__" ? "" : next) as T | "")
      }
    >
...
function SimpleStringSelect({
  ...
  const normalizedValue = hasMatchingValue
    ? value
    : placeholder
      ? "__empty__"
      : undefined;
...
export default function ClientWorkspace() {
  ...
  const [vendorJurisdictionFilter, setVendorJurisdictionFilter] =
    useState<string>("all");
  const [previewOpen, setPreviewOpen] = useState(false);
  const [previewResult, setPreviewResult] =
    useState<DraftAssessmentResult | null>(null);

  const vendorListQuery = trpc.vendor.list.useQuery(undefined, {
    refetchOnWindowFocus: false,
  });
  const jobListQuery = trpc.ai.listAssessmentJobs.useQuery(
    { limit: 30 },
    {
      refetchInterval: 5000,
    }
  );
  const saudiFrameworksQuery =
  ...
  const profileMutation = trpc.auth.updateProfile.useMutation({
    ...
    onError: err =>
      sonnerToast.error(
        t("client.profileUpdateFailed", "Profile update failed") +
          ": " +
          err.message
      ),
  });
  ...
  const createVendorMutation = trpc.vendor.create.useMutation({
    ...
    onError: err =>
      sonnerToast.error(
        t("client.vendorSaveFailed", "Vendor save failed") + ": " + err.message
      ),
  });
  ...
  const assessDraftMutation = trpc.vendor.assessDraft.useMutation({
    ...
    onError: err =>
      sonnerToast.error(
        t("client.previewError", "Preview failed") + ": " + err.message
      ),
  });

  const submitConsultationMutation =
    trpc.portal.submitAuthenticatedConsultation.useMutation({
      onSuccess: () => {
        sonnerToast.success(
          t(
            "client.toastConsultationSubmitted",
      ...
      onError: err =>
        sonnerToast.error(
          t(
            "client.consultationSubmitFailed",
  ...
  const submitAssessmentMutation = trpc.ai.submitAssessment.useMutation({
    onSuccess: data => {
      ...
      sonnerToast.success(
        t("client.toastAssessmentQueued", "Assessment queued"),
        {
    ...
    onError: err =>
      sonnerToast.error(
        t("client.assessmentSubmitFailed", "Assessment submission failed") +
          ": " +
          err.message
      ),
  });
  ...
  const vendorNameById = useMemo(() => {
    return new Map(
      (vendorListQuery.data ?? []).map(vendor => [vendor.id, vendor.vendorName])
    );
    ...
    if (vendorJurisdictionFilter !== "all") {
      list = list.filter(v =>
        v.regulatoryJurisdictions
          ? v.regulatoryJurisdictions
              .split(";")
              .includes(vendorJurisdictionFilter)
          : false
      );
    }
    return [...list].sort((a, b) =>
      vendorSortKey === "risk"
        ? (riskOrder[b.riskTier ?? ""] ?? 0) -
          (riskOrder[a.riskTier ?? ""] ?? 0)
  ...
  const handleCreateVendor = async () => {
    ...
    await createVendorMutation.mutateAsync({
      ...
      hostingEnvironment:
        vendorForm.hostingEnvironment as VendorHostingEnvironment,
      criticalityLevel: vendorForm.criticalityLevel as VendorCriticalityLevel,
      riskTier: vendorForm.riskTier as VendorRiskTier,
      thirdPartyDependencies:
        vendorForm.thirdPartyDependencies as VendorDependencyLevel,
      fourthPartyDependencies:
        vendorForm.fourthPartyDependencies as VendorDependencyLevel,
    });
  };
  ...
  const handleConsultationSubmit = async () => {
    if (
      ...
      sonnerToast.error(
        t(
          "client.validationCompleteConsultationFields",
          "Complete all consultation fields before submitting."
        )
      );
      return;
    }
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t(
                "client.signInDesc",
                "Sign in to manage your organization profile and compliance assessments."
              )}
            </CardDescription>
          </CardHeader>
        </Card>
    ...
    <div className="djac-page">
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
          <div>
            ...
            >
              {t("client.heroTitle", "Compliance Operating Hub")}
            </h1>
            <p
            ...
            >
              {t(
                "client.heroDesc",
                "Manage your organization profile, submit vendor technology stacks, trigger AI assessments, and request jurisdiction-specific consultation support."
              )}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.1fr_1.4fr]">
        <Card className="border-border/80">
          <CardHeader>
            ...
            <CardDescription>
              {t(
                "client.orgProfileDesc",
                "Keep account identity, contact ownership, and preferred language aligned for audit trails."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div>
                <Label>
                  {t("client.labelPreferredLocale", "Preferred Locale")}
                </Label>
                <SimpleStringSelect
            ...
            <p className="text-xs text-muted-foreground">
              {t(
                "client.profileHint",
                "Keep your name, email, and organization up to date."
              )}
            </p>
            <Button
            ...
            >
              {profileMutation.isPending
                ? t("client.saving", "Saving...")
                : t("client.saveProfile", "Save Profile")}
            </Button>
            {profileMutation.isPending && (
              <p
                role="status"
        ...
        <Card className="border-border/80">
          <CardHeader>
            ...
            <CardDescription>
              {t(
                "client.vendorIntakeDesc",
                "Submit market-entry vendor details and component-level architecture signals for persistent compliance scoring."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div>
                ...
                <ThemedValueSelect
                  ...
                  onChange={value =>
                    setVendorForm(prev => ({
                      ...prev,
                      industry: value as VendorIndustry | "",
                    }))
                  }
              ...
              <div className="md:col-span-2">
                <Label>
                  {t("client.vendorDescription", "Vendor Description")}
                </Label>
                <Textarea
                  rows={3}
              ...
              <div>
                <Label>
                  {t("client.headquartersLocation", "Headquarters Location")}
                </Label>
                <ThemedValueSelect
              ...
              <div>
                ...
                <ThemedValueSelect
                  ...
                  onChange={value =>
                    setVendorForm(prev => ({
                      ...prev,
                      serviceType: value as VendorServiceType | "",
                    }))
                  }
                  options={vendorServiceTypeOptions}
                  placeholder={t(
                    "client.selectServiceType",
                    "Select service type"
                  )}
              ...
              <div>
                <Label>
                  {t("client.hostingEnvironment", "Hosting Environment")}
                </Label>
                <ThemedValueSelect
                  ...
                  onChange={value =>
                    setVendorForm(prev => ({
                      ...prev,
                      hostingEnvironment: value as
                        | VendorHostingEnvironment
                        | "",
                    }))
                  }
                  options={vendorHostingEnvironmentOptions}
                  placeholder={t(
                    "client.selectHostingEnvironment",
                    "Select hosting environment"
                  )}
                  ariaLabel={t(
                    "client.hostingEnvironment",
                    "Hosting Environment"
                  )}
                  locale={localeKey}
                />
              ...
              <div>
                <Label>
                  {t("client.criticalityLevel", "Criticality Level")}
                </Label>
                <ThemedValueSelect
                  ...
                  onChange={value =>
                    setVendorForm(prev => ({
                      ...prev,
                      criticalityLevel: value as VendorCriticalityLevel | "",
                    }))
                  }
              ...
              <div>
                ...
                <ThemedValueSelect
                  ...
                  onChange={value =>
                    setVendorForm(prev => ({
                      ...prev,
                      riskTier: value as VendorRiskTier | "",
                    }))
                  }
              ...
              <div>
                ...
                <ThemedValueSelect
                  ...
                  onChange={value =>
                    setVendorForm(prev => ({
                      ...prev,
                      thirdPartyDependencies: value as
                        | VendorDependencyLevel
                        | "",
                    }))
                  }
                  options={vendorDependencyLevelOptions}
                  placeholder={t(
                    "client.selectDependencyLevel",
              ...
              <div>
                <Label>
                  {t(
                    "client.fourthPartyDependencies",
                ...
                <ThemedValueSelect
                  ...
                  onChange={value =>
                    setVendorForm(prev => ({
                      ...prev,
                      fourthPartyDependencies: value as
                        | VendorDependencyLevel
                        | "",
                    }))
                  }
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <MultiChecklist
                ...
                onToggle={value =>
                  setVendorForm(prev => ({
                    ...prev,
                    operatingCountries: toggleArrayValue(
              ...
              <MultiChecklist
                title={t(
                  "client.regulatoryJurisdictions",
                ...
                onToggle={value =>
                  setVendorForm(prev => ({
                    ...prev,
                    regulatoryJurisdictions: toggleArrayValue(
                      prev.regulatoryJurisdictions,
                      value
                    ),
                  }))
                }
              ...
              <MultiChecklist
                title={t(
                  "client.complianceCertifications",
                ...
                onToggle={value =>
                  setVendorForm(prev => ({
                    ...prev,
                    certifications: toggleArrayValue(
                      prev.certifications,
                      value
                    ),
                  }))
              ...
              <MultiChecklist
                title={t(
                  "client.dataProcessingActivities",
                ...
                onToggle={value =>
                  setVendorForm(prev => ({
                    ...prev,
                    dataProcessingActivities: toggleArrayValue(
                      prev.dataProcessingActivities,
                      value
                    ),
                  }))
                }
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  {t("client.addComponent", "Add Component")}
                </Button>
              </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={event =>
                          setVendorForm(prev => ({
                            ...prev,
                            techStackComponents: prev.techStackComponents.map(
                              (entry, entryIndex) =>
                                entryIndex === index
                                  ? {
                                      ...entry,
                            ...
                            techStackComponents: prev.techStackComponents.map(
                              (entry, entryIndex) =>
                                entryIndex === index
                                  ? {
                                      ...entry,
                                      dataHandling: event.target.value,
                                    }
                                  : entry
                            ),
                          }))
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {createVendorMutation.isPending
                  ? t("client.saving", "Saving...")
                  : t("client.saveVendorProfile", "Save Vendor Profile")}
              </Button>
              <Button
                type="button"
                variant="outline"
              ...
              >
                {assessDraftMutation.isPending ? (
                  ...
                  <>
                    ...
                    {t("client.previewScore", "Preview Compliance Score")}
                  </>
                )}
              </Button>
              <Button
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {t("client.previewTitle", "Compliance Preview")}
            </DialogTitle>
            <DialogDescription>
              {t(
                "client.previewDesc",
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1.5">
                  <p className="text-sm text-muted-foreground">
                    {t("client.previewOverallScore", "Overall Score")}
                  </p>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t(
                "client.consultationDesc",
                "Send targeted questions to DJAC experts with context on jurisdictions, vendors, and technology concerns."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {vendorJurisdictionValues.map(value => {
                    const active =
                      consultationForm.jurisdictions.includes(value);
                    return (
                      <Button
                        key={value}
                        type="button"
                        ...
                        onClick={() =>
                          setConsultationForm(prev => ({
                            ...prev,
                            jurisdictions: toggleArrayValue(
                              prev.jurisdictions,
                              value
                            ),
                          }))
              ...
              <div>
                <Label>
                  {t(
                    "client.techStackSummaryOptional",
                    "Tech Stack Summary (Optional)"
                  )}
                </Label>
                <Input
                  ...
                  onChange={event =>
                    setConsultationForm(prev => ({
                      ...prev,
                      techStackSummary: event.target.value,
                    }))
                  }
                />
              </div>
              <div className="md:col-span-2">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("client.ctrlEnterHint", "Ctrl+Enter to submit")}
                </p>
              </div>
            </div>
            <p className="text-xs text-muted-foreground">
              {t(
                "client.consultationRequiredHint",
                "All fields except Vendor Name and Tech Stack are required."
              )}
            </p>
            <Button
            ...
            >
              {submitConsultationMutation.isPending
                ? t("client.submitting", "Submitting...")
                : t("client.submitConsultation", "Submit Consultation")}
            </Button>
            {submitConsultationMutation.isPending && (
              <p
                role="status"
              ...
              >
                {t(
                  "client.consultationSubmittingHint",
          ...
          <CardContent className="space-y-4">
            <div className="rounded-lg border border-border p-3 bg-muted/30">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <h3 className="text-sm font-semibold">
                    {t(
                      "client.globalRegistryTitle",
                      "Global Compliance Intelligence"
                    )}
                  </h3>
                  <p className="text-xs text-muted-foreground">
                    {t(
                      "client.globalRegistryDesc",
                      "Expanded framework catalog, industry editions, AI agents, and knowledge graph seed data."
                    )}
                  </p>
                </div>
                {globalRegistrySummaryQuery.isLoading ? (
                  <span className="text-xs text-muted-foreground">
                    {t("client.loadingFrameworks", "Loading frameworks...")}
                  </span>
                ) : globalRegistrySummaryQuery.isError ? (
              ...
              {globalRegistrySummaryQuery.data ? (
                <div className="mt-3 flex flex-wrap gap-2 text-xs">
                  ...
                  <Badge variant="secondary">
                    {globalRegistrySummaryQuery.data.agents} AI agents
                  </Badge>
                  <Badge variant="secondary">
                    {globalRegistrySummaryQuery.data.regions} regions
                  </Badge>
                </div>
              ) : null}
            </div>
              ...
              {saudiFrameworksQuery.isLoading ? (
                ...
                <div className="mt-2 flex flex-col gap-2">
                  <p className="text-xs text-destructive">
                    {saudiFrameworksQuery.error?.message ||
                      t(
                        "client.frameworksLoadError",
                        "Failed to load frameworks."
                      )}
                  ...
                  <Button
                    ...
                    onClick={() => saudiFrameworksQuery.refetch()}
                  >
            ...
            <div className="rounded-lg border border-border p-3">
              ...
              {chinaFrameworksQuery.isLoading ? (
                ...
                <div className="mt-2 flex flex-col gap-2">
                  <p className="text-xs text-destructive">
                    {chinaFrameworksQuery.error?.message ||
                      t(
                        "client.frameworksLoadError",
                        "Failed to load frameworks."
                      )}
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="h-7 text-xs"
                    onClick={() => chinaFrameworksQuery.refetch()}
                  >
                    {t("common.retry", "Retry")}
                  </Button>
                </div>
            ...
            <p className="text-xs text-muted-foreground">
              {t(
                ...
                "Use the Compliance Tracker and Framework Analysis sections for full obligation timelines and legal references."
              )}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              {t(
                "client.assessmentIntelligenceTitle",
                "Assessment Intelligence"
              )}
            </CardTitle>
            <CardDescription>
              {t(
                "client.assessmentIntelligenceDesc",
                "Monitor live AI pipeline progress and compare recent compliance scores."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {activeSnapshot ? (
              <AIAssessmentJobProgress snapshot={activeSnapshot} />
            ) : null}

            <div className="rounded-lg border border-border p-3">
              <p className="mb-3 text-sm font-medium">
                {t(
                  "client.recentScoreDistribution",
              ...
              {completedAssessments.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  {t(
                    "client.noCompletedAssessments",
                    "No completed AI assessments yet."
                  )}
                </p>
              ) : (
                <div className="space-y-2">
              ...
              {(["critical", "high", "medium", "low"] as const).map(level => (
                ...
                >
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">
                    {t(
                      `client.severity${level.charAt(0).toUpperCase()}${level.slice(1)}`,
                      level
                    )}
                  </p>
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between gap-3 text-lg">
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => vendorListQuery.refetch()}
              >
                <RefreshCw className="mr-1 h-3.5 w-3.5" />{" "}
                {t("client.refresh", "Refresh")}
              </Button>
            </CardTitle>
            <CardDescription>
              {t(
                "client.savedVendorsDesc",
                "Run or re-run AI assessments on stored profiles."
              )}
            </CardDescription>
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <SimpleStringSelect
                  ...
                  options={[
                    {
                      value: "name",
                      label: t("client.sortByName", "Name A→Z"),
                    },
                    {
                      value: "risk",
                      label: t("client.sortByRisk", "Highest Risk First"),
                    },
                  ]}
                />
                <ThemedValueSelect
                  value={
                    vendorJurisdictionFilter === "all"
                      ? ""
                      : (vendorJurisdictionFilter as VendorJurisdiction | "")
                  }
                  onChange={value =>
                    setVendorJurisdictionFilter(value || "all")
                  }
                  options={vendorJurisdictionOptions}
                  placeholder={t(
                    "client.filterAllJurisdictions",
                    "All Jurisdictions"
                  )}
                  ariaLabel={t(
                    "client.filterByJurisdiction",
            ...
            {vendorListQuery.isError ? (
              <div className="space-y-2">
                <p className="text-sm text-destructive">
                  {vendorListQuery.error?.message ||
                    t("client.vendorsLoadError", "Failed to load vendors.")}
                </p>
                <Button
                  size="sm"
                  variant="outline"
              ...
              <p className="text-sm text-muted-foreground">
                {vendorJurisdictionFilter !== "all"
                  ? t(
                      "client.noMatchingVendors",
                      "No vendors match the selected jurisdiction."
                    )
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {vendor.serviceType ??
                          t("client.serviceFallback", "Service")}{" "}
                        ·{" "}
                        {vendor.riskTier ??
                          t("client.riskTierPending", "Risk tier pending")}
                      </p>
                    </div>
                    <Button
                      size="sm"
