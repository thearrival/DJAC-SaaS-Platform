...
function AuditTrailTab() {
  ...
  useEffect(() => {
    if (auditQuery.error)
      toast.error(t("superAdmin.auditLoadError", "Failed to load audit logs."));
  }, [auditQuery.error]);
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Select value={category} onValueChange={setCategory}>
          <SelectTrigger className="h-8 w-40 text-xs">
            <SelectValue
              placeholder={t("superAdmin.audit.category", "Category")}
            />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {t("superAdmin.audit.allCategories", "All categories")}
            </SelectItem>
            {[
              "auth",
              "data_write",
        ...
        <Select value={outcome} onValueChange={setOutcome}>
          <SelectTrigger className="h-8 w-36 text-xs">
            <SelectValue
              placeholder={t("superAdmin.audit.outcome", "Outcome")}
            />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {t("superAdmin.audit.allOutcomes", "All outcomes")}
            </SelectItem>
            <SelectItem value="success">
              {t("superAdmin.audit.success", "Success")}
            </SelectItem>
            <SelectItem value="failure">
              {t("superAdmin.audit.failure", "Failure")}
            </SelectItem>
            <SelectItem value="blocked">
              {t("superAdmin.audit.blocked", "Blocked")}
            </SelectItem>
          </SelectContent>
        </Select>
        <Button
          ...
          <thead className="bg-muted/50 text-muted-foreground">
            <tr>
              <th className="px-3 py-2 text-left font-medium w-[140px]">
                {t("superAdmin.audit.colTimestamp", "Timestamp")}
              </th>
              <th className="px-3 py-2 text-left font-medium">
                {t("superAdmin.audit.colCategory", "Category")}
              </th>
              <th className="px-3 py-2 text-left font-medium">
                {t("superAdmin.audit.colAction", "Action")}
              </th>
              <th className="px-3 py-2 text-left font-medium">
                {t("superAdmin.audit.colTarget", "Target")}
              </th>
              <th className="px-3 py-2 text-left font-medium">
                {t("superAdmin.audit.colRole", "Actor Role")}
              </th>
              <th className="px-3 py-2 text-center font-medium w-[70px]">
                {t("superAdmin.audit.colOutcome", "Outcome")}
              </th>
            </tr>
          </thead>
              ...
              logs.map(log => (
                <tr key={log.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-3 py-2">
                    <span
                      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${CATEGORY_BADGE[log.category] ?? ""}`}
                    >
...
function SaaSMetricsTab() {
  const { t } = useLocale();
  const statsQuery = trpc.admin.conversionStats.useQuery(undefined, {
  ...
  useEffect(() => {
    if (statsQuery.error)
      toast.error(
        t("superAdmin.metricsLoadError", "Failed to load SaaS metrics.")
      );
  }, [statsQuery.error]);
  ...
  return (
    <div className="space-y-6">
      {statsQuery.isError ? (
        ...
        <p className="text-sm text-muted-foreground">
          {t("superAdmin.metrics.noData", "No metrics available.")}
        </p>
      ) : (
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card>
              <CardHeader className="pb-2">
                <CardDescription>
                  {t("superAdmin.metrics.totalOrgs", "Total Organisations")}
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          {stats.planBreakdown.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>
                  {t("superAdmin.metrics.planBreakdown", "Plan Breakdown")}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
...
function RoleManagementTab() {
  const { t, locale } = useLocale();
  const usersQuery = trpc.role.listUsersWithRoles.useQuery(
    { limit: 200, offset: 0 },
    { refetchInterval: 60_000 }
  );
  const assignRole = trpc.role.assignUserRole.useMutation({
    ...
    onError: err =>
      toast.error(
        t("superAdmin.roleAssignError", "Failed to assign role.") +
          " " +
          err.message
      ),
  });
  ...
  useEffect(() => {
    if (usersQuery.error)
      toast.error(t("superAdmin.usersLoadError", "Failed to load users."));
  }, [usersQuery.error]);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <p className="flex-1 text-sm text-muted-foreground">
          {t(
            "superAdmin.roles.description",
            "Assign platform-level roles to users. Changes are logged in the audit trail."
          )}
        </p>
        <div className="relative">
          ...
          <Input
            ...
            placeholder={t(
              "superAdmin.roles.searchPlaceholder",
              "Search users…"
            )}
          ...
          <thead className="bg-muted/50 text-xs uppercase text-muted-foreground">
            <tr>
              <th className="px-4 py-2 text-left font-medium">
                {t("companyDash.colName", "Name")}
              </th>
              <th className="px-4 py-2 text-left font-medium">
                {t("companyDash.colEmail", "Email")}
              </th>
              <th className="px-4 py-2 text-left font-medium">
                {t("superAdmin.roles.lastSeen", "Last Signed In")}
              </th>
              <th className="px-4 py-2 text-left font-medium">
                {t("companyDash.colCurrentRole", "Current Role")}
              </th>
              <th className="px-4 py-2 text-left font-medium">
                {t("companyDash.colAssignRole", "Assign Role")}
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
                ...
                >
                  <p>
                    {usersQuery.error?.message ??
                      t("superAdmin.usersLoadError", "Failed to load users.")}
                  </p>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  <td className="px-4 py-2">
                    <Select
                      ...
                      onValueChange={newRole =>
                        assignRole.mutate({
                          targetUserId: u.id,
                          newRole: newRole as Parameters<
                            typeof assignRole.mutate
                          >[0]["newRole"],
                        })
                      }
                    >
...
function SystemHealthTab() {
  ...
  useEffect(() => {
    if (readinessQuery.error)
      toast.error(
        t("superAdmin.healthLoadError", "Failed to load system health.")
      );
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <p className="text-sm font-medium">
          {t("superAdmin.health.overallStatus", "Overall Status:")}
        </p>
        <Badge
          variant={overall === true ? "default" : "destructive"}
            ...
            >
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {s.details ?? (isOk ? "Operational" : "Degraded")}
                </p>
              </div>
              {isOk ? (
    ...
    <div className="djac-page">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {t(
            "superAdmin.subtitle",
            "Full platform visibility: audit trails, SaaS metrics, role management, and system health."
          )}
        </p>
      </div>
