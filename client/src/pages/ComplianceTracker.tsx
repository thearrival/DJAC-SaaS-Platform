...
const RISK_PILL_CLASS: Record<RiskFilterLevel, string> = {
  ...
  critical:
    "bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300 border-red-400",
  high: "bg-orange-100 text-orange-700 dark:bg-orange-950 dark:text-orange-300 border-orange-400",
  medium:
    "bg-yellow-100 text-yellow-700 dark:bg-yellow-950 dark:text-yellow-300 border-yellow-400",
...
function RiskBadge({ level }: { level: string }) {
  const { t } = useLocale();
  const color =
    RISK_COLORS[level as RiskLevel] ?? "bg-muted text-muted-foreground";
  return (
    <span
...
function ObligationCard({
  ...
  return (
    <div className="rounded-lg border bg-card p-4 space-y-3 hover:shadow-md transition-shadow">
          ...
          {obligation.references.length > 0 && (
            <div>
              <p className="text-xs font-medium uppercase text-muted-foreground mb-1">
                {t("tracker.references", "References")}
              </p>
              <ul className="list-disc pl-4 space-y-0.5">
                {obligation.references.map(ref => (
                  <li key={ref} className="text-xs text-muted-foreground">
                    {ref}
                  </li>
                ))}
              </ul>
      ...
      >
        {expanded
          ? t("tracker.hideDetails", "Hide details")
          : t("tracker.showDetails", "Show details")}
      </button>
    </div>
  );
}
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center gap-2 py-12 text-muted-foreground">
        ...
        <span>
          {t("tracker.loadingObligations", "Loading compliance obligations...")}
        </span>
      </div>
    );
  }
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-foreground">
                  {t("tracker.criticalObligations", "Critical Obligations")}
                </h3>
          ...
          {others.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-foreground">
                  {t("tracker.otherObligations", "Other Obligations")}
                </h3>
...
function ComparisonTab() {
  ...
  } = trpc.compliance.comparisonTable.useQuery(undefined, {
    refetchOnWindowFocus: false,
  });
  useEffect(() => {
    if (comparisonError)
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center gap-2 py-12 text-muted-foreground">
        <LoaderCircle className="h-5 w-5 animate-spin" />
        <span>
          {t("tracker.loadingComparison", "Loading comparison data...")}
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          <Input
            ...
            placeholder={t(
              "tracker.comparisonSearchPlaceholder",
              "Search topics..."
            )}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              <th className="px-4 py-3 text-left font-semibold text-foreground w-1/5">
                {t("tracker.topicHeader", "Topic")}
              </th>
              ...
              <th className="px-4 py-3 text-left font-semibold text-muted-foreground w-1/5">
                {t("tracker.notesHeader", "Notes")}
              </th>
            </tr>
          </thead>
          <tbody>
            {filteredRows.length === 0 ? (
              <tr>
                ...
                >
                  {search
                    ? t(
                        "tracker.noComparisonMatch",
                        "No topics match your search."
                      )
                    : t(
                        "tracker.noComparisonData",
              ...
              filteredRows.map(row => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-muted-foreground text-xs align-top italic">
                    {row.notes}
                  </td>
                </tr>
              ))
      ...
      {data && data.length > 0 && (
        <p className="text-xs text-muted-foreground text-right">
          {search
            ? `${filteredRows.length} / ${data.length} topics`
            : `${data.length} topics`}
        </p>
      )}
    </div>
...
function SummaryCards() {
  ...
  const saCount =
    allObligations?.filter(o => o.country === "Saudi Arabia").length ?? 0;
  const cnCount =
    allObligations?.filter(o => o.country === "China").length ?? 0;
  const criticalCount =
    allObligations?.filter(o => o.riskLevel === "critical").length ?? 0;
          ...
          <div>
            <p className="text-sm font-medium text-foreground">
              {t(
                ...
                "Failed to load obligations summary."
              )}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
        ...
        <div className="mb-4 rounded-xl border border-red-300 dark:border-red-800 bg-red-50 dark:bg-red-950/30 px-4 py-3 flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-red-800 dark:text-red-300">
              Priority Obligation — {mostUrgent.framework}
            </p>
            <p className="text-xs text-red-700 dark:text-red-400 mt-0.5 truncate">
              {mostUrgent.requirement}
            </p>
          </div>
          <span className="flex-shrink-0 rounded-full bg-red-100 dark:bg-red-900/50 border border-red-300 dark:border-red-700 px-2.5 py-0.5 text-xs font-semibold text-red-800 dark:text-red-300">
            {mostUrgent.riskLevel}
          </span>
        </div>
      )}
          ...
          <CardContent className="pt-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {t("tracker.cnObligations", "China Obligations")}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {t("tracker.criticalRiskItems", "Critical Risk Items")}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {t("tracker.timeCriticalReports", "Time-Critical Reports")}
                </p>
              </div>
            </div>
          </CardContent>
