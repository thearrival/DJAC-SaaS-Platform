...
function exportControlsCsv(
  ...
  a.href = url;
  a.download = `${frameworkCode}-controls-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
...
export default function FrameworkAnalysis() {
  ...
  } = trpc.compliance.frameworks.useQuery();
  ...
  } = trpc.compliance.controls.useQuery(selectedFrameworkId || 0, {
  ...
  } = trpc.compliance.relationships.useQuery(selectedFrameworkId || 0, {
    enabled: selectedFrameworkId !== null,
  });
  ...
  useEffect(() => {
    if (frameworksError)
      toast.error(
        t("analysis.loadError", "Failed to load compliance frameworks.")
      );
  }, [frameworksError]);
  ...
  useEffect(() => {
    if (controlsError)
      toast.error(
        t("analysis.controlsError", "Failed to load framework controls.")
      );
  }, [controlsError]);
  ...
  useEffect(() => {
    if (relationshipsError)
      toast.error(
        t(
          "analysis.relationshipsError",
          "Failed to load framework relationships."
        )
      );
  }, [relationshipsError]);
  ...
  const categoryOptions = useMemo(() => {
    if (!controls) return [];
    const cats = [
      ...new Set(controls.map(c => c.category).filter(Boolean)),
    ] as string[];
    return cats.sort();
  }, [controls]);
  ...
  const filteredControls = useMemo(() => {
    ...
    return controls.filter(c => {
      if (controlCategory !== "all" && c.category !== controlCategory)
        return false;
      if (!q) return true;
      return (
  ...
  const getCategoryColor = (category: string) => {
    const tokens: Record<string, string> = {
      ...
      "Data Security":
        "bg-purple-100 text-purple-900 dark:bg-purple-900/20 dark:text-purple-200 border border-purple-300 dark:border-purple-700",
      "Data Transfer":
        "bg-orange-100 text-orange-900 dark:bg-orange-900/20 dark:text-orange-200 border border-orange-300 dark:border-orange-700",
      "Incident Response":
        "bg-destructive/10 text-destructive border border-destructive/20",
      "Risk Management":
        "bg-yellow-100 text-yellow-900 dark:bg-yellow-900/20 dark:text-yellow-200 border border-yellow-300 dark:border-yellow-700",
      "Network Security":
        "bg-blue-100 text-blue-900 dark:bg-blue-900/20 dark:text-blue-200 border border-blue-300 dark:border-blue-700",
      "Access Control":
        "bg-cyan-100 text-cyan-900 dark:bg-cyan-900/20 dark:text-cyan-200 border border-cyan-300 dark:border-cyan-700",
      "Data Management":
        "bg-pink-100 text-pink-900 dark:bg-pink-900/20 dark:text-pink-200 border border-pink-300 dark:border-pink-700",
      Governance:
  ...
  return (
    <div className="djac-page">
      ...
      <div className="djac-page-header">
        <h1 className="djac-gradient-text text-3xl font-bold mb-2">
          {t("analysis.title", "Framework Analysis")}
        </h1>
        <p className="text-muted-foreground">
          {t(
            "analysis.subtitle",
            "See how every rule maps, overlaps, and conflicts — so you know exactly where you stand."
          )}
        </p>
      </div>
      ...
      <Card className="mb-8 djac-section-2">
        <CardHeader>
          <CardTitle>
            {t("analysis.selectFramework", "Select Framework")}
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <CardContent className="pt-8 pb-8">
            <div className="text-center max-w-lg mx-auto">
              ...
              <p className="text-sm text-muted-foreground mb-5">
                {t(
                  "analysis.guidanceBody",
                  "Choose one of the 5 active compliance frameworks above. You'll see a full breakdown of its controls, cross-jurisdiction relationships, severity levels, and recommended mitigations."
                )}
              </p>
        ...
        <>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <h3 className="font-semibold mb-2">
                    {t(
                      "analysis.enforcementAuthority",
                      "Enforcement Authority"
                    )}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    {selectedFramework.enforcementAuthority}
                  </p>
                </div>
                <div>
                  <h3 className="font-semibold mb-2">
                    {t("analysis.maxPenalty", "Maximum Penalty")}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    {selectedFramework.maxPenalty}
                  </p>
                </div>
                <div>
                  <h3 className="font-semibold mb-2">
          ...
          {(controls !== undefined || relationships !== undefined) && (
            <div className="mb-8 grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                {
                  label: t("analysis.statControls", "Controls"),
                  value: controls?.length ?? 0,
                  color: "text-primary",
                },
                {
                  label: t("analysis.statRelationships", "Relationships"),
                  value: relationships?.length ?? 0,
                  color: "text-purple-600 dark:text-purple-400",
                },
                {
                  label: t("analysis.statConflicts", "Conflicts"),
                  value:
                ...
                {
                  label: t("analysis.statHarmonized", "Harmonized"),
                  value:
                    relationships?.filter(r =>
                      ["coordination", "harmonization", "overlap"].includes(
                        r.relationshipType
                      )
                    ).length ?? 0,
              ...
              <Tabs defaultValue="controls" className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                  <TabsTrigger value="controls">
                    {t("analysis.controls", "Controls")} (
                    {filteredControls.length}
                    {controlSearch || controlCategory !== "all"
                      ? ` / ${controls?.length ?? 0}`
                      : ""}
                    )
                  </TabsTrigger>
                  <TabsTrigger value="relationships">
                    {t("analysis.relationships", "Relationships")} (
                      ...
                      <div className="flex gap-2 flex-wrap items-center">
                        <div className="relative flex-1 min-w-[180px]">
                          ...
                          <Input
                            ...
                            placeholder={t(
                              "analysis.searchControls",
                              "Search controls, requirements…"
                            )}
                        ...
                        <Button
                          ...
                          onClick={() =>
                            exportControlsCsv(
                              filteredControls,
                              selectedFramework?.code ?? "controls"
                            )
                          }
                        >
                      ...
                      {filteredControls.map(control => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-4 mb-2">
                            ...
                            {control.category && (
                              <Badge
                                className={getCategoryColor(control.category)}
                              >
                                {control.category}
                              </Badge>
                            )}
                              ...
                              <div className="flex gap-2">
                                ...
                                <div>
                                  <p className="font-medium">
                                    {t("analysis.requirement", "Requirement")}
                                  </p>
                                  <p className="text-muted-foreground">
                                    {control.requirement}
                                  </p>
                ...
                <TabsContent value="relationships" className="space-y-4">
                  {relationshipsLoading ? (
                          ...
                          <div className="flex items-start justify-between gap-4 mb-2">
                            <div>
                              <p className="font-semibold text-foreground">
                                ...
                                {frameworkById[rel.targetFrameworkId] ??
                                  `Framework ${rel.targetFrameworkId}`}
                              </p>
                            </div>
                            <div className="flex gap-2">
                              <Badge
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      {t(
                        "analysis.noRelationships",
                        "No relationships found for this framework"
                      )}
                    </div>
                  )}
                </TabsContent>
