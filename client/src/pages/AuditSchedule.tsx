...
export default function AuditSchedule() {
  ...
  const {
    ...
    isError,
    refetch,
  } = trpc.auditSchedule.list.useQuery();
  ...
  const createMut = trpc.auditSchedule.create.useMutation({
    onSuccess: () => {
      ...
      setShowCreate(false);
      setForm(BLANK);
      toast.success(t("audit.created", "Audit scheduled"));
      sounds.success();
    },
  });
  const patchMut = trpc.auditSchedule.patch.useMutation({
    onSuccess: () => {
      utils.auditSchedule.list.invalidate();
      setEditing(null);
      toast.success(t("audit.updated", "Audit updated"));
    },
  });
  const completeMut = trpc.auditSchedule.complete.useMutation({
    onSuccess: (data: { nextOccurrence: string | null }) => {
      ...
      const next = data.nextOccurrence
        ? ` ${t("audit.nextOn", "Next:")} ${formatDate(data.nextOccurrence)}`
        : "";
  ...
  const removeMut = trpc.auditSchedule.remove.useMutation({
    onSuccess: () => {
      ...
      toast.success(t("audit.deleted", "Audit deleted"));
    },
  });
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      upcoming: audits.filter(
        a => a.status === "planned" && new Date(a.scheduledDate) >= now
      ).length,
      overdue: audits.filter(a => isOverdue(a)).length,
    ...
    setEditing({
      ...
      scheduledDate: a.scheduledDate
        ? (a.scheduledDate instanceof Date
            ? a.scheduledDate
            : new Date(a.scheduledDate as unknown as string)
          )
            .toISOString()
            .slice(0, 10)
        : "",
  ...
  function handleScopeToggle(fw: string, isEdit: boolean) {
    if (isEdit && editing) {
      const s = editing.scope.includes(fw)
        ? editing.scope.filter(x => x !== fw)
        : [...editing.scope, fw];
      setEditing({ ...editing, scope: s });
    } else {
      const s = form.scope.includes(fw)
        ? form.scope.filter(x => x !== fw)
        : [...form.scope, fw];
  ...
  function submitCreate() {
    ...
    createMut.mutate({
      ...form,
      scope: form.scope.length ? form.scope : undefined,
    });
  }
  function submitEdit() {
    ...
    patchMut.mutate({
      id: editing.id!,
      ...editing,
      scope: editing.scope.length ? editing.scope : undefined,
    });
  }

  const isOpenDialog = showCreate || editing !== null;
  const activeForm = editing ?? form;
  const setActiveForm = (vals: Partial<typeof BLANK>) =>
    editing
      ? setEditing({ ...editing, ...vals })
      : setForm({ ...form, ...vals });
  ...
  return (
    <div className="djac-page">
      ...
      <div className="djac-page-header">
        <div>
          <h1 className="text-2xl font-bold">
            {t("audit.title", "Audit Schedule")}
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {t(
              "audit.subtitle",
              "Plan, track, and close compliance audits across all frameworks"
            )}
          </p>
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <StatCard
          label={t("audit.statTotal", "Total")}
          value={stats.total}
          icon={ClipboardCheck}
          color="bg-blue-100 dark:bg-blue-950 text-blue-600"
        />
        <StatCard
          label={t("audit.statUpcoming", "Upcoming")}
          value={stats.upcoming}
          icon={CalendarCheck2}
          color="bg-sky-100 dark:bg-sky-950 text-sky-600"
        />
        <StatCard
          label={t("audit.statOverdue", "Overdue")}
          value={stats.overdue}
          icon={AlertTriangle}
          color="bg-red-100 dark:bg-red-950 text-red-600"
        />
        <StatCard
          label={t("audit.statCompleted", "Completed")}
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <Select value={filterType} onValueChange={setFilterType}>
          ...
          <SelectContent>
            <SelectItem value="all">
              {t("audit.filterAllTypes", "All types")}
            </SelectItem>
            {AUDIT_TYPES.map(t2 => (
        ...
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          ...
          <SelectContent>
            <SelectItem value="all">
              {t("audit.filterAllStatuses", "All statuses")}
            </SelectItem>
            {STATUSES.map(s => (
      ...
      {isError ? (
        <Card>
          <CardContent className="py-8 text-center">
            <p className="text-sm text-muted-foreground">
              {t("audit.error", "Failed to load audits.")}
            </p>
            <Button
              variant="outline"
              size="sm"
        ...
        <p className="text-sm text-muted-foreground py-8 text-center">
          {t("audit.loading", "Loading audits…")}
        </p>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 space-y-2">
          <ClipboardCheck className="w-10 h-10 mx-auto text-muted-foreground opacity-40" />
          <p className="text-sm text-muted-foreground">
            {t("audit.emptyTitle", "No audits scheduled")}
          </p>
          <p className="text-xs text-muted-foreground">
            {t(
              "audit.emptyDesc",
              "Use 'Schedule Audit' to plan your next compliance audit."
            )}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map(a => {
            ...
            const typeBorder =
              TYPE_BORDERS[(a.auditType ?? "internal") as AuditType];
            return (
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge
                          className={`text-xs px-2 ${TYPE_COLORS[(a.auditType ?? "internal") as AuditType]}`}
                        >
                          {t(`audit.type.${a.auditType}`, a.auditType ?? "")}
                        </Badge>
                        <Badge
                          className={`text-xs px-2 ${STATUS_COLORS[(a.status ?? "planned") as AuditStatus]}`}
                        >
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                        ...
                        >
                          ...
                          {t("audit.scheduledFor", "Scheduled:")}{" "}
                          {formatDate(a.scheduledDate)}
                        </span>
                        {a.completedDate && (
                          <span className="flex items-center gap-1 text-green-600">
                            <CheckCircle2 className="w-3 h-3" />
                            {t("audit.completedOn", "Completed:")}{" "}
      ...
      >
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editing
                ? t("audit.editTitle", "Edit Audit")
                : t("audit.newTitle", "Schedule Audit")}
            </DialogTitle>
            <DialogDescription>
              {t(
                "audit.formDesc",
                "Fill in the details for this compliance audit."
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={v =>
                    setActiveForm({ auditType: v as AuditType })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={v =>
                    setActiveForm({ status: v as AuditStatus })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label>
                  {t("audit.fieldScheduledDate", "Scheduled Date")} *
                </Label>
                <Input
                  type="date"
                  value={activeForm.scheduledDate}
                  onChange={e =>
                    setActiveForm({ scheduledDate: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1">
                ...
                <Select
                  value={activeForm.recurrence}
                  onValueChange={v =>
                    setActiveForm({ recurrence: v as Recurrence })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="space-y-1">
              ...
              <Textarea
                ...
                placeholder={t(
                  "audit.descPlaceholder",
                  "Audit objectives and scope details…"
                )}
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <Textarea
                rows={2}
                value={activeForm.findings}
                onChange={e => setActiveForm({ findings: e.target.value })}
                placeholder={t(
            ...
            <div className="space-y-1">
              ...
              <Textarea
                ...
                onChange={e => setActiveForm({ notes: e.target.value })}
                placeholder={t(
                  "audit.notesPlaceholder",
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {t("audit.completeTitle", "Complete Audit")}
            </DialogTitle>
            <DialogDescription>
              {t(
                "audit.completeDesc",
                "Record your findings and mark this audit as completed."
              )}
            </DialogDescription>
          </DialogHeader>
            ...
            <Button
              ...
              onClick={() =>
                ...
                completeMut.mutate({
                  ...
                  findings: completeTarget.findings,
                })
              }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("audit.deleteTitle", "Delete Audit?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "audit.deleteDesc",
                "This will permanently remove the scheduled audit."
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() =>
                deleteTarget !== null && removeMut.mutate({ id: deleteTarget })
              }
            >
