...
function ProfileTab() {
  ...
  useEffect(() => {
    if (meQuery.error)
      toast.error(t("accountSettings.loadError", "Failed to load profile."));
  }, [meQuery.error]);
        ...
        <CardContent className="py-10 flex flex-col items-center gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {t(
                "accountSettings.retryHint",
                "Retry to refresh your account details."
              )}
            </p>
          </div>
          <Button
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t(
            "accountSettings.profileDesc",
            "Update your name, title and preferred language."
          )}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              <Label>
                {t("accountSettings.fieldLocale", "Preferred Language")}
              </Label>
              <Select
...
function PasswordTab() {
  ...
  const changePwMut = trpc.localAuth.changePassword.useMutation({
    onSuccess: () => {
      toast.success(
        t("accountSettings.passwordChanged", "Password changed successfully")
      );
      setCurrent("");
      setNext("");
      setConfirm("");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (next !== confirm) {
      setMatchErr(
        t("accountSettings.passwordMismatch", "Passwords do not match")
      );
      return;
    }
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4 max-w-sm">
          <div className="space-y-1.5">
            <Label>
              {t("accountSettings.currentPassword", "Current Password")}
            </Label>
            <PasswordInput
              value={current}
              onChange={setCurrent}
          ...
          <div className="space-y-1.5">
            <Label>
              {t("accountSettings.confirmPassword", "Confirm New Password")}
            </Label>
            <PasswordInput
              value={confirm}
              onChange={setConfirm}
          ...
          {changePwMut.error && (
            <p className="text-sm text-destructive">
              {changePwMut.error.message}
            </p>
          )}
          <Button
            type="submit"
            disabled={changePwMut.isPending || !current || !next || !confirm}
          >
...
function SecurityTab() {
  ...
  const disableMut = trpc.localAuth.disable2fa.useMutation({
    onSuccess: () => {
      toast.success(
        t("accountSettings.2faDisabled", "Two-factor authentication disabled")
      );
      void meQuery.refetch();
      setDisablePassword("");
    },
      ...
      <Card>
        <CardContent className="py-10 flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <p className="text-sm font-medium text-foreground">
              {t(
                "accountSettings.securityLoadError",
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm font-medium">
            {t("accountSettings.2faStatus", "Status")}
          </span>
          {mfaEnabled ? (
            <Badge>
              <ShieldCheck size={12} className="mr-1" />
              {t("accountSettings.2faActive", "Active")}
            </Badge>
          ) : (
            <Badge variant="outline">
              {t("accountSettings.2faInactive", "Inactive")}
            </Badge>
          )}
        </div>
          ...
          <div className="space-y-4">
            ...
            <details className="text-xs text-muted-foreground">
              <summary className="cursor-pointer">
                {t(
                  "accountSettings.2faManualEntry",
                  "Can't scan? Enter manually"
                )}
              </summary>
              <p className="mt-1 font-mono bg-muted px-2 py-1 rounded break-all">
                {secret}
              </p>
            </details>
            <div className="space-y-1.5 max-w-xs">
              <Label>
                {t("accountSettings.2faCode", "Authenticator Code")}
              </Label>
              <Input
                value={code}
                onChange={e =>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg border bg-muted/50 p-4">
              <p className="text-xs font-medium mb-2">
                {t(
                  "accountSettings.backupCodesTitle",
              ...
              <div className="grid grid-cols-2 gap-1">
                {backupCodes.map(c => (
                  <code
                    key={c}
                    className="font-mono text-xs bg-background border rounded px-2 py-0.5"
                  >
                    {c}
                  </code>
                ))}
            ...
            <p className="text-xs text-muted-foreground">
              {t(
                "accountSettings.backupCodesNote",
            ...
            <Button variant="outline" size="sm" onClick={copyBackupCodes}>
              ...
              {copied
                ? t("accountSettings.copied", "Copied!")
                : t("accountSettings.copyBackupCodes", "Copy backup codes")}
            </Button>
            <Button onClick={() => setStep("idle")}>
              {t("accountSettings.done", "Done")}
            </Button>
          </div>
        )}
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>
                    {t(
                      "accountSettings.2faDisableConfirmTitle",
                      "Disable two-factor authentication?"
                    )}
                  </AlertDialogTitle>
                  <AlertDialogDescription>
                    {t(
                      "accountSettings.2faDisableConfirmDesc",
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    onClick={() =>
                      disableMut.mutate({ password: disablePassword })
                    }
...
function PreferencesTab() {
  ...
  function handleRestartTour() {
    ...
    toast.success(
      t(
        "accountSettings.tourRestarted",
        "Tour will start on your next page load."
      )
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t(
            "accountSettings.prefDesc",
            "Sound effects and onboarding tour settings."
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium">
                {t("accountSettings.soundTitle", "UI Sound Effects")}
              </p>
              <p className="text-xs text-muted-foreground">
                {t(
                  "accountSettings.soundDesc",
                  "Professional micro-sounds on clicks, navigation, and notifications."
                )}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between gap-4 rounded-lg border p-4">
          <div>
            <p className="text-sm font-medium">
              {t("accountSettings.tourTitle", "Onboarding Tour")}
            </p>
            <p className="text-xs text-muted-foreground">
              {t(
                "accountSettings.tourDesc",
                "Restart the guided walkthrough that shows all platform features."
              )}
            </p>
          </div>
          <Button
      ...
      <Tabs defaultValue="profile">
        <TabsList className="mb-4">
          <TabsTrigger value="profile">
            {t("accountSettings.tabProfile", "Profile")}
          </TabsTrigger>
          <TabsTrigger value="password">
            {t("accountSettings.tabPassword", "Password")}
          </TabsTrigger>
          <TabsTrigger value="security">
            {t("accountSettings.tabSecurity", "Security")}
          </TabsTrigger>
          <TabsTrigger value="preferences">
            {t("accountSettings.tabPreferences", "Preferences")}
          </TabsTrigger>
        </TabsList>
