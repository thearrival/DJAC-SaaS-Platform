...
function GaugeArc({
  ...
  function arc(from: number, to: number, stroke: string) {
    ...
    return (
      <path
        d={`M${s.x} ${s.y} A${r} ${r} 0 ${to - from > Math.PI ? 1 : 0} 1 ${e.x} ${e.y}`}
        fill="none"
        stroke={stroke}
        strokeWidth="11"
        strokeLinecap="round"
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {sub && (
          <p style={{ color: "var(--djac-muted)", fontSize: 10, marginTop: 4 }}>
            {sub}
          </p>
        )}
      </div>
...
export default function Dashboard() {
  ...
  const TOOLTIP_STYLE = {
    background: isDark ? "#050508" : "var(--djac-bg-deep)",
  ...
  const timetableQuery = trpc.compliance.timetable.useQuery(undefined, {
  ...
  const hasLiveQueryError =
    frameworksQuery.isError || matrixQuery.isError || timetableQuery.isError;

  useEffect(() => {
    if (frameworksError)
      toast.error(
        t(
          "dashboard.frameworksLoadError",
  ...
  useEffect(() => {
    if (matrixError)
      toast.error(
        t("dashboard.matrixLoadError", "Failed to load compliance matrix.")
      );
  }, [matrixError]);
  useEffect(() => {
    if (timetableError)
  ...
  const severityBars = useMemo(
    () => [
      {
        name: t("common.severity.critical", "Critical"),
        count: kpis.critical,
        color: C.red,
      },
      {
        name: t("common.severity.high", "High"),
        count: kpis.high,
        color: C.orange,
      },
      {
        name: t("common.severity.medium", "Medium"),
        count: kpis.medium,
        color: C.cyan,
      },
      {
        name: t("common.severity.low", "Low"),
        count: kpis.low,
        color: C.green,
      },
    ],
  ...
  const activityData = useMemo(() => {
    ...
    const weights = [
      0.2, 0.15, 0.1, 0.12, 0.4, 0.75, 0.9, 1.0, 0.85, 0.7, 0.55, 0.35, 0.3,
    ];
    return [
      "00:00",
      "02:00",
  ...
  const fwScores = useMemo(() => {
    ...
    for (const fw of frameworks ?? []) {
      ...
      for (const row of matrix ?? []) {
        ...
        if (
          row.relationships.includes("overlap") ||
          row.relationships.includes("coordination")
        )
          s += 4;
      }
  ...
  const moduleHealth = useMemo(() => {
    ...
    return [
      {
        key: "risks",
        label: t("dashboard.moduleRisks", "Open Risks"),
        count: s.risks.open,
        accent: s.risks.open > 0 ? C.red : C.green,
        icon: ShieldAlert,
        path: "/risk-register",
        sub: `${s.risks.critical} critical`,
      },
      {
        key: "remed",
        label: t("dashboard.moduleRemediation", "Tasks Open"),
        count: s.remediation.open,
        accent: s.remediation.open > 0 ? C.orange : C.green,
        icon: Wrench,
        path: "/remediation-planner",
        sub: `${s.remediation.resolved} resolved`,
      },
      {
        key: "policies",
        label: t("dashboard.modulePolicies", "Active Policies"),
        count: s.policies.active,
        accent: C.cyan,
        icon: BookOpen,
        path: "/policy-manager",
        sub: `${s.policies.draft} draft`,
      },
      {
        key: "incidents",
        label: t("dashboard.moduleIncidents", "Incidents Open"),
        count: s.incidents.open,
        accent: s.incidents.open > 0 ? C.red : C.green,
        icon: AlertCircle,
        path: "/incident-register",
        sub: `${s.incidents.critical} critical`,
      },
      {
        key: "audits",
        label: t("dashboard.moduleAudits", "Upcoming Audits"),
        count: s.auditSchedule.upcoming,
        accent: s.auditSchedule.overdue > 0 ? C.orange : C.cyan,
        icon: ClipboardCheck,
        path: "/audit-schedule",
        sub: `${s.auditSchedule.overdue} overdue`,
      },
    ];
  }, [moduleSummary, C, t]);
  ...
  function conflictsFor(code: string) {
    return (matrix ?? []).filter(
      r =>
        ...
        r.relationships.includes("conflict")
    ).length;
  }
      ...
      <div className="djac-dash-root" style={{ color: C.text }}>
        ...
        >
          <div>
            ...
            >
              {t("dashboard.titlePrefix", "DJAC")}&nbsp;
              <span style={{ color: C.cyan }}>
                {t("dashboard.titleAccent", "TOOL")}
              </span>
            </h1>
          ...
          <div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>
            {[
              {
                label: t(
                  "dashboard.headerActionFrameworkAnalysis",
                  "Framework Analysis"
                ),
                path: "/analysis",
                accent: C.cyan,
              ...
              {
                label: t(
                  "dashboard.headerActionRunAssessment",
                  "Run Assessment"
                ),
                path: "/market-entry",
                accent: C.purple,
              ...
              {
                label: t(
                  "dashboard.headerActionComplianceTracker",
                  "Compliance Tracker"
                ),
              ...
              {
                label: t("dashboard.headerActionLawLibrary", "Law Library"),
                path: "/laws",
                accent: C.yellow,
              },
            ].map(btn => (
              <button
                ...
                title={btn.label}
                style={{
                  background: `${btn.accent}18`,
                  border: `1px solid ${btn.accent}40`,
                  borderRadius: 9,
                  color: btn.accent,
                  fontSize: 11,
                  fontWeight: 600,
          ...
          >
            <div>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 600 }}>
                {t(
                  "dashboard.loadError",
                  "Some live dashboard data failed to load."
                )}
              </p>
              <p style={{ margin: "2px 0 0", color: C.muted, fontSize: 11 }}>
                {t(
                  "dashboard.retryHint",
                  "Retry to refresh frameworks, matrix, and obligations feed."
                )}
              </p>
            </div>
            <button
        ...
        {/* KPI Row */}
        <div
          className="djac-dash-kpi-grid djac-section-2"
          style={{
            display: "grid",
            gridTemplateColumns: "repeat(4,1fr)",
            gap: 14,
            marginBottom: moduleHealth ? 14 : 20,
          }}
        >
          <KpiCard
            label={t("dashboard.kpiCritical", "Critical Conflicts")}
            value={kpis.critical}
            accent={C.red}
            sub={t(
              "dashboard.kpiCriticalSub",
              "of {count} framework pairs"
            ).replace("{count}", String(kpis.total))}
            icon={
              <AlertTriangle style={{ width: 20, height: 20, color: C.red }} />
            }
          />
          <KpiCard
            label={t("dashboard.kpiHighRisk", "High-Risk Pairs")}
            value={kpis.high}
            accent={C.orange}
            sub={t(
          ...
          />
          <KpiCard
            label={t("dashboard.kpiFrameworks", "Frameworks Active")}
            value={frameworks?.length ?? 0}
            accent={C.cyan}
            sub={t(
              "dashboard.kpiFrameworksSub",
              "CSL · DSL · PIPL · PDPL · NCA"
            )}
            icon={<Globe2 style={{ width: 20, height: 20, color: C.cyan }} />}
          />
          <KpiCard
        ...
        {moduleSummaryQuery.isError && (
          ...
          >
            <div>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 600 }}>
                {moduleSummaryQuery.error?.message ??
                  t(
                    "dashboard.moduleSummaryLoadError",
                    "Failed to load module health summary."
                  )}
              </p>
              <p style={{ margin: "2px 0 0", color: C.muted, fontSize: 11 }}>
                {t(
                  "dashboard.moduleSummaryRetryHint",
                  "Retry to refresh risk, remediation, policy, incident, and audit counters."
                )}
              </p>
            </div>
            <button
        ...
        {moduleHealth && (
          <div
            className="djac-dash-modules-strip djac-section-2b"
            style={{
              display: "grid",
                ...
                >
                  ...
                  <p
                    style={{
                      ...
                      fontWeight: 700,
                      margin: 0,
                      color: hasAlert ? m.accent : C.text,
                      lineHeight: 1,
                    }}
                  >
                    {m.count}
        ...
        {/* Charts Row */}
        <div
          className="djac-dash-chart-grid djac-section-3"
          style={{
            display: "grid",
        ...
        >
          ...
          >
            <p style={{ fontSize: 13, fontWeight: 600, margin: "0 0 3px" }}>
              {t(
                "dashboard.conflictActivityTitle",
                "Conflict Detection Activity"
              )}
            </p>
            <p style={{ color: C.muted, fontSize: 10, margin: "0 0 16px" }}>
              {t(
                "dashboard.conflictActivityDesc",
                "Framework relationship pair index · {count} active pairs · derived from live matrix"
              ).replace("{count}", String(kpis.total))}
            </p>
            <ResponsiveContainer width="100%" height={170}>
              <AreaChart
                data={activityData}
                margin={{ top: 0, right: 4, left: -26, bottom: 0 }}
              >
                <defs>
                  <linearGradient id="gConflict" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area
                  type="monotone"
                  dataKey="conflicts"
                  name={t("dashboard.chartSeriesConflicts", "Conflicts")}
                  stroke={C.cyan}
                  strokeWidth={2}
                  fill="url(#gConflict)"
                  dot={false}
                ...
                <Area
                  type="monotone"
                  dataKey="assessments"
                  name={t("dashboard.chartSeriesAssessments", "Assessments")}
                  stroke={C.purple}
                  strokeWidth={2}
                  fill="url(#gAssess)"
                  dot={false}
          ...
          >
            ...
            >
              <p style={{ fontSize: 13, fontWeight: 600, margin: 0 }}>
                {t(
                  "dashboard.severityDistributionTitle",
                  "Relationship Severity Distribution"
                )}
              </p>
              <Tag
                text={t("dashboard.totalPairs", "{count} total").replace(
                  "{count}",
                  String(kpis.total)
                )}
                bg={`${C.orange}1A`}
            ...
            <p style={{ color: C.muted, fontSize: 10, margin: "3px 0 16px" }}>
              {t(
                "dashboard.severityDistributionDesc",
                "Inter-jurisdiction control conflicts by severity level"
              )}
            </p>
            <ResponsiveContainer width="100%" height={170}>
              <BarChart
              ...
              >
                ...
                <Tooltip
                  contentStyle={TOOLTIP_STYLE}
                  labelStyle={{ color: C.muted, fontSize: 10 }}
                  cursor={{ fill: "var(--djac-card)" }}
                />
                <Bar
                  dataKey="count"
                  name={t("dashboard.pairsLabel", "Pairs")}
                  radius={[5, 5, 0, 0]}
                >
                  {severityBars.map(e => (
        ...
        <div
          className="djac-dash-main-grid djac-section-4"
          style={{ display: "grid", gridTemplateColumns: "1fr 290px", gap: 14 }}
        >
          ...
          >
            ...
            >
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, margin: 0 }}>
                  {t(
                    "dashboard.frameworksSectionTitle",
            ...
            {fwLoading ? (
              ...
              >
                {t("dashboard.loadingFrameworks", "Loading frameworks...")}
              </div>
            ) : (
              <div
                className="djac-dash-fw-grid"
                style={{
                  display: "grid",
                  ...
                  return (
                    <div
                      ...
                      onMouseLeave={e => {
                        ...
                        e.currentTarget.style.background = "var(--djac-card)";
                      }}
                      role="button"
                      tabIndex={0}
                      aria-label={t(
                        "dashboard.openFrameworkAnalysis",
                        "Open {code} framework analysis"
                      ).replace("{code}", fw.code)}
                      title={t(
                        "dashboard.openFrameworkAnalysis",
                        "Open {code} framework analysis"
                      ).replace("{code}", fw.code)}
                    ...
                    >
                      ...
                      >
                        ...
                        <div style={{ textAlign: "right" }}>
                          ...
                          <p
                            style={{ color: C.muted, fontSize: 8.5, margin: 0 }}
                          >
                            {t(
                              "dashboard.totalRelPairs",
                              "{count} pairs"
                            ).replace("{count}", String(totalRel))}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                {[
                  {
                    v: kpis.critical,
                    c: C.red,
                    l: t("dashboard.gaugeCriticalShort", "Crit."),
                  },
                  {
                    v: kpis.high,
                    c: C.orange,
                    l: t("common.severity.high", "High"),
                  },
                  {
                    v: kpis.low,
                    c: C.green,
                    l: t("common.severity.low", "Low"),
                  },
                ].map(({ v, c, l }) => (
                  <div key={l}>
                    <p
            ...
            >
              ...
              >
                ...
                <p style={{ fontSize: 12, fontWeight: 600, margin: 0 }}>
                  {t(
                    "dashboard.regulatoryObligations",
                    "Regulatory Obligations"
                  )}
                </p>
                <Tag
                  text={String(timetable?.length ?? 0)}
                  bg={`${C.cyan}18`}
                  border={`${C.cyan}35`}
                  color={C.cyan}
                />
              ...
              >
                {feedItems.map((item, i) => {
                  ...
                  return (
                    <div
                      key={`${item.framework}-${item.requirement}-${i}`}
                      style={{
                        display: "flex",
                        alignItems: "flex-start",
                        gap: 9,
                    ...
                    >
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        >
                          {item.framework} · {item.country} ·{" "}
                          <span style={{ color: col }}>{item.riskLevel}</span>
                        </p>
                      </div>
                    </div>
                  );
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                {[
                  {
                    label: t(
                      "dashboard.quickActionFrameworkAnalysis",
                  ...
                  {
                    label: t(
                      "dashboard.quickActionVendorAssessment",
                      "Vendor Assessment"
                    ),
                  ...
                  {
                    label: t(
                      "dashboard.quickActionObligationTimetable",
                  ...
                  {
                    label: t(
                      "dashboard.quickActionRiskRegister",
                      "Risk Register"
                    ),
                    icon: <ShieldAlert style={{ width: 12, height: 12 }} />,
                    path: "/risk-register",
                    color: C.red,
                  },
                  {
                    label: t(
                      "dashboard.quickActionPolicyManager",
                      "Policy Manager"
                    ),
                  ...
                  {
                    label: t(
                      "dashboard.quickActionAuditSchedule",
                      "Audit Schedule"
                    ),
                    icon: <ClipboardCheck style={{ width: 12, height: 12 }} />,
                    path: "/audit-schedule",
                    color: C.cyan,
                  },
                ].map(a => (
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <span style={{ color: C.red, fontWeight: 700, fontSize: 12 }}>
                {t(
                  "dashboard.criticalConflictDetected",
                  "{count} Critical Conflict{plural} Detected"
                )
                  .replace("{count}", String(kpis.critical))
                  .replace("{plural}", kpis.critical !== 1 ? "s" : "")}{" "}
                -
              </span>{" "}
              <span style={{ color: C.muted, fontSize: 12 }}>
                {t(
                  "dashboard.criticalConflictDesc",
                  "These cross-jurisdiction framework pairs require immediate dual-pipeline remediation."
                )}
              </span>
            </div>
            <button
        ...
        <div
          className="djac-dash-insight-grid djac-section-5"
          style={{
            display: "grid",
            gridTemplateColumns: "repeat(3,1fr)",
            gap: 14,
          ...
          {[
            {
              icon: (
                <AlertTriangle
                  style={{ width: 16, height: 16, color: C.red }}
                />
              ),
              accent: C.red,
              action: {
                label: t(
                  "dashboard.insightLocalizationAction",
                  "View Conflicts"
                ),
                path: "/analysis",
              },
              title: t(
                "dashboard.insightLocalizationTitle",
                "Data Localization Conflict"
              ),
              body: t(
            ...
            },
            {
              ...
              accent: C.yellow,
              action: {
                label: t("dashboard.insightConsentAction", "View Frameworks"),
                path: "/analysis",
              },
              title: t(
                "dashboard.insightConsentTitle",
            ...
            {
              icon: (
                <GitBranch style={{ width: 16, height: 16, color: C.purple }} />
              ),
              accent: C.purple,
              action: {
                label: t(
                  "dashboard.insightEnforcementAction",
                  "Obligation Timetable"
                ),
                path: "/compliance-tracker",
              },
              title: t(
                "dashboard.insightEnforcementTitle",
          ...
          >
            <div
              style={{
                flex: 1,
                height: 1,
                background: `linear-gradient(90deg, ${C.cyan}40, transparent)`,
              }}
            />
            <span
              style={{
            ...
            >
              {t(
                "dashboard.intelligenceCommandCenter",
                "Intelligence Command Center"
              )}
            </span>
            <div
              style={{
                flex: 1,
                height: 1,
                background: `linear-gradient(90deg, transparent, ${C.cyan}40)`,
              }}
            />
          </div>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div
              style={{
                ...
                borderRadius: "50%",
                background: C.green,
                boxShadow: `0 0 6px ${C.green}90`,
              }}
            />
            <span style={{ color: C.muted, fontSize: 10, fontWeight: 600 }}>
              {t(
                "dashboard.platformFooter",
                "DJAC Intelligence Platform - {frameworks} Frameworks - {pairs} Relationship Pairs - Live"
              )
          ...
          <div style={{ display: "flex", gap: 10 }}>
            {[
              {
                l: t("dashboard.footerReports", "Reports"),
                p: "/report-center",
                c: C.cyan,
              },
              {
                l: t("dashboard.footerLawLibrary", "Law Library"),
                p: "/laws",
                c: C.yellow,
              },
            ].map(f => (
