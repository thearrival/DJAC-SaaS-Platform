...
export default function OperationsStatus() {
  ...
  useEffect(() => {
    if (readinessQuery.error)
      sonnerToast.error(
        t("ops.readinessLoadError", "Failed to load readiness status.")
      );
  }, [readinessQuery.error]);
  useEffect(() => {
    if (historyQuery.error)
      sonnerToast.error(
        t("ops.historyLoadError", "Failed to load diagnostics history.")
      );
  }, [historyQuery.error]);
  useEffect(() => {
    if (streamConfigQuery.error)
      sonnerToast.error(
        t("ops.streamConfigLoadError", "Failed to load stream configuration.")
      );
  }, [streamConfigQuery.error]);
  useEffect(() => {
    if (jobsQuery.error)
      sonnerToast.error(
        t("ops.jobsLoadError", "Failed to load assessment jobs.")
      );
  }, [jobsQuery.error]);
  ...
  const requestClearHistory = () => {
    if (!history?.supportsClear) {
      sonnerToast.info(
        t(
          "ops.clearUnavailable",
          "Clear history is unavailable in the current queue mode."
        )
      );
      return;
    }
  ...
  const handleClearHistory = async () => {
    try {
      ...
      sonnerToast.error(
        t("ops.historyClearFailed", "Failed to clear history"),
        {
          description:
            getErrorMessage(error) || t("common.unknownError", "Unknown error"),
        }
      );
    }
  };
    ...
    <div className="djac-page">
          ...
          <CardContent className="flex flex-col gap-3 py-4 text-center sm:flex-row sm:items-center sm:justify-between sm:text-left">
            <p className="text-sm text-muted-foreground">
              {coreLoadErrorMessage ??
                t(
                  "ops.loadError",
                  "Failed to load one or more operations datasets."
                )}
            </p>
            <Button
              variant="outline"
            ...
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                <div className="rounded-md border bg-slate-50 p-3">
                  ...
                  <Badge
                    className={
                      readiness.services.database.ready
                        ? "bg-green-100 dark:bg-green-950/50 text-green-800 dark:text-green-300 mt-2"
                        : "bg-red-100 dark:bg-red-950/50 text-red-800 dark:text-red-300 mt-2"
                    }
                  >
                    {readiness.services.database.ready
                      ? t("ops.ready", "Ready")
                      : t("ops.notReady", "Not Ready")}
                  </Badge>
                ...
                <div className="rounded-md border bg-slate-50 p-3">
                  ...
                  <Badge
                    className={
                      readiness.services.redis.ready
                        ? "bg-green-100 dark:bg-green-950/50 text-green-800 dark:text-green-300 mt-2"
                        : "bg-red-100 dark:bg-red-950/50 text-red-800 dark:text-red-300 mt-2"
                    }
                  >
                  ...
                  <p className="text-slate-600 mt-2">
                    {readiness.services.redis.details}
                  </p>
                </div>
                ...
                <div className="rounded-md border bg-slate-50 p-3">
                  ...
                  <Badge
                    className={
                      readiness.services.aiOrchestrator.ready
                        ? "bg-green-100 dark:bg-green-950/50 text-green-800 dark:text-green-300 mt-2"
                        : "bg-red-100 dark:bg-red-950/50 text-red-800 dark:text-red-300 mt-2"
                    }
                  >
                  ...
                  <p className="text-slate-600 mt-2">
                    {readiness.services.aiOrchestrator.details}
                  </p>
                  <p className="text-slate-600 mt-1">
                    {t("ops.agentSwarm", "Agent Swarm Configured")}:{" "}
                    {readiness.services.aiOrchestrator.agentSwarmConfigured
                      ? t("ops.yes", "Yes")
                      : t("ops.no", "No")}
                  </p>
                  <p className="text-slate-600 mt-1">
                    {t("ops.wsPathLabel", "WebSocket")}:{" "}
                    {readiness.services.aiOrchestrator.websocketPath}
                  </p>
                </div>
              </div>
              <p className="text-xs text-slate-500">
                {t("ops.lastUpdated", "Last updated")}:{" "}
                {formatTime(
                  readiness.timestamp,
                  t("ops.notAvailable", "N/A"),
                  locale
        ...
        <CardHeader className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          ...
          <div className="flex flex-col items-start gap-2 md:items-end">
            <Button
              ...
              disabled={
                !history?.supportsClear || clearHistoryMutation.isPending
              }
            >
            ...
            >
              {clearHistoryMutation.isPending
                ? t(
                    "ops.clearInProgressHint",
                    "Clearing persisted history. Active jobs continue running."
                  )
                : t(
                    "ops.clearHint",
                    "Only saved history is removed. Running jobs are not interrupted."
                  )}
            </p>
          </div>
        </CardHeader>
          ...
          {historyQuery.isError ? (
            <div className="space-y-3">
              <p className="text-sm text-red-700 dark:text-red-400">
                {historyQuery.error?.message ??
                  t(
                    "ops.historyLoadError",
                    "Failed to load diagnostics history."
                  )}
              </p>
              <Button
                variant="outline"
            ...
            <div className="flex items-center gap-2 text-slate-600">
              <LoaderCircle className="h-4 w-4 animate-spin" />
              <span>
                {t("ops.historyLoading", "Loading history diagnostics...")}
              </span>
            </div>
          ) : history ? (
            <div className="space-y-3">
                ...
                <tbody>
                  {jobs.map(job => (
                    <tr key={job.id} className="border-t">
                      ...
                      <td className="px-3 py-2 text-slate-700">
                        {job.result?.inputSummary.source ??
                          t("ops.notAvailable", "N/A")}
                      </td>
                      <td className="px-3 py-2 text-slate-700">
                        {job.result?.inputSummary.vendorId ??
                          t("ops.notAvailable", "N/A")}
                      </td>
                      <td className="px-3 py-2 text-slate-700">
                        {formatTime(
      ...
      <AlertDialog open={isClearDialogOpen} onOpenChange={setIsClearDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("ops.clearConfirmTitle", "Clear persisted history?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "ops.clearConfirm",
