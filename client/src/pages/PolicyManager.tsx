...
export default function PolicyManager() {
  ...
  const listQ = trpc.policyManager.list.useQuery(undefined, {
    staleTime: 30_000,
  });
  ...
  const filtered = useMemo(() => {
    ...
    return policies.filter(p => {
      ...
      if (q) {
        const fields = [p.title, p.policyCode, p.description, p.notes]
          .join(" ")
          .toLowerCase();
  ...
  const stats = useMemo(
    () => ({
      ...
      overdue: policies.filter(
        p => isOverdue(p.nextReviewAt) && p.status !== "retired"
      ).length,
    }),
    [policies]
  );
  ...
  return (
    <div className="djac-page">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">
              {t("policy.title", "Policy Manager")}
            </h1>
            <p className="text-sm text-muted-foreground">
              {t(
                "policy.subtitle",
                "Manage your organisation's compliance policy library"
              )}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <StatCard
          label={t("policy.statActive", "Active")}
          value={stats.active}
        />
        <StatCard label={t("policy.statDraft", "Draft")} value={stats.draft} />
        <StatCard
          label={t("policy.statUnderReview", "Under Review")}
          value={stats.underReview}
        />
        <StatCard
          label={t("policy.statOverdue", "Overdue Review")}
          value={stats.overdue}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="all">
              {t("policy.filterAllTypes", "All types")}
            </SelectItem>
            {POLICY_TYPES.map(pt => (
        ...
        >
          <SelectTrigger className="h-9 w-44">
            <SelectValue
              placeholder={t("policy.filterStatus", "All statuses")}
            />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {t("policy.filterAllStatuses", "All statuses")}
            </SelectItem>
        ...
        <Select value={filterFW} onValueChange={setFilterFW}>
          <SelectTrigger className="h-9 w-44">
            <SelectValue
              placeholder={t("policy.filterFramework", "All frameworks")}
            />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {t("policy.filterAllFrameworks", "All frameworks")}
            </SelectItem>
            {FRAMEWORKS.map(fw => (
      ...
      {listQ.isError ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-16 text-muted-foreground">
            ...
            <p className="font-semibold">
              {listQ.error?.message ??
                t("policy.loadError", "Failed to load policies.")}
            </p>
            <Button
              variant="outline"
              size="sm"
        ...
        <div className="text-center py-16 text-muted-foreground">
          {t("policy.loading", "Loading policies…")}
        </div>
      ) : filtered.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-16 text-muted-foreground">
            <FilePen className="h-10 w-10 opacity-30" />
            <p className="font-semibold">
              {t("policy.emptyTitle", "No policies yet")}
            </p>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {t(
                "policy.formDesc",
                "Fill in the details for this policy entry."
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e =>
                  setForm(f => ({ ...f, policyCode: e.target.value }))
                }
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={v =>
                  setForm(f => ({ ...f, policyType: v as PolicyType }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={form.status}
                onValueChange={v =>
                  setForm(f => ({ ...f, status: v as PolicyStatus }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={v =>
                  setForm(f => ({ ...f, reviewCycleMonths: v }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e =>
                  setForm(f => ({ ...f, nextReviewAt: e.target.value }))
                }
              />
            </div>
            ...
            <div className="sm:col-span-2 space-y-1.5">
              ...
              <Textarea
                ...
                rows={3}
                placeholder={t(
                  "policy.descPlaceholder",
                  "Scope, purpose, and key requirements of this policy…"
                )}
              />
            </div>
            ...
            <div className="sm:col-span-2 space-y-1.5">
              ...
              <Textarea
                ...
                rows={2}
                placeholder={t(
                  "policy.notesPlaceholder",
                  "Implementation notes, exceptions, or review history…"
                )}
              />
            </div>
          </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("policy.deleteTitle", "Delete policy?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("policy.deleteDesc", "This will permanently delete")} &ldquo;
...
function PolicyCard({
  ...
  return (
    <Card
      className={`border-l-4 ${overdue ? "border-l-red-500" : "border-l-primary/30"}`}
    >
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {controlRefs.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1.5">
                {controlRefs.map(ref => (
                  <span
                    key={ref}
                    className="text-xs bg-muted text-muted-foreground border px-1.5 py-0.5 rounded font-mono"
                  >
                    {ref}
                  </span>
                ))}
            ...
            <div className="flex items-center gap-1.5 mt-2">
              ...
              />
              <span
                className={`text-xs ${overdue ? "text-red-500 font-semibold" : "text-muted-foreground"}`}
              >
                {policy.nextReviewAt
