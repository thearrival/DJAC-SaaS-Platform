...
const ACTION_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  user_login: {
    label: "Auth",
    cls: "bg-sky-100 text-sky-800 dark:bg-sky-900/30 dark:text-sky-300 border-sky-300",
  },
  user_logout: {
    label: "Auth",
    cls: "bg-sky-100 text-sky-800 dark:bg-sky-900/30 dark:text-sky-300 border-sky-300",
  ...
  password_reset_requested: {
    label: "Auth",
    cls: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 border-yellow-300",
  },
  ...
  "2fa_enabled": {
    label: "2FA",
    cls: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300 border-emerald-300",
  },
  "2fa_disabled": {
    label: "2FA",
    cls: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 border-red-300",
  },
  // Vendor
  vendor_created: {
    label: "Vendor",
    cls: "bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300 border-violet-300",
  },
  vendor_updated: {
    label: "Vendor",
    cls: "bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300 border-violet-300",
  ...
  vendor_assessed: {
    label: "Assess",
    cls: "bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300 border-indigo-300",
  },
  assessment_report_exported: {
    label: "Export",
    cls: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 border-blue-300",
  },
  ...
  org_created: {
    label: "Org",
    cls: "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300 border-amber-300",
  },
  ...
  user_role_changed: {
    label: "RBAC",
    cls: "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300 border-orange-300",
  },
  user_invited: {
    label: "Invite",
    cls: "bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-300 border-cyan-300",
  },
  user_removed: {
    label: "RBAC",
    cls: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 border-red-300",
  ...
  subscription_created: {
    label: "Billing",
    cls: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border-green-300",
  },
  ...
  trial_started: {
    label: "Billing",
    cls: "bg-teal-100 text-teal-800 dark:bg-teal-900/30 dark:text-teal-300 border-teal-300",
  },
  // Data
  report_downloaded: {
    label: "Data",
...
const CATEGORY_OPTIONS = [
  ...new Set(Object.values(ACTION_BADGE).map(v => v.label)),
].sort();
  ...
  return (
    <div>
      ...
      >
        ...
        {open
          ? "Hide"
          : `Show ${keys.length} field${keys.length > 1 ? "s" : ""}`}
      </button>
      {open && (
...
export default function AuditLog() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter(
      ev => (ACTION_BADGE[ev.action]?.label ?? "Other") === filterCategory
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {t(
                "auditLog.subtitle",
                "Immutable record of platform security and data events."
              )}
            </p>
          </div>
        </div>
      ...
      <Card className="border-border bg-card/60">
        <CardContent className="pt-5 pb-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">
                {t("auditLog.filterUserId", "User ID")}
              </Label>
              <Input
                type="number"
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">
                {t("auditLog.filterLimit", "Max rows")}
              </Label>
              <Input
                type="number"
                className="h-8 text-xs"
          ...
          <>
            ...
            {query.dataUpdatedAt > 0 && (
              <span className="ml-auto text-xs">
                {t("auditLog.lastRefreshed", "Last refreshed:")}{" "}
              ...
              <TableHeader>
                <TableRow className="bg-muted/40 hover:bg-muted/40">
                  <TableHead className="text-xs w-48">
                    {t("auditLog.colTimestamp", "Timestamp")}
                  </TableHead>
                  <TableHead className="text-xs w-20">
                    {t("auditLog.colUserId", "User")}
                  </TableHead>
                  <TableHead className="text-xs w-24">
                    {t("auditLog.colCategory", "Category")}
                  </TableHead>
                  <TableHead className="text-xs">
                    {t("auditLog.colAction", "Action")}
                  </TableHead>
                  <TableHead className="text-xs w-20">
                    {t("auditLog.colTargetId", "Target")}
                  </TableHead>
                  <TableHead className="text-xs">
                    {t("auditLog.colPayload", "Payload")}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
        ...
        <Card className="border-border bg-card/60">
          <CardContent className="pt-12 pb-12 flex flex-col items-center gap-3 text-center">
            ...
            <p className="font-semibold text-foreground">
              {t("auditLog.emptyTitle", "No audit events yet")}
            </p>
            <p className="text-sm text-muted-foreground max-w-sm">
              {t(
                "auditLog.emptyDesc",
                "Audit events are written when users perform vendor assessments, export reports, change roles, manage invites, and other security-relevant actions."
              )}
            </p>
          </CardContent>
        </Card>
