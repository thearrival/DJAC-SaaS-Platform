  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      {isError ? (
        <Alert
          variant="destructive"
          className="border-red-900/60 bg-zinc-950/70 text-red-200"
        >
          ...
          <AlertDescription>
            ...
            <Button
              variant="outline"
              size="sm"
              className="mt-2 border-zinc-700 bg-zinc-900 text-zinc-100 hover:bg-zinc-800"
              onClick={() => refetch()}
            >
              Retry
            </Button>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        <Badge
                          className={`gap-1 text-xs bg-zinc-800 text-zinc-400`}
                        >
                          {cc?.icon}
                          {cc?.label ?? item.category}
                        </Badge>
                        <Badge className={`text-xs ${tc.color}`}>
                          {tc.label}
                        </Badge>
                        {item.cveId && (
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {item.affectedSectors.split(",").map(s => (
                              <Badge
                                key={s}
                                className="text-xs bg-zinc-800 text-zinc-400 capitalize"
                              >
                                {s.trim()}
                              </Badge>
                            ))}
