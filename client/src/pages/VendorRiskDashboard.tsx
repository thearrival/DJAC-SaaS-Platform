...
function KpiCard({
  ...
  return (
    ...
    >
      ...
      <span
        style={{ fontSize: 12, color: "var(--djac-muted)", lineHeight: 1.3 }}
      >
        {label}
      </span>
    </div>
...
export default function VendorRiskDashboard() {
  ...
  const bulkAssessMutation = trpc.vendor.bulkAssess.useMutation({
    ...
    onError: err => {
      ...
      sounds.error();
      toast.error(
        t("vendorRisk.batchErrorCount", "Batch assessment failed") +
          ": " +
          err.message
      );
    },
  ...
  // KPI counts
  const tierCounts = {
    "tier-1-critical": vendors.filter(v => v.riskTier === "tier-1-critical")
      .length,
    "tier-2-high": vendors.filter(v => v.riskTier === "tier-2-high").length,
    "tier-3-moderate": vendors.filter(v => v.riskTier === "tier-3-moderate")
      .length,
  ...
  const displayed = [...vendors]
    ...
    .filter(v => {
      ...
      return j
        .split(",")
        .map(s => s.trim())
        .some(jv => jv === jurisFilter || jv === "Both");
    })
    .sort((a, b) => {
      let cmp = 0;
      if (sort.key === "risk") {
        cmp =
          (RISK_ORDER[b.riskTier ?? ""] ?? 0) -
          (RISK_ORDER[a.riskTier ?? ""] ?? 0);
      } else if (sort.key === "name") {
        cmp = (a.vendorName ?? "").localeCompare(b.vendorName ?? "");
      } else if (sort.key === "criticality") {
        const cLevels: Record<string, number> = {
          "mission-critical": 4,
          high: 3,
          moderate: 2,
          low: 1,
        ...
        cmp =
          (cLevels[b.criticalityLevel ?? ""] ?? 0) -
          (cLevels[a.criticalityLevel ?? ""] ?? 0);
  ...
  return (
    <div className="djac-page">
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 14, color: "var(--djac-muted)" }}>
            {t(
              "vendorRisk.subtitle",
              "Aggregated risk posture across all registered third-party vendors."
            )}
          </p>
        </div>
        <div
      ...
      >
        ...
        >
          <span
            style={{ fontSize: 12, color: "var(--djac-muted)", minWidth: 68 }}
          >
        ...
        >
          <span
            style={{ fontSize: 12, color: "var(--djac-muted)", minWidth: 90 }}
          >
      ...
      >
        {isLoading ? (
          ...
          <div style={{ padding: 64, textAlign: "center" }}>
            ...
            >
              {t(
                "vendorRisk.errorDesc",
                "An error occurred while fetching vendor data. Please try again."
              )}
            </p>
            <button
          ...
          <div style={{ padding: 64, textAlign: "center" }}>
            ...
            >
              {t(
                ...
                "Register your first third-party vendor to begin compliance assessments, gap analysis, and risk scoring across 28 jurisdictions."
              )}
            </p>
            <Link href="/client-workspace">
              <button
            ...
            >
              <thead>
                ...
                >
                  {[
                    {
                      key: "name" as SortKey,
                      label: t("vendorRisk.colName", "Vendor"),
                    },
                    {
                      key: "service" as SortKey,
                      label: t("vendorRisk.colService", "Service"),
                    },
                    {
                      key: "risk" as SortKey,
                      label: t("vendorRisk.colRisk", "Risk Tier"),
                    },
                    {
                      key: "criticality" as SortKey,
                      label: t("vendorRisk.colCriticality", "Criticality"),
                    },
                    null,
                    {
                      key: "date" as SortKey,
                      label: t("vendorRisk.colAdded", "Added"),
                    },
                    null,
                  ].map((col, i) => {
                    if (col === null) {
              ...
              <tbody>
                {displayed.map((vendor, idx) => {
                  const jurisdictions = parseChips(
                    vendor.regulatoryJurisdictions
                  );
                  const critColors = CRIT_COLOR[
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() =>
                deleteTarget &&
                deleteMutation.mutate({ vendorId: deleteTarget.id })
              }
        ...
        <DialogContent style={{ maxWidth: 520 }}>
          ...
          >
            ...
            >
              ...
              >
                {batchSelected.size}/{vendors.length}{" "}
                {t("vendorRisk.vendors", "vendors")}
              </span>
            </div>
            {vendors.map(v => {
              ...
              const rColors =
                RISK_COLOR[v.riskTier ?? ""] ?? RISK_COLOR["tier-4-low"];
              return (
                <label
                  key={v.id}
                  style={{
          ...
          {batchResult && (
            ...
            >
              ...
              {batchResult.errors.map(e => {
                const name =
                  vendors.find(v => v.id === e.vendorId)?.vendorName ??
                  `#${e.vendorId}`;
                return (
                  <div
                    key={e.vendorId}
          ...
          <DialogFooter style={{ marginTop: 16 }}>
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                cursor:
                  batchRunning || batchSelected.size === 0
                    ? "not-allowed"
                    : "pointer",
            ...
            >
              {batchRunning ? (
                <>
                  ...
                  {t("vendorRisk.batchRunning", "Queueing…")}
                </>
              ) : (
                <>
                  <Bot size={14} />{" "}
                  {t("vendorRisk.batchConfirm", "Queue Assessments")}
                </>
              )}
            </button>
          </DialogFooter>
