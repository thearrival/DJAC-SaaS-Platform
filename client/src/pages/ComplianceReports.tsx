      ...
      <CardContent className="px-4 pb-4 space-y-1">
        {items.map(item => (
          <div key={item.label} className="flex justify-between text-xs">
            ...
            <span
              className={`font-semibold ${item.warn && item.value > 0 ? "text-red-600 dark:text-red-400" : ""}`}
            >
              {item.value}
            </span>
          </div>
...
export default function ComplianceReports() {
  ...
  const moduleOptions: {
    ...
    {
      value: "gaps",
      label: t("reports.module.gaps", "Gap Findings"),
      icon: ShieldOff,
    },
    {
      value: "risks",
      label: t("reports.module.risks", "Risk Register"),
      icon: AlertTriangle,
    },
    {
      value: "remediation",
      label: t("reports.module.remediation", "Remediation"),
      icon: Wrench,
    },
    {
      value: "policies",
      label: t("reports.module.policies", "Policies"),
      icon: BookOpen,
    },
    {
      value: "incidents",
      label: t("reports.module.incidents", "Incidents"),
      icon: ShieldAlert,
    },
    {
      value: "audit_schedule",
      label: t("reports.module.audit_schedule", "Audit Schedule"),
      icon: ClipboardCheck,
    },
  ];
    ...
    <div className="djac-page">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold">
            {t("reports.title", "Compliance Reports")}
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {t(
              "reports.subtitle",
              "Org-wide compliance snapshot and module-level data export"
            )}
            {summary?.generatedAt && (
      ...
      {summaryError ? (
        <Card>
          <CardContent className="py-6 text-center">
            <p className="text-sm text-muted-foreground">
              {t(
                "reports.summaryError",
                "Failed to load the compliance report summary."
              )}
            </p>
            <Button
              variant="outline"
        ...
        <p className="text-sm text-muted-foreground py-4 text-center">
          {t("reports.loading", "Generating report…")}
        </p>
      ) : (
        summary && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            <SummaryCard
              icon={Building2}
              label={t("reports.module.vendors", "Vendors")}
              total={summary.vendors.total}
            ...
            <SummaryCard
              icon={ShieldOff}
              label={t("reports.module.gaps", "Gap Findings")}
              total={summary.gaps.total}
              color="bg-orange-100 dark:bg-orange-950 text-orange-600"
              items={[
                {
                  label: t("reports.critical", "Critical"),
                  value: summary.gaps.critical,
                  warn: true,
                },
                {
                  label: t("reports.high", "High"),
                  value: summary.gaps.high,
                  warn: true,
                },
              ]}
            ...
            <SummaryCard
              icon={Wrench}
              label={t("reports.module.remediation", "Remediation")}
              total={summary.remediation.total}
              ...
              items={[
                ...
                {
                  label: t("reports.resolved", "Resolved"),
                  value: summary.remediation.resolved,
                },
              ]}
            ...
            <SummaryCard
              icon={BookOpen}
              label={t("reports.module.policies", "Policies")}
              total={summary.policies.total}
              color="bg-purple-100 dark:bg-purple-950 text-purple-600"
              items={[
                {
                  label: t("reports.active", "Active"),
                  value: summary.policies.active,
                },
                {
                  label: t("reports.draft", "Draft"),
                  value: summary.policies.draft,
                },
              ]}
            ...
            <SummaryCard
              icon={ClipboardCheck}
              label={t("reports.module.audit_schedule", "Audit Schedule")}
              total={summary.auditSchedule.total}
              color="bg-teal-100 dark:bg-teal-950 text-teal-600"
              items={[
                {
                  label: t("reports.upcoming", "Upcoming"),
                  value: summary.auditSchedule.upcoming,
                },
                {
                  label: t("reports.overdue", "Overdue"),
                  value: summary.auditSchedule.overdue,
                  warn: true,
                },
                {
                  label: t("reports.completed", "Completed"),
                  value: summary.auditSchedule.completed,
                },
              ]}
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <CardTitle className="text-base">
              {t("reports.moduleData", "Module Data Export")}
            </CardTitle>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button
                ...
                disabled={rowsLoading || moduleRows.length === 0}
              >
        ...
        <CardContent className="px-0 pb-0">
          {rowsError ? (
            <div className="py-8 text-center">
              <p className="text-sm text-muted-foreground">
                {t("reports.rowsError", "Failed to load module data.")}
              </p>
              <Button
                variant="outline"
                size="sm"
            ...
            <p className="text-sm text-muted-foreground py-8 text-center">
              {t("reports.loading", "Loading data…")}
            </p>
          ) : displayRows.length === 0 ? (
            <p className="text-sm text-muted-foreground py-8 text-center">
              {rowSearch
                ? t("reports.noSearchMatch", "No rows match your search.")
                : t("reports.noData", "No data for this module")}
            </p>
          ) : (
            <div className="overflow-x-auto max-h-96">
                ...
                <thead className="bg-muted/50 sticky top-0">
                  <tr>
                    {Object.keys(displayRows[0]).map(col => (
                      <th
                        key={col}
                        className="text-left font-semibold px-4 py-2 whitespace-nowrap text-muted-foreground"
                      >
                        {col}
                      </th>
                    ))}
              ...
              {displayRows.length > 200 ? (
                <p className="text-xs text-muted-foreground text-center py-3">
                  {t(
                    "reports.truncated",
                    "Showing first 200 rows. Download CSV for full data."
                  )}
                </p>
              ) : (
