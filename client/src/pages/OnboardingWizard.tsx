...
export default function OnboardingWizard() {
  ...
  const deadlinesQuery = trpc.deadlines.list.useQuery(
    {
      jurisdiction:
        selectedJurisdiction === "Other" ? "Both" : selectedJurisdiction,
    },
  ...
  const createOrgMutation = trpc.billing.createOrganization.useMutation({
    ...
    onError: err => {
      toast.error(
        t("wizard.orgCreateError", "Failed to create organization.") +
          " " +
          err.message
      );
    },
  ...
  const assessMutation = trpc.vendor.assess.useMutation({
    ...
    onError: err => {
      toast.error(
        t("wizard.assessError", "Assessment failed.") + " " + err.message
      );
    },
  });
  ...
  const reportMutation = trpc.vendor.report.useMutation({
    ...
    onError: err => {
      toast.error(
        t("wizard.reportError", "Failed to generate report.") +
          " " +
          err.message
      );
    },
  ...
  const hasCoreLoadError =
    subStatusQuery.isError || vendorsQuery.isError || deadlinesQuery.isError;
  const coreLoadErrorMessage =
    subStatusQuery.error?.message ??
    vendorsQuery.error?.message ??
    deadlinesQuery.error?.message;
  ...
  const downloadReport = () => {
    ...
    const blob = new Blob([reportResult.content], {
  ...
  return (
    <div className="djac-page">
      <Card className="border-primary/30 bg-gradient-to-r from-blue-50/50 via-background to-cyan-50/50 dark:from-blue-950/20 dark:to-cyan-950/20">
        <CardHeader>
          ...
          <CardDescription>
            {t(
              "wizard.subtitle",
              "Welcome aboard! 🚀 In 5 minutes, you'll set up your organization, pick your compliance jurisdictions, assess a vendor, and generate your first AI-powered compliance report. Let's get started!"
            )}
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="mb-4">
            ...
            <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
              <div
                ...
                style={{
                  width: `${((activeStep + 1) / stepTitles.length) * 100}%`,
                }}
              />
            </div>
          </div>
            ...
            {stepTitles.map((title, index) => {
              ...
              return (
                <button
                  ...
                  aria-label={t(
                    "wizard.stepButtonLabel",
                    "Step {current} of {total}: {title}. {status}"
                  )
      ...
      {hasCoreLoadError && (
        <QueryErrorPanel
          message={
            coreLoadErrorMessage ??
            t("wizard.loadError", "Failed to load onboarding data.")
          }
          onRetry={() => {
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              1. {t("wizard.step1", "Organization Setup")}
            </CardTitle>
            <CardDescription>
              {hasOrganization
                ...
                : t(
                    "wizard.step1_desc",
                    "Create your organization and start trial setup."
                  )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {hasOrganization ? (
              <div className="rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm">
                {t("wizard.orgReady", "Organization connected")}:{" "}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              2. {t("wizard.step2", "Jurisdiction Focus")}
            </CardTitle>
            <CardDescription>
              {t(
                "wizard.step2_desc",
                "Pick your primary compliance jurisdiction to prioritize deadlines."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="max-w-sm space-y-1">
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={t(
                      "wizard.selectJurisdictionPlaceholder",
            ...
            <div className="rounded-lg border p-3">
              <p className="mb-2 text-sm font-medium">
                {t("wizard.deadlinePreview", "Deadline Preview")}
              </p>
              {deadlinesQuery.isLoading ? (
                <p
                  ...
                  aria-live="polite"
                >
                  {t("wizard.loadingDeadlines", "Loading...")}
                </p>
              ) : deadlinesQuery.isError ? (
                <QueryErrorPanel
                  message={
                    deadlinesQuery.error?.message ??
                    t("wizard.deadlinesLoadError", "Failed to load deadlines.")
                  }
                  onRetry={() => {
                    void deadlinesQuery.refetch();
                  }}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t(
                "wizard.step3_desc",
                "Choose an existing vendor profile or create one first."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {vendorsQuery.isLoading ? (
              <p
                ...
                aria-live="polite"
              >
                {t("wizard.loadingVendors", "Loading vendors...")}
              </p>
            ) : vendorsQuery.isError ? (
              <QueryErrorPanel
                message={
                  vendorsQuery.error?.message ??
                  t("wizard.vendorsLoadError", "Failed to load vendors.")
                }
                onRetry={() => {
                  void vendorsQuery.refetch();
                }}
              ...
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  {t("wizard.noVendors", "No vendor profiles found yet.")}
                </p>
                <Button
                  variant="outline"
                  onClick={() => navigate("/vendor-assessment")}
                >
                  {t(
                    "wizard.createVendor",
                    "Go to Vendor Assessment to create one"
                  )}
                </Button>
              </div>
            ) : (
              <>
                <div className="max-w-md space-y-1">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t(
                          "wizard.selectVendorPlaceholder",
                          "Choose a vendor"
                        )}
                      />
                    </SelectTrigger>
                ...
                {selectedVendor ? (
                  <div className="rounded-md border border-primary/30 bg-primary/5 p-3 text-sm">
                    {t("wizard.selectedVendor", "Selected")}:{" "}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t(
                "wizard.step4_desc",
                "Run AI compliance assessment for selected vendor."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">
              {t("wizard.vendor", "Vendor")}:{" "}
              <strong>{selectedVendor?.vendorName ?? "—"}</strong>
            </p>

            <Button
              onClick={handleRunAssessment}
              disabled={!selectedVendorId || assessMutation.isPending}
            >
                  ...
                  <strong>
                    {String(
                      assessmentResult.riskLevel ??
                        t("wizard.unknown", "unknown")
                    )}
                  </strong>
                </p>
                <p>
                  {t("wizard.score", "Score")}:{" "}
                  <strong>{Number(assessmentResult.overallScore ?? 0)}%</strong>
                </p>
                <p>
                  {t("wizard.gaps", "Gaps")}:{" "}
                  <strong>{assessmentResult.gaps?.length ?? 0}</strong>
                </p>
              </div>
            ) : null}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t(
                "wizard.step5_desc",
                "Export a first compliance report for stakeholders."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <Button
              ...
              disabled={!selectedVendorId || reportMutation.isPending}
            >
            ...
            {reportResult ? (
              <div className="space-y-2 rounded-md border border-emerald-500/40 bg-emerald-500/10 p-3">
                <p className="text-sm">
                  {t("wizard.reportReady", "Report ready")}:{" "}
                  <strong>{reportResult.fileName}</strong>
                </p>
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" onClick={downloadReport}>
                    {t("wizard.download", "Download")}
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
