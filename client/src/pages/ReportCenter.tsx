...
function deriveReportInsights(markdown: string) {
  ...
  const subSectionCount = lines.filter(line =>
    line.trim().startsWith("### ")
  ).length;
  const tableRowCount = lines.filter(
    line => line.trim().startsWith("|") && !/^\|[-\s|]+\|?$/.test(line.trim())
  ).length;
...
export default function ReportCenter() {
  ...
  const generateMutation = trpc.compliance.report.useMutation({
    ...
    onError(err) {
      toast.error(
        t("reportCenter.toastGenerationFailed", "Generation failed"),
  ...
  const downloadDocxMutation = trpc.compliance.reportDocx.useMutation({
    onSuccess(data) {
      ...
      toast.success(
        t(
          "reportCenter.toastDocxDownloaded",
          "Official Word template downloaded"
        ),
        {
          description: `${t("reportCenter.toastReportId", "Report ID")}: ${data.reportId}`,
        }
    ...
    onError(err) {
      toast.error(t("reportCenter.toastDocxFailed", "Word export failed"), {
        description: err.message,
      });
    },
  });
  ...
  const downloadPdfMutation = trpc.compliance.reportPdf.useMutation({
    ...
    onError(err) {
      toast.error(t("reportCenter.toastPdfFailed", "PDF export failed"), {
        description: err.message,
      });
    },
  });
  ...
  const emailReportMutation = trpc.compliance.emailReport.useMutation({
    ...
    onError(err) {
      toast.error(t("reportCenter.toastEmailFailed", "Report email failed"), {
        description: err.message,
      });
    },
  });
  ...
  function handleDownloadMd() {
    ...
    URL.revokeObjectURL(url);
    toast.success(
      t("reportCenter.toastDownloaded", "Markdown file downloaded")
    );
  }
  ...
  async function handleCopyMd() {
    ...
    try {
      ...
      toast.success(
        t("reportCenter.toastCopied", "Markdown copied to clipboard")
      );
    } catch {
  ...
  const createShareMutation = trpc.compliance.createShareLink.useMutation({
    onSuccess(data) {
      ...
      toast.success(
        t("reportCenter.toastShareCopied", "Share link copied to clipboard"),
        {
          description: t(
            "reportCenter.toastShareExpiry",
    ...
    onError(err) {
      toast.error(
        t("reportCenter.toastShareFailed", "Failed to create share link"),
        { description: err.message }
      );
    },
  });
  ...
  useEffect(() => {
    if (reportByTokenQuery.data) {
      ...
      toast.success(
        t("reportCenter.toastSharedReport", "Shared report loaded")
      );
    }
  ...
  useEffect(() => {
    if (reportByTokenQuery.error) {
      toast.error(
        t("reportCenter.toastShareInvalid", "Share link is expired or invalid")
      );
    ...
    <div className="djac-page">
      <Dialog open={isEmailDialogOpen} onOpenChange={setIsEmailDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t("reportCenter.emailDialogTitle", "Email PDF report")}
            </DialogTitle>
            <DialogDescription>
              {t(
                "reportCenter.emailDialogDescription",
                "Send the official template-backed Word report. A rendered PDF copy is also attached for quick sharing."
              )}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-2">
              <label
                ...
                htmlFor="report-email-recipient"
              >
      ...
      <div className="mx-auto max-w-7xl px-6 py-6">
        ...
        <div className="no-print mb-6 grid gap-4 lg:grid-cols-3 djac-section-2">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">
                  {t(
                    "reportCenter.professionalPackTitle",
                    "Government-ready output"
                  )}
                </p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {t(
                    "reportCenter.professionalPackDesc",
                    "Official Word-template export, audit identifiers, and structured Markdown for compliance records."
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-border/50 bg-gradient-to-br from-card via-card to-sky-500/5 dark:to-sky-500/10 p-5 shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">
                  {t(
                    "reportCenter.localeReadyTitle",
                    "Cross-border presentation"
                  )}
                </p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {t(
                    "reportCenter.localeReadyDesc",
                    "Generate in English, Arabic, or Chinese with localized report content and template-aware PDF rendering."
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-border/50 bg-gradient-to-br from-card via-card to-amber-500/5 dark:to-amber-500/10 p-5 shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">
                  {t("reportCenter.deliveryTitle", "Delivery workflow")}
                </p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {t(
                    "reportCenter.deliveryDesc",
                    "Review online, download the official Word template output, and send the report package directly by email from the reporting workspace."
                  )}
                </p>
              </div>
            </div>
        ...
        <div className="no-print mb-6 grid gap-4 rounded-xl border border-border/40 bg-card/40 p-5 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              <SelectTrigger className="h-9">
                <SelectValue
                  placeholder={t(
                    "reportCenter.selectJurisdictionPlaceholder",
                    "Select jurisdiction"
                  )}
                />
              </SelectTrigger>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              <SelectTrigger className="h-9">
                <SelectValue
                  placeholder={t(
                    "reportCenter.selectLanguagePlaceholder",
                    "Select language"
                  )}
                />
              </SelectTrigger>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              <SelectTrigger className="h-9">
                <SelectValue
                  placeholder={t(
                    "reportCenter.selectReportTypePlaceholder",
                    "Select type"
                  )}
                />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="full_compliance">
                  {t("reportCenter.typeFullCompliance", "Full Compliance")}
                </SelectItem>
                <SelectItem value="gap_analysis">
                  {t("reportCenter.typeGapAnalysis", "Gap Analysis")}
                </SelectItem>
                <SelectItem value="vendor_assessment">
                  {t("reportCenter.typeVendorAssessment", "Vendor Assessment")}
                </SelectItem>
                <SelectItem value="risk_assessment">
                  {t("reportCenter.typeRiskAssessment", "Risk Assessment")}
                </SelectItem>
                <SelectItem value="executive_summary">
                  {t("reportCenter.typeExecutiveSummary", "Executive Summary")}
                </SelectItem>
                <SelectItem value="regulatory_deadline">
                  {t(
                    "reportCenter.typeRegulatoryDeadline",
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2 pt-1.5">
              <Badge variant="secondary" className="text-xs">
                {t("reportCenter.formatDocx", "Official DOCX")}
              </Badge>
              <Badge variant="secondary" className="text-xs">
                {t("reportCenter.formatPdf", "PDF")}
              </Badge>
              <Badge variant="secondary" className="text-xs">
                {t("reportCenter.formatMarkdown", "Markdown")}
              </Badge>
            </div>
          </div>
          ...
          <div className="flex flex-col justify-end gap-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              {t(
                "reportCenter.generateShortcutHint",
                "Press Ctrl+Enter to generate from anywhere on this page."
              )}
            </p>
          </div>
        </div>
        ...
        {isLoading && (
          <div
            className="no-print flex items-center justify-center gap-3 rounded-xl border border-border/40 bg-card/30 py-24"
            role="status"
            aria-live="polite"
          >
        ...
        {result && (
          <div>
              ...
              <div className="no-print mb-5 rounded-2xl border border-border/50 bg-gradient-to-br from-card via-card/80 to-primary/5 p-5 shadow-sm">
                <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-foreground">
                      {t("reportCenter.scorecardTitle", "Compliance Scorecard")}
                    </p>
                    <Badge variant="outline" className="font-mono text-xs">
                      {t("reportCenter.scorecardVersion", "v")}{" "}
                      {result.scorecardSummary.reportVersion}
                    </Badge>
                  </div>
                  <Badge variant="secondary" className="text-xs">
                    {result.templateName}
                ...
                <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
                  ...
                  <div className="sm:col-span-1 lg:col-span-2 flex flex-col items-center justify-center rounded-xl border border-primary/20 bg-primary/5 p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground mb-1">
                      {t("reportCenter.scorecardOverall", "Overall Score")}
                    </p>
                    <p
                      className={`text-4xl font-bold ${result.scorecardSummary.overallScore >= 80 ? "text-emerald-600 dark:text-emerald-400" : result.scorecardSummary.overallScore >= 65 ? "text-amber-600 dark:text-amber-400" : "text-rose-600 dark:text-rose-400"}`}
                    >
                    ...
                    <div className="mt-2 h-1.5 w-full rounded-full bg-border/50">
                      <div
                        ...
                        style={{
                          width: `${result.scorecardSummary.overallScore}%`,
                        }}
                      />
                    </div>
                  </div>
                  ...
                  {result.scorecardSummary.saudiScore !== null && (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-border/40 bg-card/60 p-3">
                      <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                        {t("reportCenter.scorecardSaudi", "Saudi Arabia")}
                      </p>
                      <p className="mt-1 text-2xl font-bold text-foreground">
                        {result.scorecardSummary.saudiScore}
                        <span className="text-sm text-muted-foreground">
                          /100
                        </span>
                      </p>
                  ...
                  {result.scorecardSummary.chinaScore !== null && (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-border/40 bg-card/60 p-3">
                      <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                        {t("reportCenter.scorecardChina", "China")}
                      </p>
                      <p className="mt-1 text-2xl font-bold text-foreground">
                        {result.scorecardSummary.chinaScore}
                        <span className="text-sm text-muted-foreground">
                          /100
                        </span>
                      </p>
                  ...
                  <div className="flex flex-col items-center justify-center rounded-xl border border-rose-200/60 dark:border-rose-800/40 bg-rose-50/40 dark:bg-rose-950/20 p-3">
                    <div className="flex items-center gap-1">
                      ...
                      <p className="text-xs uppercase tracking-[0.14em] text-rose-700 dark:text-rose-400">
                        {t("reportCenter.scorecardGaps", "Gaps")}
                      </p>
                    </div>
                    <p className="mt-1 text-2xl font-bold text-rose-700 dark:text-rose-400">
                      {result.scorecardSummary.gapCount}
                    </p>
                  </div>
                  {/* Critical findings */}
                  <div className="flex flex-col items-center justify-center rounded-xl border border-orange-200/60 dark:border-orange-800/40 bg-orange-50/40 dark:bg-orange-950/20 p-3">
                    <p className="text-xs uppercase tracking-[0.14em] text-orange-700 dark:text-orange-400">
                      {t("reportCenter.scorecardCritical", "Critical")}
                    </p>
                    <p className="mt-1 text-2xl font-bold text-orange-700 dark:text-orange-400">
                      {result.scorecardSummary.criticalFindings}
                    </p>
                  </div>
                  {/* Frameworks */}
                  <div className="flex flex-col items-center justify-center rounded-xl border border-border/40 bg-card/60 p-3">
                    <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                      {t("reportCenter.scorecardFrameworks", "Frameworks")}
                    </p>
                    <p className="mt-1 text-2xl font-bold text-foreground">
                      {result.scorecardSummary.frameworksCovered}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="no-print mb-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                      {t("reportCenter.metricSections", "Sections")}
                    </p>
                    <p className="text-2xl font-semibold text-foreground">
                      {reportInsights?.sectionCount ?? 0}
                    </p>
                  </div>
                </div>
              </div>
              <div className="rounded-2xl border border-border/50 bg-card/70 p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                      {t("reportCenter.metricDetailBlocks", "Detail blocks")}
                    </p>
                    <p className="text-2xl font-semibold text-foreground">
                      {(reportInsights?.subSectionCount ?? 0) +
                        (reportInsights?.tableRowCount ?? 0)}
                    </p>
                  </div>
                </div>
              </div>
              <div className="rounded-2xl border border-border/50 bg-card/70 p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                      {t("reportCenter.metricDelivery", "Delivery")}
                    </p>
                    <p className="text-sm font-semibold text-foreground">
                      {t(
                        "reportCenter.metricDeliveryValue",
                        "DOCX, PDF, Markdown, Email"
                      )}
                    </p>
                  </div>
                </div>
              ...
              <div className="rounded-2xl border border-border/50 bg-card/70 p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                      {t("reportCenter.metricStatus", "Readiness")}
                    </p>
                    <p className="text-sm font-semibold text-foreground">
                      {t(
                        "reportCenter.metricStatusValue",
            ...
            <div className="no-print mb-5 rounded-2xl border border-border/50 bg-gradient-to-r from-card via-card to-rose-500/5 dark:to-rose-500/10 p-5 shadow-sm">
              <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <p className="text-sm font-semibold text-foreground">
                    {t(
                      "reportCenter.executiveSummaryTitle",
                      "Executive delivery summary"
                    )}
                  </p>
                  <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
                    {t(
                      "reportCenter.executiveSummaryDesc",
                      "This package combines narrative analysis, structured comparison tables, and remediation priorities in a board-ready format aligned to the official reporting template."
                    )}
                  </p>
                </div>
            ...
            <p className="no-print mb-4 text-xs text-muted-foreground">
              {t(
                "reportCenter.actionsHint",
                "Download the official Word-template report, send the report package directly from this workspace, or keep the Markdown source for version control."
              )}
            </p>
              ...
              <div className="not-prose mb-8 rounded-2xl border border-border/60 bg-gradient-to-r from-card via-card to-sky-500/5 dark:to-sky-500/10 p-5 shadow-sm">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="space-y-1 text-sm text-muted-foreground sm:text-right">
                    <div>
                      {t("reportCenter.toastReportId", "Report ID")}:{" "}
                      {result.reportId}
                    </div>
