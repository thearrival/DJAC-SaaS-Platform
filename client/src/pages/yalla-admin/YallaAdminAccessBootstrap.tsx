...
export default function YallaAdminAccessBootstrap() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/yalla-admin/bootstrap?${params.toString()}`, {
      ...
      .catch((cause: unknown) => {
        setError(
          cause instanceof Error ? cause.message : "Access bootstrap failed."
        );
        setWorking(false);
      });
  }, [location, navigate]);
    ...
    <div className="min-h-screen bg-[#0a0f1e] flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-slate-800/60 bg-slate-900/85 p-8 text-center backdrop-blur-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          {working
            ? "Validating the private owner link and preparing the isolated console."
            : "The owner entry link could not be validated."}
        </p>
