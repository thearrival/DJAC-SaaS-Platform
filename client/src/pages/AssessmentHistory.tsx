...
> = {
  ...
  failed: {
    ...
    icon: ({ size = 13 }) => (
      <AlertCircle size={size} style={{ color: "#ef4444" }} />
    ),
  },
  running: {
    label: "Running",
...
function JobCard({
  ...
  const _criticalCount =
    assessment?.gaps.filter(g => g.severity === "critical").length ?? 0;
  const _highCount =
    assessment?.gaps.filter(g => g.severity === "high").length ?? 0;
        ...
        {assessment && assessment.gaps.length > 0 && (
          ...
          >
            ...
            {(["critical", "high", "medium", "low"] as const).map(sev => {
              const count = assessment.gaps.filter(
                g => g.severity === sev
              ).length;
              ...
              return (
                ...
                >
                  <SevIcon size={11} />
                  <span style={{ color: RISK_COLOR[sev], fontWeight: 600 }}>
                    {count}
                  </span>
                  <span
          ...
          <Collapsible open={expanded} onOpenChange={setExpanded}>
            <CollapsibleTrigger asChild>
              ...
              >
                ...
                {expanded ? "Hide" : "Show"} pipeline events (
                {job.events.length})
              </button>
            </CollapsibleTrigger>
            <CollapsibleContent>
              <div
                style={{
                  ...
                  border:
                    "1px solid var(--djac-border, rgba(148,163,184,0.10))",
                  maxHeight: 320,
                  overflowY: "auto",
                }}
              >
...
export default function AssessmentHistory() {
  ...
  const {
    ...
    error,
    refetch,
  } = trpc.ai.listAssessmentJobs.useQuery(
    { limit: 100 },
    { staleTime: 30_000 }
  );
  ...
  const clearMutation = trpc.ai.clearHistory.useMutation({
    onSuccess: () => {
      toast.success(
        t("assessmentHistory.cleared", "Assessment history cleared.")
      );
      void refetch();
    },
    onError: err => {
      toast.error(
        t("assessmentHistory.clearError", "Failed to clear history") +
          ": " +
          err.message
      );
    },
  ...
  useEffect(() => {
    if (error) {
      toast.error(
        t("assessmentHistory.loadError", "Failed to load assessment history") +
          ": " +
          error.message
      );
    }
      ...
      {!isLoading && !error && (
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <StatTile
            ...
            color="#ef4444"
            icon={({ size, style }) => (
              <AlertCircle size={size} style={style} />
            )}
          />
          <StatTile
        ...
        >
          ...
          <div style={{ textAlign: "center" }}>
            ...
            >
              {filter === "all"
                ? t(
                    "assessmentHistory.noJobsDesc",
                    "Run a vendor assessment from the Vendor Risk Dashboard to populate this history."
                  )
                : t(
                    "assessmentHistory.noJobsFilterDesc",
      ...
      <Dialog open={confirmClear} onOpenChange={setConfirmClear}>
        <DialogContent style={{ maxWidth: 420 }}>
          <DialogHeader>
            <DialogTitle>
              {t(
                "assessmentHistory.clearConfirmTitle",
