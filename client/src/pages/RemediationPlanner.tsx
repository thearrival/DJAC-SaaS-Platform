...
function SeverityIcon({
  ...
  if (severity === "critical")
    return <ShieldX size={size} style={{ color: col }} />;
  if (severity === "high")
    return <ShieldAlert size={size} style={{ color: col }} />;
  if (severity === "medium")
...
function TaskCard({
  ...
  const vendorName =
    task.vendorId != null ? vendorMap.get(task.vendorId) : undefined;
  const assigneeName =
    ...
    >
      <DialogContent style={{ maxWidth: 560 }}>
        <DialogHeader>
          <DialogTitle>
            {editMode
              ? t("remediation.editTitle", "Edit Task")
              : t("remediation.createTitle", "Create Remediation Task")}
          </DialogTitle>
          <DialogDescription>
            {t(
              "remediation.formDesc",
              "Track a compliance gap remediation effort with assignee, due date, and severity."
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <div>
            <Label htmlFor="rm-title">
              {t("remediation.labelTitle", "Title")} *
            </Label>
            <Input
              id="rm-title"
              value={f.title}
              ...
              placeholder={t(
                "remediation.titlePlaceholder",
                "e.g. Implement Data Breach Notification Procedure"
              )}
              maxLength={255}
            />
          </div>
          ...
          <div>
            <Label htmlFor="rm-desc">
              {t("remediation.labelDescription", "Description")}
            </Label>
            <Textarea
              id="rm-desc"
              value={f.description}
              ...
              placeholder={t(
                "remediation.descPlaceholder",
                "Optional background on the gap and what needs to change."
              )}
              rows={2}
              maxLength={2000}
            />
          ...
          >
            <div>
              ...
              <Select
                value={f.severity}
                onValueChange={v => setF("severity", v)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {(
                    ["critical", "high", "medium", "low"] as TaskSeverity[]
                  ).map(s => (
                    <SelectItem
                      key={s}
                      value={s}
            ...
            <div>
              ...
              <Select
                value={f.status}
                onValueChange={v => setF("status", v as TaskStatus)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="open">
                    {t("remediation.statusOpen", "Open")}
                  </SelectItem>
                  <SelectItem value="in_progress">
                    {t("remediation.statusInProgress", "In Progress")}
                  </SelectItem>
                  <SelectItem value="resolved">
                    {t("remediation.statusResolved", "Resolved")}
                  </SelectItem>
                  <SelectItem value="accepted_risk">
                    {t("remediation.statusAcceptedRisk", "Accepted Risk")}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          >
            <div>
              <Label htmlFor="rm-gapcode">
                {t("remediation.labelGapCode", "Gap Code")}
              </Label>
              <Input
                id="rm-gapcode"
                value={f.gapCode}
            ...
            <div>
              ...
              <Select
                value={f.vendorId || "__none"}
                onValueChange={v => setF("vendorId", v === "__none" ? "" : v)}
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={t(
                      "remediation.selectVendor",
                      "Select vendor…"
                    )}
                  />
                </SelectTrigger>
          ...
          >
            <div>
              ...
              <Select
                value={f.assignedToUserId || "__none"}
                onValueChange={v =>
                  setF("assignedToUserId", v === "__none" ? "" : v)
                }
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={t("remediation.selectAssignee", "Assign to…")}
                  />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none">
                    {t("remediation.unassigned", "Unassigned")}
                  </SelectItem>
                  {members.map(m => (
            ...
            <div>
              <Label htmlFor="rm-due">
                {t("remediation.labelDueDate", "Due Date")}
              </Label>
              <Input
                id="rm-due"
                type="date"
          ...
          <div>
            <Label htmlFor="rm-notes">
              {t("remediation.labelNotes", "Notes")}
            </Label>
            <Textarea
              id="rm-notes"
              value={f.notes}
              ...
              placeholder={t(
                "remediation.notesPlaceholder",
                "Additional context, links, or evidence…"
              )}
              rows={2}
              maxLength={2000}
            />
...
export default function RemediationPlanner() {
  ...
  const getEditInitial = (task: RemediationTask): TaskFormValues => ({
    ...
    assignedToUserId:
      task.assignedToUserId != null ? String(task.assignedToUserId) : "",
  ...
  const {
    ...
    isError,
    refetch,
  } = trpc.remediation.list.useQuery(undefined, { staleTime: 30_000 });
  const vendorsQuery = trpc.vendor.list.useQuery(undefined, {
    retry: false,
    refetchOnWindowFocus: false,
  });
  const membersQuery = trpc.orgMembers.list.useQuery(undefined, {
    retry: false,
    refetchOnWindowFocus: false,
  });
  ...
  const supportLoadErrorMessage =
    vendorsQuery.error?.message ?? membersQuery.error?.message;
  ...
  const removeMutation = trpc.remediation.remove.useMutation({
    ...
    onError: err => {
      toast.error(
        t("remediation.deleteError", "Failed to delete task") +
          ": " +
          err.message
      );
      sounds.error();
    ...
    <div className="djac-page">
        ...
        <div>
          ...
          >
            {t(
              ...
              "Convert compliance gap findings into actionable tasks. Assign, track, and close remediation efforts across your vendor portfolio."
            )}
          </p>
        </div>
        <Button
            ...
            <p style={{ margin: "3px 0 0", fontSize: 12, color: "#94a3b8" }}>
              {supportLoadErrorMessage ??
                t(
                  ...
                  "Retry to reload vendors and team members."
                )}
            </p>
          </div>
          <Button
      ...
      {!isLoading && !isError && (
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          />
          <StatCard
            label={t("remediation.colOpen", "Open")}
            count={stats.open}
            color="#94a3b8"
          ...
          />
          <StatCard
            label={t("remediation.colAcceptedRisk", "Accepted Risk")}
      ...
      {!isLoading && !isError && tasks.length > 0 && (
        ...
        >
          {KANBAN_COLUMNS.map(col => {
            const colTasks = (tasks as RemediationTask[]).filter(
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("remediation.deleteConfirm", "Delete this task?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              ...
              {deleteTask && (
                <>
                  {" "}
                  &ldquo;<strong>{deleteTask.title}</strong>&rdquo;
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
