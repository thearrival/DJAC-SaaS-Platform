...
export default function ComplianceHeatmap() {
  ...
  const { t } = useLocale();
  const [activeFilter, setActiveFilter] = useState<RelationshipType | "all">(
    "all"
  );
  ...
  const {
    ...
    error,
    refetch,
  } = trpc.compliance.matrix.useQuery();
  ...
  const criticalCount = allCells.filter(
    c => c.maxSeverity === "critical"
  ).length;
  ...
  const _lowCount = allCells.filter(
    c => c.maxSeverity === "low" || c.maxSeverity === "none"
  ).length;
            ...
            >
              <p>
                {error.message ||
                  t(
                    "heatmap.error",
                    "Failed to load compliance matrix. Please refresh."
                  )}
              </p>
              <Button
                size="sm"
      ...
      <div className="rounded-xl border border-border/50 bg-card p-4">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(
                ["critical", "high", "medium", "low", "none"] as Severity[]
              ).map(s => (
