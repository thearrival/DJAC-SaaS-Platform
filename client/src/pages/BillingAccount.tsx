...
function OrgSetupForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        {t(
          "billing.setupSubtitle",
          "Create your organization to start your 7-day free trial. No credit card required."
        )}
      </p>
      <p style={{ fontSize: 12, color: "var(--djac-muted)", marginBottom: 16 }}>
        {t(
          "billing.requiredHint",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div>
          ...
          <input
            ...
            placeholder={t(
              "billing.billingEmailPlaceholder",
              "billing@example.com"
            )}
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={value =>
              setForm(p => ({
                ...p,
                primaryJurisdiction: value as typeof form.primaryJurisdiction,
              }))
            }
          >
            <SelectTrigger
              aria-label={t(
                "billing.primaryJurisdiction",
            ...
            >
              ...
              <SelectItem value="China">
                {t("billing.jurisdictionChina", "China Only")}
              </SelectItem>
        ...
        {createOrg.isPending ? (
          <div
            role="status"
            aria-live="polite"
            style={{ fontSize: 12, color: "var(--djac-muted)" }}
          >
            {t(
              "billing.setupSubmittingHint",
              "Creating your organization and starting the trial..."
            )}
          </div>
        ) : null}
...
export default function BillingAccount() {
  ...
  const statusQuery = trpc.billing.getSubscriptionStatus.useQuery(undefined, {
    enabled: isLoggedIn,
  });
  ...
  const isTrialActive =
    status?.plan === "free_trial" && (status?.trialDaysRemaining ?? 0) > 0;
  const trialDays = status?.trialDaysRemaining ?? 0;
  const isBillingBusy =
    statusQuery.isLoading || historyQuery.isLoading || portalMutation.isPending;

  if (!isLoggedIn) {
    return (
      <div style={{ padding: 48, textAlign: "center" }}>
        <p style={{ color: "var(--djac-muted)", marginBottom: 16 }}>
          {t("billing.signInPrompt", "Please sign in to view billing.")}
        </p>
        <Link href="/signup">
          <button
            style={{
      ...
      <div className="djac-page">
        ...
        >
          ...
          >
            {t(
              "billing.retryHint",
              "Retry to refresh your billing and subscription details."
            )}
          </p>
          <Button
            variant="outline"
    ...
    <div className="djac-page">
            ...
            {isTrialActive ? (
              <div style={{ fontSize: 13, color: planMeta.color }}>
                ...
                {t(
                  ...
                  "{days} day(s) remaining in trial"
                ).replace("{days}", String(trialDays))}
                {status?.trialEndsAt &&
                  ` · ${t("billing.expiresOn", "Expires")} ${formatDate(status.trialEndsAt, locale)}`}
              </div>
              ...
              <div style={{ fontSize: 13, color: "var(--djac-muted)" }}>
                ...
                {formatDate(status.currentPeriodEnd, locale)}
        ...
        >
          ...
          <div>
            <strong style={{ color: "#f59e0b" }}>
              {t("billing.trialExpiringSoon", "Trial expiring soon")}
            </strong>
        ...
        >
          ...
          >
            ...
            <h2 style={{ fontSize: 16, fontWeight: 600 }}>
              {t("billing.historyTitle", "Billing History")}
            </h2>
            <button
              onClick={() => historyQuery.refetch()}
              aria-label={t(
                "billing.refreshHistory",
          ...
          {historyQuery.isError ? (
            ...
            >
              ...
              {t(
                "billing.noEvents",
                "No billing events yet. Events will appear here once you subscribe."
              )}
            </div>
          ) : (
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div
                          style={{ fontSize: 11, color: "var(--djac-muted)" }}
                        >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <Link
            href="/pricing"
            style={{ color: "var(--djac-muted)", textDecoration: "none" }}
          >
            {t("billing.viewPlans", "View Plans")}
          </Link>
          <a
            ...
            style={{ color: "var(--djac-muted)", textDecoration: "none" }}
          >
            {t("billing.stripeTerms", "Stripe Terms")}{" "}
