...
function TierBadge({ tier }: { tier: string }) {
  ...
  return (
    <Badge
      variant="outline"
      className={`font-semibold uppercase text-[10px] ${TIER_BG[t]}`}
    >
      {tier}
    </Badge>
  );
...
function ScoreBar({ value, max = 100 }: { value: number; max?: number }) {
  ...
  const tier: Tier =
    pct >= 80 ? "critical" : pct >= 60 ? "high" : pct >= 40 ? "medium" : "low";
  return (
...
export default function ContinuousCompliance() {
  ...
  // ── tRPC queries ──────────────────────────────────────────────────────────
  const summaryQ = trpc.ctem.getRiskSummary.useQuery(undefined, {
    staleTime: 30_000,
  });
  const runsQ = trpc.ctem.listRuns.useQuery(
    { limit: 10 },
    { staleTime: 10_000 }
  );
  ...
  const fwExposureQ = trpc.ctem.getFrameworkExposure.useQuery(undefined, {
    staleTime: 60_000,
  });
  const vendorsQ = trpc.ctem.listVendorsForAssets.useQuery(undefined, {
    staleTime: 120_000,
  });
  ...
  const exposureTrendData = useMemo(() => {
    ...
    return [...runsQ.data].reverse().map((r, i) => ({
      name: `#${i + 1}`,
  ...
  const scoresByAssetId = useMemo(() => {
    ...
    return new Map(
      riskScoresQ.data.map((s: any) => [s.assetId ?? s.asset?.id, s])
    );
  }, [riskScoresQ.data]);
  ...
  const firstLoadError =
    ...
    fwExposureQ.error ??
    vendorsQ.error;
  const lastRun = summary?.lastRun as LastRunShape | null | undefined;
  const drift = lastRun?.scoreDelta ?? 0;
    ...
    <div className="djac-page">
              ...
              {(["critical", "high", "medium", "low"] as Tier[]).map(tier => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-full border px-2.5 py-0.5 text-[11px] font-semibold transition-opacity ${
      ...
      <div className="djac-stat-grid">
        <KpiCard
          ...
          sub={
            summaryUnavailable
              ...
              : `${summary?.criticalAssets ?? 0} critical`
          }
        ...
        <KpiCard
          ...
          sub={
            summaryUnavailable
              ? t("ctem.summaryUnavailable", "Summary unavailable")
              : `${summary?.exploitableVulns ?? 0} ${t("ctem.exploitable", "exploitable")}`
          }
        ...
        <KpiCard
          ...
          sub={
            summaryUnavailable
              ? t("ctem.summaryUnavailable", "Summary unavailable")
              : drift > 0
                ? `↑ ${drift} pts since last run`
                : drift < 0
                  ? `↓ ${Math.abs(drift)} pts since last run`
                  : "No change since last run"
          }
          colour={
            summary?.avgPriorityScore && summary.avgPriorityScore >= 60
              ? TIER_COLOURS.high
              : TIER_COLOURS.medium
          }
        />
      ...
      <div className="djac-chart-grid">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">
              Average priority score across the last {exposureTrendData.length}{" "}
              scans
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            {exposureTrendData.length === 0 ? (
              <div className="flex h-40 items-center justify-center text-sm text-muted-foreground">
                {t(
                  "ctem.noScanRuns",
                  "No scan runs yet — click Run Scan to begin"
                )}
              </div>
            ) : (
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">
              Vulnerabilities mapped per regulatory framework
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            {fwData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={180}>
                ...
                >
                  ...
                  <Bar
                    dataKey="Critical"
                    stackId="a"
                    fill={TIER_COLOURS.critical}
                  />
                  <Bar dataKey="High" stackId="a" fill={TIER_COLOURS.high} />
                  <Bar
                    dataKey="Medium"
                    stackId="a"
                    fill={TIER_COLOURS.medium}
                  />
                  <Bar
                    dataKey="Low"
                    stackId="a"
                    fill={TIER_COLOURS.low}
                    radius={[3, 3, 0, 0]}
                  />
                </BarChart>
              </ResponsiveContainer>
          ...
          {assetsQ.isLoading ? (
            ...
            <div className="flex flex-col items-center gap-2 py-12">
              ...
              <p className="text-sm text-muted-foreground">
                {t("ctem.assetsLoadError", "Failed to load assets.")}
              </p>
              <Button
                size="sm"
                variant="outline"
            ...
            <div className="flex flex-col items-center gap-2 py-16">
              ...
              <p className="text-sm font-medium">
                {t("ctem.noAssets", "No assets registered")}
              </p>
              <p className="max-w-xs text-center text-xs text-muted-foreground">
                {t(
                  "ctem.noAssetsSub",
                  "Add assets via the Vendor Assessment workflow to begin continuous monitoring"
                )}
              </p>
            </div>
          ) : (
                    ...
                    return (
                      <TableRow key={asset.id}>
                        ...
                        <TableCell className="py-2">
                          {score ? (
                            <TierBadge tier={score.priorityTier} />
                          ) : (
                        ...
                        <TableCell className="py-2">
                          {score ? (
                            <ScoreBar value={score.finalPriorityScore} />
                          ) : (
                            "—"
                          )}
                        </TableCell>
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">
            Unpatched vulnerabilities with known exploit code — sorted by CVSS
            score
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {vulnsQ.isLoading ? (
            ...
            <div className="flex flex-col items-center gap-2 py-10">
              ...
              <p className="text-sm text-muted-foreground">
                {t("ctem.vulnsLoadError", "Failed to load vulnerabilities.")}
              </p>
              <Button
                size="sm"
                variant="outline"
                    ...
                    <TableRow key={v.id}>
                      ...
                      <TableCell className="py-2">
                        <Button
                          ...
                          onClick={() =>
                            patchVuln.mutate({ id: v.id, isPatched: true })
                          }
                        >
                          {patchVuln.isPending ? (
              ...
              {lastRun && (
                <div className="rounded-md border p-3 text-xs space-y-1.5">
                  <div className="flex justify-between">
                    ...
                    <span>{formatRunTime(lastRun.startedAt)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{lastRun.assetsScanned ?? "—"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Vulns found</span>
                    <span>{lastRun.vulnsFound ?? "—"}</span>
                  </div>
                  {lastRun.alertRaised ? (
                    <div className="flex justify-between">
          ...
          <CardContent className="p-0">
            {runsQ.isLoading ? (
              ...
              <div className="flex flex-col items-center gap-2 py-10">
                <XCircle size={22} className="text-destructive" />
                <p className="text-sm text-muted-foreground">
                  {t("ctem.runsLoadError", "Failed to load scan history.")}
                </p>
                <Button
                  size="sm"
                  variant="outline"
                    ...
                    {runsQ.data.map((run: any) => (
                      <TableRow key={run.id}>
                        ...
                        <TableCell className="py-2">
                          <Badge
                            variant="outline"
                            className={`text-[10px] ${run.runStatus === "completed" ? "bg-green-500/10 text-green-500 border-green-500/30" : run.runStatus === "failed" ? "bg-red-500/10 text-red-500 border-red-500/30" : ""}`}
                          >
                            {run.runStatus}
                          </Badge>
                        </TableCell>
                        <TableCell className="py-2 text-xs capitalize text-muted-foreground">
                          {run.triggeredBy}
                        </TableCell>
                        <TableCell className="py-2 tabular-nums text-xs">
                          {run.assetsScanned ?? "—"}
                        ...
                        <TableCell className="py-2 tabular-nums text-xs">
                          {run.avgPriorityScore != null
                            ? Math.round(run.avgPriorityScore)
                            : "—"}
                        </TableCell>
                        <TableCell className="py-2 tabular-nums text-xs">
                          {run.scoreDelta != null ? (
      ...
      <blockquote className="rounded-lg border border-dashed p-4 text-xs italic text-muted-foreground">
        ...
        {t(
          "ctem.shiftLeft",
          "Continuous compliance means monitoring live exposure — not waiting for the annual audit."
        )}
        "
      </blockquote>
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {t(
                "ctem.addAssetDesc",
                "Register an IT asset for continuous threat exposure monitoring"
              )}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={v => setAssetForm({ ...assetForm, region: v })}
                >
                  <SelectTrigger className="text-xs">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1">
                <Label htmlFor="ipDomain">
                  {t("ctem.ipDomain", "IP / Domain")}
                </Label>
                <Input
                  id="ipDomain"
                  value={assetForm.ipDomain}
                  onChange={e =>
                    setAssetForm({ ...assetForm, ipDomain: e.target.value })
                  }
              ...
              <div className="col-span-2 space-y-2">
                <Label>
                  {t("ctem.criticalityScore", "Criticality Score")} (
                  {assetForm.criticalityScore}/10)
                </Label>
                <input
                  type="range"
                  min={1}
                  ...
                  onChange={e =>
                    setAssetForm({
                      ...assetForm,
                      criticalityScore: Number(e.target.value),
                    })
                  }
              ...
              {vendorsQ.data && vendorsQ.data.length > 0 && (
                <div className="col-span-2 space-y-1">
                  ...
                  <Select
                    ...
                    onValueChange={v =>
                      setAssetForm({ ...assetForm, vendorId: v })
                    }
                  >
                    <SelectTrigger className="text-xs">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                <Checkbox
                  ...
                  onCheckedChange={v =>
                    setAssetForm({ ...assetForm, isInternetFacing: !!v })
                  }
                />
                <Label htmlFor="internetFacing">
                  {t("ctem.internetFacing", "Internet-facing")}
                </Label>
              </div>
              <div className="flex items-center gap-3">
                <Checkbox
                  ...
                  onCheckedChange={v =>
                    setAssetForm({ ...assetForm, handlesPersonalData: !!v })
                  }
                />
                <Label htmlFor="personalData">
                  {t("ctem.personalData", "Handles Personal Data")}
                </Label>
              </div>
              <div className="flex items-center gap-3">
                <Checkbox
                  ...
                  onCheckedChange={v =>
                    setAssetForm({ ...assetForm, handlesCriticalData: !!v })
                  }
                />
                <Label htmlFor="criticalData">
                  {t("ctem.criticalData", "Handles Critical Data")}
                </Label>
              </div>
            </div>
            <div className="space-y-1">
            ...
            <Button
              ...
              onClick={() =>
                createAsset.mutate({
                  ...
                  assetType: assetForm.assetType as
                    | "web_application"
                    | "api_endpoint"
                    | "database"
                    | "cloud_service"
            ...
            >
              {createAsset.isPending ? (
                <>
                  ...
                  {t("ctem.registering", "Registering…")}
                </>
              ) : (
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {t(
                "ctem.addVulnDesc",
                "Record a vulnerability against a monitored asset"
              )}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1">
              ...
              <Select
                value={vulnForm.assetId}
                onValueChange={v => setVulnForm({ ...vulnForm, assetId: v })}
              >
                <SelectTrigger className="text-xs">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={e =>
                    setVulnForm({ ...vulnForm, cveId: e.target.value })
                  }
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={v => setVulnForm({ ...vulnForm, severity: v })}
                >
                  <SelectTrigger className="text-xs">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="space-y-1">
              <Label htmlFor="vulnTitle">
                {t("ctem.vulnTitle", "Title / Description")}
              </Label>
              <Input
                id="vulnTitle"
                value={vulnForm.title}
                onChange={e =>
                  setVulnForm({ ...vulnForm, title: e.target.value })
                }
            ...
            <div className="space-y-1">
              <Label>
                {t("ctem.cvssScore", "CVSS Score (0–100)")} —{" "}
                {(Number(vulnForm.cvssScore) / 10).toFixed(1)}/10
              </Label>
              <input
                type="range"
                min={0}
                ...
                onChange={e =>
                  setVulnForm({ ...vulnForm, cvssScore: e.target.value })
                }
                className="w-full accent-primary"
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                <Checkbox
                  ...
                  onCheckedChange={v =>
                    setVulnForm({ ...vulnForm, exploitAvailable: !!v })
                  }
                />
                <Label htmlFor="exploitAvail">
                  {t("ctem.exploitAvailableLabel", "Exploit Available")}
                </Label>
              </div>
              <div className="flex items-center gap-3">
                <Checkbox
                  ...
                  onCheckedChange={v =>
                    setVulnForm({ ...vulnForm, isConfirmed: !!v })
                  }
                />
                <Label htmlFor="isConfirmed">
                  {t("ctem.confirmed", "Confirmed")}
                </Label>
              </div>
            </div>
            <div className="space-y-1">
              ...
              <Textarea
                ...
                onChange={e =>
                  setVulnForm({ ...vulnForm, notes: e.target.value })
                }
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddVulnOpen(false)}>
              Cancel
            </Button>
            <Button
              disabled={
