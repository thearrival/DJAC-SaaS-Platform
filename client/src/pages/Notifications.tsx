...
function TypeIcon({
  ...
  if (type === "trial")
    return <Clock className={cls} style={{ color }} aria-hidden="true" />;
  if (type === "deadline")
    return (
      <CalendarCheck2 className={cls} style={{ color }} aria-hidden="true" />
    );
  if (type === "assessment")
    return <ShieldAlert className={cls} style={{ color }} aria-hidden="true" />;
...
export default function Notifications() {
  ...
  useEffect(() => {
    if (billingQuery.error)
      toast.error(
        t(
          "notifications.billingLoadError",
  ...
  useEffect(() => {
    if (deadlinesQuery.error)
      toast.error(
        t(
          "notifications.deadlinesLoadError",
          "Failed to load deadline notifications."
        )
      );
    ...
    <div className="djac-page">
      ...
      {hasLoadError && (
        <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-3">
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            {t(
              "notifications.retryHint",
              "Retry to refresh billing and deadline alerts."
            )}
          </p>
          <Button
            variant="outline"
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <TypeIcon
                    type={notif.type}
                    severity={isRead ? "info" : notif.severity}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p
                        className={`text-sm font-semibold leading-snug ${isRead ? "text-muted-foreground" : "text-foreground"}`}
                      >
                        {notif.title}
                        {!isRead && (
                          <span
                            ...
                            style={{
                              background: SEVERITY_COLOR[notif.severity],
                            }}
                            aria-label="Unread"
                          />
                        )}
                    ...
                    {notif.linkTo && (
                      <Link
                        ...
                        style={{
                          color:
                            SEVERITY_COLOR[isRead ? "info" : notif.severity],
                        }}
      ...
      {notifications.length > 0 && (
        <p className="text-xs text-muted-foreground text-center">
          {t("notifications.typeDeadline", "Compliance Deadline")} ·{" "}
          {t("notifications.typeTrialExpiring", "Trial Expiring")} ·{" "}
          {t("notifications.typeSystem", "System")}
          {" — "}
