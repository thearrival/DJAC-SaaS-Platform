...
function dueDateBadge(item: DsrItem) {
  ...
  return (
    <Badge
      variant="outline"
      className="text-xs bg-green-500/10 text-green-500 border-green-500/30 gap-1"
    >
...
function KpiCard({
  ...
  return (
    <Card
      className={`djac-stat-card${danger && Number(value) > 0 ? " border-red-500/40" : ""}`}
    >
      <CardContent className="flex items-center gap-4 p-5">
        ...
        <div>
          ...
          <p
            className={`text-2xl font-bold ${danger && Number(value) > 0 ? "text-red-500" : ""}`}
          >
            {value}
          </p>
        </div>
...
export default function DataSubjectRequests() {
  ...
  const openItems =
    summary?.open ?? items.filter(d => OPEN_STATUSES.includes(d.status)).length;
  const overdueItems =
    summary?.overdue ??
    items.filter(d => {
    ...
    <div className="djac-page">
          ...
          <CardContent className="py-4 flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {summaryQuery.error?.message ??
                t(
                  "dsr.summaryLoadError",
                  "Failed to load DSR summary metrics."
                )}
            </p>
            <Button
              size="sm"
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select
              ...
              onValueChange={v =>
                setFilterJurisdiction(v as Jurisdiction | "all")
              }
            >
              <SelectTrigger className="w-36 h-8 text-xs">
              ...
              <div className="text-center">
                <p className="text-lg font-semibold text-muted-foreground">
                  {listQuery.error?.message ??
                    t("dsr.loadError", "Failed to load DSR requests")}
                </p>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>
                      {t("dsr.colJurisdiction", "Jurisdiction")}
                    </TableHead>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{t("dsr.logRequest", "Log DSR Request")}</DialogTitle>
            <DialogDescription>
              {t(
                "dsr.dialogDesc",
          ...
          <div className="space-y-4 py-2">
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1.5 bg-muted/40 rounded px-3 py-2">
              ...
              Response deadline:{" "}
              <strong>{jurisdictionDeadline(form.jurisdiction)}</strong>
            </p>
