...
const PLAN_COLORS: Record<string, string> = {
  free_trial:
    "bg-amber-500/15 text-amber-700 dark:text-amber-400 border-amber-300/30",
  starter: "bg-sky-500/15 text-sky-700 dark:text-sky-400 border-sky-300/30",
  professional:
    "bg-violet-500/15 text-violet-700 dark:text-violet-400 border-violet-300/30",
  enterprise:
...
export default function OrgSettings() {
  ...
  useEffect(() => {
    if (orgQuery.error)
      toast.error(
        t("orgSettings.loadError", "Failed to load organization settings.")
      );
  }, [orgQuery.error]);
  const updateMut = trpc.orgSettings.update.useMutation({
    onSuccess: () => {
      toast.success(
        t("orgSettings.savedSuccess", "Organization settings saved")
      );
    ...
    <div className="djac-page">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {t(
              "orgSettings.subtitle",
              "Manage your organization's profile and compliance configuration."
            )}
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            {t(
              "orgSettings.identityDesc",
              "These values are set by the system and cannot be changed here."
            )}
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            {isAdmin
              ? t(
                  "orgSettings.profileDescAdmin",
                  "Edit your organization's display name, contact, and compliance configuration."
                )
              : t(
                  "orgSettings.profileDescReadOnly",
                  "You need owner or admin role to edit these settings."
                )}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSave} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                {t(
                  ...
                  "Receives invoices, trial reminders, and compliance deadline alerts."
                )}
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={t(
                  "orgSettings.industryPlaceholder",
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                {t(
                  "orgSettings.jurisdictionNote",
                  "Controls which compliance frameworks are highlighted across the platform."
                )}
              </p>
            </div>
          ...
          <CardContent>
            <div className="flex items-center justify-between gap-4 rounded-md border border-destructive/20 bg-destructive/5 p-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {t(
                    "orgSettings.deleteOrgDesc",
                    "You must cancel any active subscription before deleting. Contact support to proceed."
                  )}
                </p>
              </div>
              <Button
                ...
                onClick={() =>
                  window.location.assign(
                    "mailto:support@djac.io?subject=Delete+Organization+Request"
                  )
                }
              >
