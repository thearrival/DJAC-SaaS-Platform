  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {sub && (
          <p style={{ color: "var(--djac-muted)", fontSize: 10, marginTop: 5 }}>
            {sub}
          </p>
        )}
      </div>
...
function CalcCard({
  ...
  const nums = useMemo(
    () =>
      Object.fromEntries(
        fields.map(f => [f.key, parseSafe(vals[f.key] ?? "")])
      ),
    [vals, fields]
  );
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ fontSize: 11, color: "var(--djac-muted)", fontWeight: 600 }}
        >
          {resultLabel}
        </span>
...
export default function SaaSMetrics() {
  ...
  useEffect(() => {
    if (frameworksQuery.error)
      toast.error(
        t("saas.frameworksLoadError", "Failed to load compliance frameworks.")
      );
  }, [frameworksQuery.error]);
  useEffect(() => {
    if (matrixQuery.error)
      toast.error(
        t("saas.matrixLoadError", "Failed to load compliance matrix.")
      );
  }, [matrixQuery.error]);
  useEffect(() => {
    if (timetableQuery.error)
      toast.error(
        t("saas.timetableLoadError", "Failed to load compliance timetable.")
      );
  ...
  const liveKpis = useMemo(() => {
    ...
    const criticals = (matrix ?? []).filter(
      r => r.maxSeverity === "critical"
    ).length;
    ...
    const posture =
      totalPairs > 0
        ? Math.max(
            0,
            Math.round(100 - ((criticals * 14 + highs * 7) / totalPairs) * 100)
          )
        : 0;

    // Framework adoption rate (product adoption analogue)
    const adoption =
      fwCount > 0 ? Math.min(100, Math.round((fwCount / 5) * 100)) : 0;

    // Churn analogue: % of pairs with conflicts
    const conflictChurn =
      totalPairs > 0 ? Math.round(((criticals + highs) / totalPairs) * 100) : 0;
    ...
    <div className="djac-page">
      ...
      <div className="djac-section-1" style={{ marginBottom: 28 }}>
        ...
        >
          <div>
            ...
            >
              <span className="djac-gradient-text">
                SaaS Business Intelligence
              </span>
            </h1>
            <p
              style={{ color: "var(--djac-muted)", fontSize: 13, marginTop: 6 }}
            >
              {t(
                "saasMetrics.subtitle",
                "Track MRR · ARR · Churn · LTV · CAC · NRR · Rule of 40 — plus live compliance-scoped health KPIs."
              )}
            </p>
          </div>
      ...
      <div className="djac-section-1" style={{ marginBottom: 24 }}>
        <SectionTitle
          ...
          sub={t(
            "saasMetrics.liveKpisSub",
            "Derived from live framework matrix — compliance-scoped SaaS health signals"
          )}
        />
        {hasLiveQueryError ? (
          <div
          ...
          >
            <div>
              <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>
                {t(
                  "saasMetrics.liveDataError",
                  "Failed to load live compliance KPIs."
                )}
              </div>
              <div style={{ color: "var(--djac-muted)", fontSize: 13 }}>
                {t(
                  "saasMetrics.liveDataRetryHint",
                  "Retry to refresh the compliance-backed health metrics."
                )}
              </div>
            </div>
            <button
          ...
          <div
            ...
            className="djac-dash-kpi-grid"
          >
            <MetricCard
              label={t("saasMetrics.postureLabel", "Compliance Posture %")}
              value={`${liveKpis.posture}%`}
              sub={t(
                "saasMetrics.postureSubMrr",
                "Equivalent to platform health score"
              )}
              accent={
                liveKpis.posture >= 80
                  ? C.green
            ...
            <MetricCard
              ...
              sub={t(
                "saasMetrics.adoptionSub",
                "Active frameworks / 5 target coverage"
              )}
              accent={C.cyan}
            ...
            <MetricCard
              ...
              sub={t(
                "saasMetrics.conflictChurnSub",
                "% of pairs with critical/high risk"
              )}
              accent={
                liveKpis.conflictChurn > 20
                  ? C.red
                  : liveKpis.conflictChurn > 10
                    ? C.orange
                    : C.green
              }
              icon={
            ...
            <MetricCard
              label={t(
                "saasMetrics.obligationCoverageLabel",
                "Obligation Coverage"
              )}
              value={String(liveKpis.obligations)}
              sub={t(
                "saasMetrics.obligationCoverageSub",
                "Active regulatory obligations tracked"
              )}
              accent={C.purple}
              icon={
                <Target style={{ width: 18, height: 18, color: C.purple }} />
              }
              trend="neutral"
      ...
      <div className="djac-section-2" style={{ marginBottom: 24 }}>
        <SectionTitle
          icon={
            <DollarSign style={{ width: 16, height: 16, color: C.green }} />
          }
          title={t("saasMetrics.coreMetricsTitle", "Core Revenue Metrics")}
          sub={t(
            "saasMetrics.coreMetricsSub",
            "MRR · ARR · Churn · LTV · CAC — the foundation of every healthy SaaS business"
          )}
        />
        <div
          style={{
        ...
        >
          ...
          <MetricCard
            ...
            accent={C.cyan}
            icon={
              <BarChart3 style={{ width: 18, height: 18, color: C.cyan }} />
            }
            formula="ARR = MRR × 12"
          />
          <MetricCard
          ...
          <MetricCard
            ...
            accent={C.orange}
            icon={
              <RefreshCw style={{ width: 18, height: 18, color: C.orange }} />
            }
      ...
      <div className="djac-section-3" style={{ marginBottom: 24 }}>
        <SectionTitle
          ...
          title={t(
            "saasMetrics.acquisitionTitle",
            "Acquisition & Efficiency Metrics"
          )}
          sub={t(
            "saasMetrics.acquisitionSub",
            "CAC · CAC Payback · CAC:LTV Ratio · NRR · GRR"
          )}
        />
        <div
          style={{
        ...
        >
          <MetricCard
            ...
            accent={C.purple}
            icon={
              <DollarSign style={{ width: 18, height: 18, color: C.purple }} />
            }
          ...
          <MetricCard
            ...
            accent={C.cyan}
            icon={
              <RefreshCw style={{ width: 18, height: 18, color: C.cyan }} />
            }
          ...
          <MetricCard
            ...
            accent={C.yellow}
            icon={
              <ArrowUp style={{ width: 18, height: 18, color: C.yellow }} />
            }
          ...
          <MetricCard
            ...
            icon={
              <CheckCircle2
                style={{ width: 18, height: 18, color: C.orange }}
              />
            }
          ...
          <MetricCard
            ...
            value={
              liveKpis.totalPairs > 0 ? pct(100 - liveKpis.conflictChurn) : "—"
            }
            ...
            icon={
              <ShieldCheck
                style={{
                  width: 18,
                  height: 18,
                  color: liveKpis.conflictChurn < 20 ? C.green : C.red,
                }}
              />
            }
      ...
      >
        <SectionTitle
          ...
          title={t(
            "saasMetrics.calcTitle",
            "Interactive SaaS Metric Calculators"
          )}
          sub={t(
            "saasMetrics.calcSub",
        ...
        >
          ...
          <CalcCard
            ...
            compute={({ lost, startCustomers, revLost, startRev }) => {
              const logo =
                startCustomers > 0 ? (lost / startCustomers) * 100 : 0;
              const rev = startRev > 0 ? (revLost / startRev) * 100 : 0;
              return logo === 0 && rev === 0
                ? "—"
                : `${logo.toFixed(1)}%  |  ${rev.toFixed(1)}%`;
            }}
          />
          <CalcCard
            ...
            compute={({ startMrr, expansion, contraction, churnMrr }) => {
              ...
              const nrr =
                ((startMrr + expansion - contraction - churnMrr) / startMrr) *
                100;
      ...
      <div id="rule40" className="djac-section-4" style={{ marginBottom: 24 }}>
        <SectionTitle
          ...
          title={t(
            "saasMetrics.rule40Title",
            "Rule of 40 & Advanced Efficiency Metrics"
          )}
          sub={t(
            "saasMetrics.rule40Sub",
            "Balance growth and profitability — the investor benchmark for SaaS health"
          )}
        />
        <div
          style={{
        ...
        >
          ...
          <CalcCard
            ...
            fields={[
              {
                key: "mrrGrowth",
                label: "MRR Growth (consecutive months)",
                placeholder: "e.g. 3",
              },
              {
                key: "retentionMonths",
                label: "Retention Run Streak (months)",
                placeholder: "e.g. 3",
              },
              {
                key: "salesGrowth",
                label: "Sales Growth Streak",
                placeholder: "e.g. 2",
              },
            ]}
            ...
            compute={({ mrrGrowth, retentionMonths, salesGrowth }) => {
              const pass =
                mrrGrowth >= 3 && retentionMonths >= 3 && salesGrowth >= 2;
              return pass
                ? "✓ Rule satisfied"
                : `${mrrGrowth < 3 ? "MRR " : ""}${retentionMonths < 3 ? "Retention " : ""}${salesGrowth < 2 ? "Sales " : ""}gap`;
            }}
          />
        </div>
      </div>
      ...
      >
        <SectionTitle
          ...
          title={t(
            "saasMetrics.advancedTitle",
            "Advanced & Qualitative Metrics"
          )}
          sub={t(
            "saasMetrics.advancedSub",
            "Cohort analysis · Engagement scoring · Customer health · Compliance privacy metrics"
          )}
        />
        <div
          style={{
          ...
          className="djac-dash-insight-grid"
        >
          <InfoPanel
            icon={
              <BarChart3 style={{ width: 14, height: 14, color: C.cyan }} />
            }
            accent={C.cyan}
          ...
          <InfoPanel
            icon={
              <ShieldCheck style={{ width: 14, height: 14, color: C.purple }} />
            }
            accent={C.purple}
          ...
          <InfoPanel
            icon={
              <CheckCircle2 style={{ width: 14, height: 14, color: C.red }} />
            }
            accent={C.red}
      ...
      >
        ...
        >
          {[
            {
              label: "MRR",
              formula: "Customers × Avg Rev / Month",
              c: C.green,
            },
            { label: "ARR", formula: "MRR × 12", c: C.cyan },
            {
              label: "Logo Churn",
              formula: "Customers Lost / Start Customers",
              c: C.red,
            },
            {
              label: "Revenue Churn",
              formula: "Rev Lost / Start Revenue",
              c: C.orange,
            },
            {
              label: "LTV",
              formula: "(Avg MRR × Lifetime) − Support Cost",
              c: C.yellow,
            },
            {
              label: "CAC",
              formula: "Sales & Mkt Spend / Customers Acquired",
              c: C.purple,
            },
            {
              label: "CAC Payback",
            ...
            {
              label: "Magic Number",
              formula: "LTV / CAC  (>1.0 = sustainable)",
              c: C.yellow,
            },
            {
              label: "Burn Multiple",
              formula: "Burn Rate / Net New ARR  (<1.5 = efficient)",
              c: C.red,
            },
          ].map(row => (
            <div
      ...
      >
        <span style={{ color: "var(--djac-muted)", fontSize: 10 }}>
          DJAC Intelligence Platform · SaaS Metrics Module ·{" "}
          {t(
            "saasMetrics.footerNote",
            "Data privacy compliant — all calculations are client-side only"
          )}
        </span>
      </div>
    </div>
