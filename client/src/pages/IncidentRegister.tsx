  ...
  return (
    <Card className="flex-1 min-w-[110px]">
      <CardContent className="pt-4 pb-3 px-4">
        ...
        <p
          className={`text-2xl font-bold mt-1 ${alert ? "text-red-600 dark:text-red-400" : ""}`}
        >
          {value}
        </p>
      </CardContent>
...
function SeverityBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${col.bg} ${col.text}`}
    >
...
export default function IncidentRegister() {
  ...
  const listQ = trpc.incidentRegister.list.useQuery(undefined, {
    staleTime: 30_000,
  });
  ...
  const markNotifiedM = trpc.incidentRegister.markNotified.useMutation({
    onSuccess: () => {
      utils.incidentRegister.list.invalidate().catch(() => undefined);
      toast.success(
        t("incident.notificationSent", "Regulatory notification marked as sent")
      );
      sounds.success();
    },
    onError: err => toast.error(err.message),
  ...
  const incidents: IncidentRow[] =
    (listQ.data as unknown as IncidentRow[]) ?? [];
  ...
  function toggleArr(
    field: "affectedFrameworks" | "affectedDataTypes",
    val: string
  ) {
    setForm(f => ({
      ...f,
  ...
  function openEdit(i: IncidentRow) {
    ...
    setForm({
      ...
      occurredAt: i.occurredAt
        ? new Date(i.occurredAt).toISOString().slice(0, 16)
        : "",
      detectedAt: i.detectedAt
        ? new Date(i.detectedAt).toISOString().slice(0, 16)
        : "",
  ...
  function handleSubmit() {
    const payload = {
      ...
      affectedDataSubjects: form.affectedDataSubjects
        ? parseInt(form.affectedDataSubjects, 10)
        : undefined,
      ...
      notificationDeadlineHours:
        parseInt(form.notificationDeadlineHours, 10) || 72,
  ...
  return (
    <div className="djac-page">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">
              {t("incident.title", "Incident Register")}
            </h1>
            <p className="text-sm text-muted-foreground">
              {t(
                "incident.subtitle",
                "Track, investigate, and close compliance incidents and data breaches"
              )}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        <StatCard
          label={t("incident.statTotal", "Total")}
          value={stats.total}
        />
        <StatCard label={t("incident.statOpen", "Active")} value={stats.open} />
        <StatCard
          label={t("incident.statCritical", "Critical")}
          value={stats.critical}
          alert={stats.critical > 0}
        />
        <StatCard
          label={t("incident.statOverdueNotif", "Overdue Notification")}
          value={stats.overdueNotification}
          alert={stats.overdueNotification > 0}
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Select
          value={filterSev}
          onValueChange={v => setFilterSev(v as Severity | "all")}
        >
          <SelectTrigger className="h-9 w-40">
            <SelectValue
              placeholder={t("incident.filterSeverity", "All severities")}
            />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {t("incident.filterAllSeverities", "All severities")}
            </SelectItem>
            {SEVERITIES.map(s => (
              <SelectItem key={s} value={s}>
                {t(`incident.severity.${s}`, s)}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
        ...
        >
          <SelectTrigger className="h-9 w-44">
            <SelectValue
              placeholder={t("incident.filterStatus", "All statuses")}
            />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {t("incident.filterAllStatuses", "All statuses")}
            </SelectItem>
            {STATUSES.map(s => (
              <SelectItem key={s} value={s}>
                {t(`incident.status.${s}`, s)}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
        <Select
          value={filterType}
          onValueChange={v => setFilterType(v as IncidentType | "all")}
        >
          ...
          <SelectContent>
            <SelectItem value="all">
              {t("incident.filterAllTypes", "All types")}
            </SelectItem>
            {INCIDENT_TYPES.map(tp => (
              <SelectItem key={tp} value={tp}>
                {t(`incident.type.${tp}`, tp)}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      {listQ.isLoading ? (
        <div className="text-center py-16 text-muted-foreground">
          {t("incident.loading", "Loading incidents…")}
        </div>
      ) : listQ.isError ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-16 text-muted-foreground">
            ...
            <p className="font-semibold text-foreground">
              {t("incident.loadError", "Failed to load incidents")}
            </p>
            <QueryErrorPanel
              message={
                listQ.error?.message ??
                t(
                  "incident.loadErrorHint",
                  "Retry to refresh incident records."
                )
              }
              onRetry={() => {
                void listQ.refetch();
          ...
          {filtered.map(inc => (
            <IncidentCard
              ...
              onStatusChange={(id, status) =>
                updateStatusM.mutate({ id, status })
              }
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {editTarget
                ? t("incident.editTitle", "Edit Incident")
                : t("incident.newTitle", "Report Incident")}
            </DialogTitle>
            <DialogDescription>
              {t(
                "incident.formDesc",
                "Record the details of this compliance incident."
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e =>
                  setForm(f => ({ ...f, incidentCode: e.target.value }))
                }
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={v =>
                  setForm(f => ({ ...f, incidentType: v as IncidentType }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={form.status}
                onValueChange={v =>
                  setForm(f => ({ ...f, status: v as IncidentStatus }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e =>
                  setForm(f => ({ ...f, detectedAt: e.target.value }))
                }
              />
            </div>
            ...
            <div className="space-y-1.5">
              <Label>
                {t("incident.fieldDataSubjects", "Affected Data Subjects")}
              </Label>
              <Input
                type="number"
                min="0"
                ...
                onChange={e =>
                  setForm(f => ({ ...f, affectedDataSubjects: e.target.value }))
                }
                placeholder="0"
              />
            </div>
            ...
            <div className="space-y-1.5">
              <Label>
                {t(
                  "incident.fieldNotifDeadline",
                  "Notification Deadline (hours)"
                )}
              </Label>
              <Select
                value={form.notificationDeadlineHours}
                onValueChange={v =>
                  setForm(f => ({ ...f, notificationDeadlineHours: v }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="sm:col-span-2 flex items-center gap-2 pt-1">
              <Checkbox
                ...
                onCheckedChange={v =>
                  setForm(f => ({ ...f, regulatoryNotificationRequired: !!v }))
                }
              />
            ...
            <div className="sm:col-span-2 space-y-1.5">
              <Label>
                {t("incident.fieldFrameworks", "Affected Frameworks")}
              </Label>
                ...
                {FRAMEWORKS.map(fw => (
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={() =>
                        toggleArr("affectedFrameworks", fw)
                      }
                    />
                    {fw}
                  </label>
            ...
            <div className="sm:col-span-2 space-y-1.5">
              <Label>
                {t("incident.fieldDataTypes", "Affected Data Types")}
              </Label>
              <div className="flex flex-wrap gap-3 pt-1">
                {DATA_TYPES.map(dt => (
                  <label
            ...
            <div className="sm:col-span-2 space-y-1.5">
              ...
              <Textarea
                ...
                placeholder={t(
                  "incident.descPlaceholder",
                  "What happened? Initial scope, systems affected…"
                )}
              />
            </div>
            ...
            <div className="sm:col-span-2 space-y-1.5">
              ...
              <Textarea
                ...
                placeholder={t(
                  "incident.lessonsPlaceholder",
                  "What was learned? Corrective actions taken…"
                )}
              />
            </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("incident.deleteTitle", "Delete incident?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("incident.deleteDesc", "This will permanently delete")} &ldquo;
...
function IncidentCard({
  ...
  const notifRequired =
    i.regulatoryNotificationRequired && !i.regulatoryNotificationSentAt;
  const elapsed = hoursElapsed(i.occurredAt ?? i.detectedAt);
  const deadlineHours = i.notificationDeadlineHours ?? 72;
  const notifOverdue =
    notifRequired && elapsed !== null && elapsed > deadlineHours;
  const notifWarning =
    notifRequired && elapsed !== null && elapsed > deadlineHours * 0.75;
  const hoursLeft =
    elapsed !== null ? Math.max(0, deadlineHours - elapsed) : null;
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <SeverityBadge
                severity={i.severity}
                label={t(`incident.severity.${i.severity}`, i.severity)}
              />
              <StatusBadge
                status={i.status}
                label={t(`incident.status.${i.status}`, i.status)}
              />
              <span
            ...
            {notifRequired && (
              <div
                className={`flex items-center gap-1.5 mt-2 px-2 py-1 rounded text-xs font-medium ${notifOverdue ? "bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-300" : notifWarning ? "bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300" : "bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300"}`}
              >
                <Bell className="h-3.5 w-3.5 shrink-0" />
                {notifOverdue
                  ? t(
                      "incident.notifOverdue",
            ...
            {i.regulatoryNotificationSentAt && (
              <div className="flex items-center gap-1.5 mt-1.5 text-xs text-emerald-600 dark:text-emerald-400">
                ...
                {t("incident.notifiedAt", "Regulatory notification sent")}{" "}
                {formatDate(i.regulatoryNotificationSentAt)}
              </div>
            )}
            ...
            {dataTypes.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1.5">
                {dataTypes.map(dt => (
                  <span
                    key={dt}
                    className="text-xs bg-muted border text-muted-foreground px-1.5 py-0.5 rounded"
                  >
                    {dt}
                  </span>
                ))}
            ...
            <div className="flex flex-wrap gap-3 mt-2 text-xs text-muted-foreground">
              {i.occurredAt && (
                <span>
                  {t("incident.occurred", "Occurred")}:{" "}
                  {formatDate(i.occurredAt)}
                </span>
              )}
              {i.affectedDataSubjects != null && (
                <span>
                  {t("incident.dataSubjects", "Data subjects")}:{" "}
                  {i.affectedDataSubjects.toLocaleString()}
                </span>
              )}
            </div>
          </div>
