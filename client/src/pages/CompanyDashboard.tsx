...
function CompanyDashboardContent() {
  ...
  // ── Data queries ─────────────────────────────────────────────────────────
  const deadlineSummary = trpc.deadlines.summary.useQuery(undefined, {
  ...
  const vendorList = trpc.vendor.list.useQuery(undefined, {
  ...
  const assignUserRole = trpc.role.assignUserRole.useMutation({
    ...
    onError: err =>
      toast.error(
        t("companyDash.roleAssignError", "Failed to assign role: ") +
          err.message
      ),
  });

  useEffect(() => {
    if (deadlineSummary.error)
      toast.error(
        t("companyDash.deadlinesLoadError", "Failed to load deadlines.")
      );
  }, [deadlineSummary.error]);
  useEffect(() => {
    if (vendorList.error)
      toast.error(t("companyDash.vendorsLoadError", "Failed to load vendors."));
  }, [vendorList.error]);
  useEffect(() => {
    if (usersWithRoles.error)
      toast.error(t("companyDash.usersLoadError", "Failed to load users."));
  ...
  const isLoading =
    deadlineSummary.isLoading || deadlines.isLoading || vendorList.isLoading;
  const hasCoreLoadError =
    deadlineSummary.isError || deadlines.isError || vendorList.isError;
          ...
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              <Link
                ...
                className="ml-auto text-xs font-normal text-primary flex items-center gap-0.5 hover:opacity-75 transition-opacity"
              >
            ...
            {deadlines.isError ? (
              <div className="space-y-2">
                <p className="text-sm text-muted-foreground">
                  {t(
                    "companyDash.deadlinesLoadError",
                ...
                {(deadlines.data ?? []).map(d => (
                  ...
                  >
                    ...
                    <div className="flex shrink-0 flex-wrap gap-1.5">
                      <span
                        className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${priorityBadge[d.priority ?? "low"] ?? ""}`}
                      >
                        {d.priority}
                      </span>
                      <span
                        className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${statusBadge[d.status ?? "upcoming"] ?? ""}`}
                      >
                        {d.status}
                      </span>
                    </div>
        ...
        <Card>
          ...
          <CardContent>
            {vendorList.isError ? (
              <div className="space-y-2">
                <p className="text-sm text-muted-foreground">
                  {t("companyDash.vendorsLoadError", "Failed to load vendors.")}
                </p>
                <Button
                  size="sm"
                  variant="outline"
      ...
      {isPlatformAdmin && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t(
                "companyDash.roleManagementDesc",
                "Assign or change platform roles for registered users."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="mb-4 flex items-center gap-3">
              <label className="text-sm font-medium">
                {t("companyDash.filterByRole", "Filter:")}
              </label>
              <Select value={roleFilter} onValueChange={setRoleFilter}>
                ...
                <SelectContent>
                  <SelectItem value="all">
                    {t("companyDash.allRoles", "All roles")}
                  </SelectItem>
            ...
            {usersWithRoles.isLoading ? (
              ...
              <div className="space-y-2">
                <p className="text-sm text-muted-foreground">
                  {t("companyDash.usersLoadError", "Failed to load users.")}
                </p>
                <Button
                  size="sm"
                  variant="outline"
