...
function LocalUsersPanel() {
  ...
  const listQuery = trpc.localAuth.adminList.useQuery(undefined, {
    retry: false,
  });
        ...
        <CardContent className="py-8">
          <QueryErrorPanel
            message={
              ...
              t("admin.localUsersLoadError", "Failed to load local users.")
            }
            onRetry={() => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {t("admin.localUsersTitle", "Local Registered Users")}
        </CardTitle>
        <CardDescription>
          {t(
            "admin.localUsersDesc",
            "Users who registered directly via email and password (Visitor or Professional accounts)."
          )}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {listQuery.isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">
            {t("admin.localUsersEmpty", "No local registrations yet.")}
          </p>
        ) : (
              ...
              <thead>
                <tr className="border-b border-border text-xs text-muted-foreground">
                  <th className="pb-2 text-left font-medium">
                    {t("admin.localUsersColNameEmail", "Name / Email")}
                  </th>
                  <th className="pb-2 text-left font-medium">
                    {t("admin.localUsersColType", "Type")}
                  </th>
                  <th className="pb-2 text-left font-medium">
                    {t("admin.localUsersColCompanyRole", "Company / Role")}
                  </th>
                  <th className="pb-2 text-left font-medium">
                    {t("admin.localUsersColStatus", "Status")}
                  </th>
                  <th className="pb-2 text-left font-medium">
                    {t("admin.localUsersColJoined", "Joined")}
                  </th>
                  <th className="pb-2 text-left font-medium">
                    {t("admin.localUsersColActions", "Actions")}
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {rows.map(u => (
                  <tr key={u.id} className="py-2">
                    ...
                    <td className="py-2 pr-4">
                      <span
                        className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${typeColor[u.userType] ?? ""}`}
                      >
                        {u.userType}
                      </span>
                    </td>
                    ...
                    <td className="py-2 pr-4">
                      <span
                        className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${statusColor[u.status] ?? ""}`}
                      >
                        {u.status}
                      </span>
                    </td>
...
export default function AdminControlCenter() {
  ...
  const frameworkEdges: FrameworkEdge[] = useMemo(() => {
    ...
    for (const row of matrix) {
      ...
      const rel =
        (row.relationships ?? []).join("/") || (row.maxSeverity ?? "related");
  ...
  const accessRequestsQuery = trpc.admin.accessRequests.useQuery(
    { limit: 80 },
    { refetchInterval: 15000 }
  );
  const consultationsQuery = trpc.admin.consultations.useQuery(
    { limit: 80 },
  ...
  const assessmentsQuery = trpc.admin.assessments.useQuery(
    { limit: 80 },
    { refetchInterval: 15000 }
  );
  const vendorsQuery = trpc.admin.vendors.useQuery(
    { limit: 100 },
  ...
  const conversionStatsQuery = trpc.admin.conversionStats.useQuery(undefined, {
  ...
  const respondConsultationMutation =
    trpc.admin.respondConsultation.useMutation({
      onSuccess: async () => {
        sonnerToast.success(
          t("admin.toastConsultationUpdated", "Consultation updated")
        );
  ...
  const updateAccessRequestStatusMutation =
    trpc.admin.updateAccessRequestStatus.useMutation({
      onSuccess: async () => {
        sonnerToast.success(
          t("admin.toastAccessRequestUpdated", "Access request updated")
        );
      ...
      onError: () => {
        sonnerToast.error(
          t(
            "admin.toastAccessRequestUpdateError",
  ...
  const retentionMutation = trpc.admin.enforceInteractionRetention.useMutation({
    ...
    onError: () => {
      sonnerToast.error(
        t("admin.retentionRunError", "Failed to run retention operation.")
      );
    },
  });

  const deleteInteractionMutation =
    trpc.admin.deleteInteractionData.useMutation({
      onSuccess: async result => {
        sonnerToast.success(
          t(
            "admin.interactionDeleteDone",
            `Deleted ${result.deletedLogs} interaction logs.`
          )
        );
        setDeleteUserId("");
      ...
      onError: () => {
        sonnerToast.error(
          t(
            "admin.interactionDeleteError",
            "Failed to delete interaction logs."
          )
        );
      },
    });
  ...
  const handleDeleteInteractionData = async () => {
    const userId =
      deleteUserId.trim().length > 0 ? Number(deleteUserId) : undefined;
    const organizationId =
      deleteOrganizationId.trim().length > 0
        ? Number(deleteOrganizationId)
        : undefined;

    if (!userId && !organizationId) {
      sonnerToast.error(
        t(
          "admin.interactionDeleteTargetRequired",
          "Provide a user ID or organization ID."
        )
      );
      return;
    }
    ...
    if (
      ...
      (organizationId && Number.isNaN(organizationId))
    ) {
      sonnerToast.error(
        t(
          "admin.interactionDeleteTargetInvalid",
          "Deletion target IDs must be valid numbers."
        )
      );
      return;
    }
  ...
  const handleRespondConsultation = async (consultationId: number) => {
    ...
    if (response.length < 10) {
      sonnerToast.error(
        t(
          "admin.errorAdminResponseMinLength",
  ...
  const notificationHighlights =
    overviewQuery.data?.notificationHighlights ?? [];
  ...
  const isPrivacyActionPending =
    retentionMutation.isPending || deleteInteractionMutation.isPending;
  ...
  const hasCoreLoadError =
    ...
    privacyStatsQuery.isError ||
    matrixQuery.isError;
  const coreLoadErrorMsg =
    overviewQuery.error?.message ||
    usersQuery.error?.message ||
    accessRequestsQuery.error?.message ||
    consultationsQuery.error?.message ||
    notificationsQuery.error?.message ||
    assessmentsQuery.error?.message ||
    vendorsQuery.error?.message ||
    activityQuery.error?.message ||
    privacyStatsQuery.error?.message ||
  ...
  const frameworkLoadByCode = useMemo(() => {
    ...
    for (const row of assessmentsQuery.data ?? []) {
      ...
      const canonicalCode = normalizedCode.includes("PIPL")
        ? "PIPL"
  ...
  const maxFrameworkLoad = Math.max(
    ...Array.from(frameworkLoadByCode.values()),
    1
  );
    ...
    return (
      <div className="mx-auto max-w-3xl">
        <Card>
          <CardHeader>
            <CardTitle>
              {t("admin.signInTitle", "Admin Control Center")}
            </CardTitle>
            <CardDescription>
              {t(
                "admin.signInDesc",
                "Sign in with an admin account to access platform operations."
              )}
            </CardDescription>
          </CardHeader>
        </Card>
        ...
        <Card className="border-destructive/40">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-destructive">
              ...
              {t("admin.accessRestrictedTitle", "Access Restricted")}
            </CardTitle>
            <CardDescription>
              {t(
                "admin.accessRestrictedDesc",
                "The administrative control center is available only to users with the admin role."
              )}
            </CardDescription>
          </CardHeader>
        </Card>
    ...
    <div className="djac-page">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-300">
              {t("admin.heroBadge", "Admin Control Center")}
            </p>
            <h1 className="mt-2 text-3xl font-semibold">
              {t("admin.heroTitle", "Enterprise Governance Console")}
            </h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-200">
              {t(
                "admin.heroDesc",
                "Oversee client onboarding, consultation workflows, vendor risk posture, and real-time AI pipeline telemetry."
              )}
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-8">
        ...
        <Card className="col-span-1">
          <CardContent className="p-4">
            <p className="text-xs text-muted-foreground">
              {t("admin.accessRequests", "Access Requests")}
            </p>
            <p className="text-2xl font-semibold">
              {totals?.openAccessRequests ?? 0}
            </p>
          </CardContent>
        </Card>
        <Card className="col-span-1">
          <CardContent className="p-4">
            <p className="text-xs text-muted-foreground">
              {t("admin.consultations", "Consultations")}
            </p>
            <p className="text-2xl font-semibold">
              {totals?.openConsultations ?? 0}
            </p>
          </CardContent>
        </Card>
        <Card className="col-span-1">
        ...
        <Card className="col-span-1">
          <CardContent className="p-4">
            <p className="text-xs text-muted-foreground">
              {t("admin.criticalGaps", "Critical Gaps")}
            </p>
            <p className="text-2xl font-semibold text-destructive">
              {totals?.criticalGaps ?? 0}
            </p>
          </CardContent>
        </Card>
        <Card className="col-span-1">
          <CardContent className="p-4">
            <p className="text-xs text-muted-foreground">
              {t("admin.unreadAlerts", "Unread Alerts")}
            </p>
            <p className="text-2xl font-semibold">
              {totals?.unreadNotifications ?? 0}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            {t("admin.privacyTitle", "Interaction Privacy Controls")}
          </CardTitle>
          <CardDescription>
            {t(
              "admin.privacyDesc",
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="rounded-lg border border-border p-3">
              <p className="text-xs text-muted-foreground">
                {t("admin.privacyTotalLogs", "Total Logs")}
              </p>
              <p className="text-2xl font-semibold">
                {privacyStatsQuery.data?.totalLogs ?? 0}
              </p>
            </div>
            <div className="rounded-lg border border-border p-3">
              <p className="text-xs text-muted-foreground">
                {t("admin.privacyOldLogs", "Older Than Retention")}
              </p>
              <p className="text-2xl font-semibold text-amber-600">
                {privacyStatsQuery.data?.logsOlderThanRetention ?? 0}
              </p>
            </div>
            <div className="rounded-lg border border-border p-3">
              <p className="text-xs text-muted-foreground">
                {t("admin.privacyCutoff", "Cutoff Date")}
              </p>
              <p className="text-sm font-medium">
                {formatDateTime(privacyStatsQuery.data?.cutoffIso ?? null)}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div className="space-y-3 rounded-lg border border-border p-3">
              <Label htmlFor="retentionDays">
                {t("admin.retentionDays", "Retention Days")}
              </Label>
              <Input
                id="retentionDays"
                type="number"
                ...
                onChange={event =>
                  setRetentionDays(
                    Math.max(7, Math.min(365, Number(event.target.value) || 90))
                  )
                }
              />
              <p className="text-xs text-muted-foreground">
                {t(
                  "admin.retentionHint",
                  "Run Dry Run first to preview how many logs will be removed."
                )}
              </p>
              ...
              {retentionMutation.isPending ? (
                ...
                >
                  {t(
                    "admin.privacyActionRunning",
                    "Privacy operation in progress."
                  )}
                </p>
              ) : null}
            </div>
            ...
            <div className="space-y-3 rounded-lg border border-border p-3">
              ...
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                ...
                <Input
                  type="number"
                  placeholder={t(
                    "admin.organizationIdPlaceholder",
                    "Organization ID"
                  )}
              ...
              <p className="text-xs text-muted-foreground">
                {t(
                  "admin.deleteShortcutHint",
                  "Provide a User ID or Organization ID, then press Ctrl+Enter to delete."
                )}
              </p>
              <Button
                variant="destructive"
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              {t("admin.metricsTitle", "Compliance Metrics")}
            </CardTitle>
            <CardDescription>
              {t(
                "admin.metricsDesc",
                "Daily platform activity, risk distribution, and top recent assessments."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div className="rounded-lg border border-border p-3">
              <p className="mb-2 text-sm font-medium">
                {t("admin.activity7Day", "7-Day Activity")}
              </p>
              {activitySeries.length === 0 ? (
                <EmptyPanel
                  title={t("admin.emptyPanelTitle", "Nothing to show yet")}
                  message={t(
                    "admin.noActivityPoints",
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-7 gap-1 text-[10px] text-muted-foreground">
                    {activitySeries.map(point => (
                      <span key={point.day} className="truncate text-center">
                        {point.day}
                      </span>
                    ))}
                  </div>
            ...
            <div className="rounded-lg border border-border p-3 lg:col-span-2">
              <p className="mb-2 text-sm font-medium">
                {t("admin.recentAssessmentScores", "Recent Assessment Scores")}
              </p>
              {recentAssessments.length === 0 ? (
                <EmptyPanel
                  title={t("admin.emptyPanelTitle", "Nothing to show yet")}
                  message={t(
                    "admin.noAssessmentsRecorded",
            ...
            <div className="rounded-lg border border-border p-3 lg:col-span-2">
              <div className="mb-2 flex items-center justify-between gap-2">
                <p className="text-sm font-medium">
                  {t("admin.usageHeatmap", "Feature Usage Heatmap")}
                </p>
              ...
              {!usageHeatmap || usageHeatmap.totalEvents === 0 ? (
                <EmptyPanel
                  title={t("admin.emptyPanelTitle", "Nothing to show yet")}
                  message={t(
                    "admin.usageHeatmapEmpty",
                    "No interaction telemetry captured yet."
                  )}
                />
              ) : (
                <div className="space-y-3">
                      ...
                      <tbody>
                        {usageHeatmap.contexts.map(context => (
                          <tr key={context}>
                            ...
                            {usageHeatmap.actions.map(action => {
                              const value =
                                heatmapCellLookup.get(
                                  `${context}::${action}`
                                ) ?? 0;
                              const ratio =
                                usageHeatmap.maxCellValue > 0
                                  ? value / usageHeatmap.maxCellValue
                                  : 0;
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("admin.usageTotalEvents", "Total events in window")}:{" "}
                    {usageHeatmap.totalEvents}
                  </p>
                </div>
              )}
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              {t("admin.matrixTitle", "Cross-Jurisdiction Comparison Matrix")}
            </CardTitle>
            <CardDescription>
              {t(
                "admin.matrixDesc",
          ...
          <CardContent className="space-y-4">
            {regionCoverage.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                {t(
                  "admin.matrixNoData",
                  "No region coverage metrics available yet."
                )}
              </p>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge variant="outline">
                        {t("admin.matrixPressure", "Pressure")}: {pressureIndex}
                      </Badge>
                    </div>
                    ...
                    <div className="mt-3">
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span>
                          {t("admin.riskPressureIndex", "Risk Pressure Index")}
                        </span>
                      ...
                      <div className="mt-1 h-2 rounded-full bg-muted">
                        <div
                          ...
                          style={{
                            width: `${Math.max(0, Math.min(100, pressureWidth))}%`,
                          }}
                        />
                      </div>
                    </div>
            ...
            <div className="rounded-lg border border-border bg-white/90 p-3 dark:bg-slate-950/50">
              <svg viewBox="0 0 560 340" className="h-[300px] w-full">
                ...
                {frameworkEdges.map(edge => {
                  const source = frameworkNodes.find(
                    node => node.code === edge.source
                  );
                  const target = frameworkNodes.find(
                    node => node.code === edge.target
                  );
                  if (!source || !target) {
                    return null;
                  }
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              {t("admin.aiFeedTitle", "Real-Time AI Pipeline Feed")}
            </CardTitle>
            <CardDescription>
              {t(
                "admin.aiFeedDesc",
                "Observe running job stages and inspect detailed progress events."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {selectedSnapshot ? (
              <AIAssessmentJobProgress snapshot={selectedSnapshot} />
            ) : null}
            <div className="max-h-[280px] space-y-2 overflow-auto rounded-lg border border-border p-3">
              {sortedSnapshots.length === 0 ? (
                <EmptyPanel
                  title={t("admin.emptyPanelTitle", "Nothing to show yet")}
                  message={t(
                    "admin.aiFeedNoSnapshots",
                    "No active job snapshots received yet."
                  )}
                />
              ) : (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-medium">
                        {snapshot.id.slice(0, 8)} · {snapshot.stage}
                      </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              {t("admin.notificationsTitle", "Notification Highlights")}
            </CardTitle>
            <CardDescription>
              {t(
                "admin.notificationsDesc",
                "Fast triage for unread events and operational updates."
              )}
            </CardDescription>
          </CardHeader>
      ...
      <Tabs defaultValue="consultations" className="space-y-3">
        <TabsList className="w-full justify-start overflow-x-auto">
          <TabsTrigger value="consultations">
            ...
            {t("admin.tabConsultations", "Consultations")}
          </TabsTrigger>
          ...
          <TabsTrigger value="access">
            ...
            {t("admin.tabAccess", "Access Requests")}
          </TabsTrigger>
          <TabsTrigger value="assessments">
            <ShieldAlert className="h-4 w-4" />
            {t("admin.tabAssessments", "Assessments")}
          </TabsTrigger>
          <TabsTrigger value="vendors">
            <Globe2 className="h-4 w-4" />
            {t("admin.tabVendors", "Vendors")}
          </TabsTrigger>
          <TabsTrigger value="localusers">
            <UserPlus className="h-4 w-4" />
            {t("admin.tabLocalUsers", "Local Users")}
          </TabsTrigger>
          <TabsTrigger value="conversion">
            <ChartBar className="h-4 w-4" />
            {t("admin.tabConversion", "Conversion Analytics")}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="consultations">
          <Card>
            <CardHeader>
              <CardTitle>
                {t(
                  "admin.consultationQueueTitle",
                  "Consultation Workflow Queue"
                )}
              </CardTitle>
              <CardDescription>
                {t(
                  "admin.consultationQueueDesc",
                  "Respond to complex requests and update workflow status."
                )}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {consultationsQuery.isError ? (
                ...
                (consultationsQuery.data ?? []).map(item => {
                  const statusValue =
                    consultationStatusDrafts[item.id] ??
                    (item.status === "new"
                      ? "in_review"
                      : (item.status as ConsultationMutationStatus));
                  const priorityValue =
                    consultationPriorityDrafts[item.id] ??
                    (item.priority as ConsultationPriority);
                  const responseValue =
                    responseDrafts[item.id] ?? item.adminResponse ?? "";
                  ...
                  return (
                    ...
                    >
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="in_review">
                                {t("admin.statusInReview", "In review")}
                              </SelectItem>
                              <SelectItem value="responded">
                                {t("admin.statusResponded", "Responded")}
                              </SelectItem>
                              <SelectItem value="closed">
                                {t("admin.statusClosed", "Closed")}
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                      ...
                      <div className="mt-3">
                        <Label>
                          {t("admin.adminResponse", "Admin Response")}
                        </Label>
                        <Textarea
                          rows={3}
                          className="mt-1"
                        ...
                        />
                        <p
                          id={`admin-response-hint-${item.id}`}
                          className="mt-1 text-xs text-muted-foreground"
                        >
                          {t(
                            "admin.consultationShortcutHint",
                            "Press Ctrl+Enter to save this response."
                          )}
                        </p>
                      </div>
        ...
        <TabsContent value="users">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("admin.usersTitle", "User Access Governance")}
              </CardTitle>
              <CardDescription>
                {t(
                  "admin.usersDesc",
                  "Review role assignments and client lifecycle status."
                )}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
              {usersQuery.isError ? (
                <QueryErrorPanel
                  message={
                    usersQuery.error?.message ??
                    t("admin.usersLoadError", "Failed to load users.")
                  }
                  onRetry={() => {
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          <p className="text-sm font-semibold">
                            {account.name ??
                              t("admin.unknownUser", "Unknown User")}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {account.email} ·{" "}
                            {t("admin.lastActivity", "Last activity")}{" "}
                            {formatDateTime(account.lastActivityAt)}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <Badge variant={statusBadgeVariant(account.role)}>
                            {account.role}
                          </Badge>
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="invited">
                                {t("admin.statusInvited", "Invited")}
                              </SelectItem>
                              <SelectItem value="suspended">
                                {t("admin.statusSuspended", "Suspended")}
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
        ...
        <TabsContent value="access">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("admin.accessQueueTitle", "Access Request Queue")}
              </CardTitle>
              <CardDescription>
                {t(
                  "admin.accessQueueDesc",
                  "Monitor inbound registrations from prospective clients."
                )}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
        ...
        <TabsContent value="assessments">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("admin.assessmentRegistryTitle", "Assessment Registry")}
              </CardTitle>
              <CardDescription>
                {t(
                  "admin.assessmentRegistryDesc",
                  "Persisted cross-border compliance assessments and gap severity."
                )}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline">
                          {t("admin.criticalGapsShort", "Critical gaps")}:{" "}
                          {assessment.criticalGapCount}
                        </Badge>
        ...
        <TabsContent value="vendors">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("admin.vendorPortfolioTitle", "Vendor Portfolio Overview")}
              </CardTitle>
              <CardDescription>
                {t(
                  "admin.vendorPortfolioDesc",
                  "Organization ownership, region footprint, and technology profile depth."
                )}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
              {vendorsQuery.isError ? (
                <QueryErrorPanel
                  message={
                    vendorsQuery.error?.message ??
                    t("admin.vendorsLoadError", "Failed to load vendors.")
                  }
                  onRetry={() => {
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {t("admin.owner", "Owner")}: {vendor.ownerName} ·{" "}
                          {vendor.ownerOrganization}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant={statusBadgeVariant(vendor.riskTier)}>
                          {vendor.riskTier}
                        </Badge>
                        <Badge variant="outline">
                          {t("admin.techComponents", "Tech components")}:{" "}
                          {vendor.techStackCount}
                        </Badge>
                      </div>
                    </div>
                    <div className="mt-2 flex flex-wrap gap-1">
                      {vendor.operatingCountries.map(country => (
                        <Badge
                          key={`${vendor.id}-ops-${country}`}
                          variant="secondary"
                        >
                          {t("admin.opsPrefix", "Ops")}: {country}
                        </Badge>
                      ))}
                      {vendor.dataLocations.map(country => (
                        <Badge
                          key={`${vendor.id}-data-${country}`}
                          variant="outline"
                        >
                          {t("admin.dataPrefix", "Data")}: {country}
                        </Badge>
                      ))}
                    </div>
                  </div>
        ...
        <TabsContent value="conversion">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("admin.conversionTitle", "Conversion Analytics")}
              </CardTitle>
              <CardDescription>
                {t(
                  "admin.conversionDesc",
                  "Overview of trial-to-paid conversion across all organizations."
                )}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {conversionStatsQuery.isLoading ? (
                ...
                <div className="py-8 text-center text-sm">
                  <p className="text-destructive">
                    {conversionStatsQuery.error.message ??
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                        <div className="rounded-lg border border-border p-4 text-center">
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {t("admin.conversionTrialOrgs", "Active Trials")}
                          </p>
                        </div>
                        <div className="rounded-lg border border-border p-4 text-center">
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {t("admin.conversionExpiredOrgs", "Expired Trials")}
                          </p>
                        </div>
                        <div className="rounded-lg border border-border p-4 text-center">
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {t("admin.conversionPaidOrgs", "Paid Subscribers")}
                          </p>
                        </div>
                        <div className="rounded-lg border border-border p-4 text-center">
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {t("admin.conversionRate", "Conversion Rate")}
                          </p>
                        </div>
                      </div>
                      {stats.planBreakdown.length > 0 && (
                        <div>
                          <p className="mb-2 text-sm font-medium">
                            {t("admin.conversionTitle", "Plan Breakdown")}
                          </p>
                          <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>
              {t("admin.activityFeedTitle", "Recent Activity Feed")}
            </CardTitle>
            <CardDescription>
              {t(
                "admin.activityFeedDesc",
                "Audit-level interaction stream across admin, client, and system actors."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="max-h-[320px] space-y-2 overflow-auto">
            {activityQuery.isError ? (
              <QueryErrorPanel
                message={
                  activityQuery.error?.message ??
                  t("admin.activityLoadError", "Failed to load activity feed.")
                }
                onRetry={() => {
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {t(
                "admin.consultationSnapshotTitle",
                "Recent Consultation Snapshot"
              )}
            </CardTitle>
            <CardDescription>
              {t(
                "admin.consultationSnapshotDesc",
                "Latest client requests with assigned response ownership."
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="max-h-[320px] space-y-2 overflow-auto">
