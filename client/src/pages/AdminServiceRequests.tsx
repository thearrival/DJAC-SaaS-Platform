...
export default function AdminServiceRequests() {
  ...
  const {
    ...
    isError,
    refetch,
  } = trpc.serviceRequests.adminList.useQuery(undefined, {
  ...
  const stats = {
    ...
    open: requests.filter(r => !["completed", "cancelled"].includes(r.status))
      .length,
  ...
  function handleAdvancePipeline(req: ServiceRequest) {
    ...
    updateMutation.mutate({
      id: req.id,
      status: next as Parameters<typeof updateMutation.mutate>[0]["status"],
    });
  }
                ...
                <div className="grid grid-cols-2 gap-3 text-sm bg-muted/30 rounded-lg p-4">
                  <div>
                    ...
                    <p className="font-medium mt-0.5">
                      {formatServiceType(selected.serviceType)}
                    </p>
                  </div>
                  <div>
