 ...
 * powered by the rule-based multi-jurisdiction assessment engine covering 28 jurisdictions.
 *
 * Features:
...
function ScorePill({ label, score }: { label: string; score: number }) {
  ...
  return (
    ...
    >
      ...
      <span
        style={{ fontSize: 10, color: "var(--djac-muted-foreground, #94a3b8)" }}
      >
        {label}
      </span>
    </div>
      ...
      <CollapsibleTrigger asChild>
        <button
          ...
          style={{
            ...
            background: open ? SEVERITY_CONFIG[gap.severity].bg : "transparent",
            cursor: "pointer",
            textAlign: "start",
          ...
          <Collapsible open={expanded} onOpenChange={setExpanded}>
            <CollapsibleTrigger asChild>
              ...
              >
                ...
                {assessment.gaps.length !== 1 ? "s" : ""} identified
              </button>
            </CollapsibleTrigger>
            <CollapsibleContent>
...
export default function GapTracker() {
  ...
  const { data, isLoading, error, refetch } = trpc.vendor.gapSummary.useQuery(
    undefined,
    {
      staleTime: 120_000,
    }
  ...
  useEffect(() => {
    if (error) {
      toast.error(
        t("gapTracker.loadError", "Failed to load gap analysis") +
          ": " +
          error.message
      );
    }
  ...
  const filteredEntries = useMemo(() => {
    ...
    return data
      ...
      .filter(
        entry =>
          entry.assessment.gaps.length > 0 ||
    ...
    <div className="djac-page">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        >
          {t(
            "gapTracker.subtitle",
            "Rule-based multi-jurisdiction compliance gap analysis across all registered vendors. Identifies gaps across 28 jurisdictions including UK GDPR, PIPEDA, APPI, POPIA, and 40+ frameworks with remediation guidance."
          )}
        </p>
      </div>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
          {(
            ["all", "china", "saudi", "cross_border"] as JurisdictionFilter[]
          ).map(jur => (
            <button
              key={jur}
              type="button"
        ...
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {filteredEntries.map(entry => (
              <VendorCard
                key={`vendor-${entry.vendor.id}`}
                entry={entry as GapEntry}
              />
            ))}
          </div>
        </>
