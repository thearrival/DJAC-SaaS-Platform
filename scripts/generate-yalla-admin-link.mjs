...
const secret = (
  getArg("secret", process.env.YALLA_ADMIN_SECRET || "") || ""
).trim();
...
const nonceMode = (getArg("nonce-mode", "one-time") || "one-time")
  .trim()
  .toLowerCase();
...
if (!secret) {
  console.error(
    "Missing YALLA_ADMIN_SECRET. Provide --secret or export YALLA_ADMIN_SECRET."
  );
  process.exit(1);
}
