...
async function postTrpc(path, payload, headers = {}) {
  const response = await fetchWithHint(
...
async function run() {
  ...
  if (mode === "strict") {
    assert(
      register.status >= 500,
      "register should fail in strict mode without DB"
    );
    assert(
      access.status >= 500,
      "access request should fail in strict mode without DB"
    );
    assert(
  ...
  assert(
    access.status === 200,
    "access request should succeed in permissive mode"
  );
  assert(
  ...
  const needsVerification =
    registerBody?.result?.data?.json?.pendingVerification === true;

  if (needsVerification) {
    // OTP/verification flow: no session cookie until verified
    log(
      "register",
      "pendingVerification=true (OTP flow — no session cookie expected)"
    );
  } else {
    const cookiePair = (register.setCookie.split(";")[0] || "").trim();
    assert(
      cookiePair.startsWith("djac_local_session="),
      "register should set local auth cookie"
    );
    ...
    assert(
      meBody.includes(`"email":"${email}"`),
      "localAuth.me should include the new user"
    );
  }
  ...
  assert(
    login.status === 200,
    "localAuth.login should succeed with valid credentials"
  );

  const sessionCookie = (login.setCookie || register.setCookie || "")
    .split(";")[0]
    .trim();
  ...
  assert(
    report.status === 200,
    "compliance.report should succeed for authenticated user"
  );
  assert(
    report.text.includes("markdown"),
    "compliance.report should return markdown payload"
  );
  assert(
    reportPdf.status === 200,
    "compliance.reportPdf should succeed for authenticated user"
  );
  assert(
    reportPdf.text.includes("application/pdf"),
    "compliance.reportPdf should return PDF metadata"
  );
  assert(
    reportDocx.status === 200,
    "compliance.reportDocx should succeed for authenticated user"
  );
  assert(
    reportDocx.text.includes(
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    ),
    "compliance.reportDocx should return DOCX metadata"
  );
  ...
  if (checkEmail) {
    ...
    assert(
      reportEmail.status === 200,
      "compliance.emailReport should succeed when SMOKE_CHECK_REPORT_EMAIL=true"
    );
  }
