...
if (showHelp) {
  ...
  console.log("Options:");
  console.log(
    "  --env-file <path>   Load environment values from this file (default: .env.production)"
  );
  console.log("  --json              Output machine-readable JSON summary");
  console.log(
    "  --fail-on-warn      Treat warnings as blocking failures (strict mode)"
  );
  console.log(
    "  --require-billing   Require complete Stripe billing configuration"
  );
...
addRequired("JWT_SECRET", has("JWT_SECRET"), "JWT_SECRET is required.");
...
addRequired("DATABASE_URL", has("DATABASE_URL"), "DATABASE_URL is required.");

addRequired(
  "DATABASE_URL_FORMAT",
  isValidUrl(get("DATABASE_URL"), [
    "postgresql:",
    "postgres:",
    "mysql:",
    "mariadb:",
...
addRequired("APP_URL", has("APP_URL"), "APP_URL is required.");
...
addRequired(
  "YALLA_ADMIN_SECRET",
  has("YALLA_ADMIN_SECRET") &&
    get("YALLA_ADMIN_SECRET") !== "change-me-in-production-long-random-string",
...
const stripeAnyConfigured =
  stripeSecretKey || stripeWebhookSecret || configuredStripePrices.length > 0;
...
if (stripeAnyConfigured) {
  ...
  for (const key of STRIPE_PRICE_KEYS) {
    addRequired(key, has(key), `${key} is required when Stripe is enabled.`);
  }
} else {
  if (requireBilling) {
    addRequired(
...
const recommendedWarnings = checks.filter(
  c => c.severity === "recommended" && !c.ok
);
const passed = checks.filter(c => c.ok).length;

const blocked =
  requiredFailures.length > 0 || (failOnWarn && recommendedWarnings.length > 0);
...
console.log(
  `Env file: ${resolvedEnvFile} ${fs.existsSync(resolvedEnvFile) ? "(loaded)" : "(not found, using process env)"}`
);
console.log(
  `Options: failOnWarn=${failOnWarn} requireBilling=${requireBilling}`
);
console.log("");
